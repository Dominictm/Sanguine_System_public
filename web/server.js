...
const {
  ...
  parseCharacter, parseLocation, parseChronicleLocation,
...
const aiBusy = require('./lib/ai-busy');
...
const { trashRouterFactory } = require('./routes/trash');
...
// Миниатюры арта: /city-img-thumb/<тот же путь, что у /city-img>?w=640|96.
// Делаются лениво при первом запросе и лежат в cities/<city>/.thumbs/.
// Любая осечка (нет файла, WebP, ошибка ресайза) — редирект на исходник:
// интерфейс не должен ломаться из-за миниатюры.
app.get('/city-img-thumb/*', async (req, res) => {
  const original = req.path.replace('/city-img-thumb', '/city-img');
  try {
    const width = parseInt(req.query.w, 10);
    const thumbAbs = await ensureThumb(req.params[0], width);
    if (!thumbAbs) return res.redirect(302, original);
    res.set('X-Content-Type-Options', 'nosniff');
    res.sendFile(thumbAbs, { maxAge: '1h' }, err => { if (err && !res.headersSent) res.redirect(302, original); });
  } catch {
    if (!res.headersSent) res.redirect(302, original);
  }
});
...
const ACTION_MAP = {
  ...
  // ── Cities ────────────────────────────────────────────────────────────────
  'GET /api/cities':                          () => 'Города — список',
  'GET /api/cities/summary':                  () => 'Города — сводка',
  'GET /api/cities/:slug/detail':             req => `Город — карточка: ${D(req.params.slug)}`,
  'GET /api/cities/:slug/factions-list':      req => `Фракции города: ${D(req.params.slug)}`,
  'GET /api/cities/:slug/districts':          req => `Районы: ${D(req.params.slug)}`,
  'POST /api/cities':                         req => `➕ Создание города: ${D(req.body?.slug || '?')}`,
  'PUT /api/cities/:slug':                    req => `✏  Редактирование города: ${D(req.params.slug)}`,
  'DELETE /api/cities/:slug':                 req => `🗑 Удаление города: ${D(req.params.slug)}`,
  // ── Characters ────────────────────────────────────────────────────────────
  'GET /api/characters':                      req => `Персонажи — загрузка (${reqCity(req)})`,
  'GET /api/characters/all-images':           req => `Карусель — загрузка всех артов (${reqCity(req)})`,
  'GET /api/characters/:slug/images':         req => `Арты персонажа: ${D(req.params.slug)}`,
  'GET /api/characters/:slug/delete-preview': req => `🗑 Превью удаления: ${D(req.params.slug)}`,
  'GET /api/characters/:slug/diary':          req => `Дневник: ${D(req.params.slug)} → ${req.query.file || '?'}`,
  'GET /api/characters/:slug/sheet':          req => `Лист V20: ${D(req.params.slug)}`,
  'GET /api/characters/:slug/sheet-data':     req => `Лист V20 (данные): ${D(req.params.slug)}`,
  'POST /api/characters':                     req => `➕ Создание персонажа: ${D(req.body?.slug || req.body?.name || '?')} (${reqCity(req)})`,
  'POST /api/characters/:slug/upload-image':  req => `📷 Загрузка изображения → ${D(req.params.slug)}`,
  'POST /api/characters/:slug/upload-belonging': req => `🧳 Добавление имущества → ${D(req.params.slug)}`,
  'POST /api/characters/:slug/generate-appearance': req => `🤖 Генерация внешности: ${D(req.params.slug)}`,
  'POST /api/characters/:slug/generate-prompt':    req => `🎨 Генерация промта: ${D(req.params.slug)}`,
  'POST /api/characters/:slug/generate-personality': req => `🎭 Генерация характера и голоса: ${D(req.params.slug)}`,
  'POST /api/characters/:slug/generate-biography':   req => `📖 Генерация биографии: ${D(req.params.slug)}`,
  'POST /api/characters/:slug/diary/generate':      req => `📝 Генерация дневника: ${D(req.params.slug)}`,
  'POST /api/characters/:id/dialogue':       req => `💬 Диалог: ${D(req.params.id)}`,
  'POST /api/characters/:slug/sheet/generate': req => `🤖 Генерация V20-листа: ${D(req.params.slug)}`,
  'POST /api/export/characters':              () => `📤 Экспорт персонажей`,
  'POST /api/import/characters':              () => `📥 Импорт персонажей`,
  'PUT /api/characters/:slug/fields':         req => `✏  Редактирование полей: ${D(req.params.slug)}`,
  'PUT /api/characters/:slug/relations':      req => `✏  Редактирование отношений: ${D(req.params.slug)}`,
  'PUT /api/characters/:slug/diary':          req => `✏  Сохранение дневника: ${D(req.params.slug)}`,
  'PUT /api/characters/:slug/sheet':          req => `✏  Сохранение V20-листа: ${D(req.params.slug)}`,
  'PUT /api/characters/:slug/sheet-data':     req => `✏  Сохранение V20-данных: ${D(req.params.slug)}`,
  'PUT /api/characters/:slug/belonging/:filename': req => `🧳 Правка имущества: ${D(req.params.filename)} ← ${D(req.params.slug)}`,
  'DELETE /api/characters/:slug':             req => `🗑 Удаление персонажа: ${D(req.params.slug)}`,
  'DELETE /api/characters/:slug/images/:filename':  req => `🗑 Удаление арта: ${D(req.params.filename)} ← ${D(req.params.slug)}`,
  'DELETE /api/characters/:slug/belonging/:filename': req => `🗑 Удаление имущества: ${D(req.params.filename)} ← ${D(req.params.slug)}`,
  // ── Locations ─────────────────────────────────────────────────────────────
  'GET /api/locations':                       req => `Локации — загрузка (${reqCity(req)})`,
  'GET /api/locations/:slug/images':          req => `Арты локации: ${D(req.params.slug)}`,
  'GET /api/locations/:slug/backlinks':       req => `Обратные ссылки локации: ${D(req.params.slug)}`,
  'POST /api/locations':                      req => `➕ Создание локации: ${D(req.body?.slug || req.body?.name || '?')} (${reqCity(req)})`,
  'POST /api/locations/:slug/upload-image':   req => `📷 Загрузка изображения локации → ${D(req.params.slug)}`,
  'POST /api/locations/generate':             req => `🤖 Генерация локаций (${reqCity(req)})`,
  'POST /api/locations/parse-generated':      req => `🤖 Парсинг сгенерированных локаций`,
  'PUT /api/locations/:slug/fields':          req => `✏  Редактирование локации: ${D(req.params.slug)}`,
  'PUT /api/locations/:slug/district':        req => `✏  Смена района: ${D(req.params.slug)}`,
  'DELETE /api/locations/:slug':              req => `🗑 Удаление локации: ${D(req.params.slug)}`,
  // ── Graph ─────────────────────────────────────────────────────────────────
  'GET /api/graph':                           req => `Граф связей (${reqCity(req)})`,
  // ── Chronicles ────────────────────────────────────────────────────────────
  'GET /api/chronicle':                       req => `Хроника (${reqCity(req)})`,
  'GET /api/chronicles':                      req => `Хроники — список (${reqCity(req)})`,
  'GET /api/chronicles/:slug/fields':         req => `Хроника — поля: ${D(req.params.slug)}`,
  'GET /api/chronicles/:slug/events':         req => `События хроники: ${D(req.params.slug)}`,
  'GET /api/chronicles/:slug/book-data':      req => `Данные «книги»: ${D(req.params.slug)}`,
  'GET /api/chronicles/:slug/delete-preview': req => `🗑 Превью удаления хроники: ${D(req.params.slug)}`,
  'POST /api/chronicles':                     req => `➕ Создание хроники: ${D(req.body?.slug || '?')} (${reqCity(req)})`,
  'POST /api/chronicles/:slug/recap':         req => `📺 Рекап «Ранее в хронике…»: ${D(req.params.slug)}`,
  'PUT /api/chronicles/:slug/fields':         req => `✏  Редактирование хроники: ${D(req.params.slug)}`,
  'DELETE /api/chronicles/:slug':             req => `🗑 Удаление хроники: ${D(req.params.slug)}`,
  // ── Modules ───────────────────────────────────────────────────────────────
  'GET /api/modules':                         req => `Модули — загрузка (${reqCity(req)})`,
  'GET /api/modules/:name':                   req => `Модуль: ${D(req.params.name)}`,
  'GET /api/chronicles/:slug/modules':        req => `Модули хроники: ${D(req.params.slug)}`,
  'GET /api/chronicles/:chr/modules/:mod/detail': req => `Детали модуля: ${req.params.mod} (${req.params.chr})`,
  'GET /api/chronicles/:chr/modules/:mod/delete-preview': req => `🗑 Превью удаления модуля: ${req.params.mod} (${req.params.chr})`,
  'GET /api/chronicles/:chr/modules/:mod/session-notes': req => `📝 Заметки сессии: ${req.params.mod}`,
  'GET /api/chronicles/:chr/modules/:mod/scene-notes': req => `📝 Заметки сцены: ${req.params.mod}`,
  'POST /api/chronicles/:slug/modules':      req => `➕ Создание модуля: ${D(req.body?.slug || '?')} (${D(req.params.slug)})`,
  'POST /api/chronicles/:chr/modules/:mod/fill': req => `🤖 Заполнение модуля: ${req.params.mod} (${req.params.chr})`,
  'POST /api/chronicles/:chr/modules/:mod/close': req => `🏁 Закрытие модуля: ${req.params.mod} (${req.params.chr})`,
  'POST /api/chronicles/:chr/modules/:mod/npc': req => `👤 Добавление НПС в модуль: ${req.params.mod} (${req.params.chr})`,
  'POST /api/chronicles/:chr/modules/:mod/session': req => `➕ Добавление сессии в модуль: ${req.params.mod} (${req.params.chr})`,
  'POST /api/chronicles/:chr/modules/:mod/scenario/section/regenerate': req => `🤖 Регенерация раздела сценария: ${req.params.mod}`,
  'POST /api/chronicles/:chr/modules/:mod/scenario/block/regenerate': req => `🤖 Регенерация блока сценария: ${req.params.mod}`,
  'POST /api/chronicles/:chr/modules/:mod/scenario/scene': req => `➕ Добавление сцены: ${req.params.mod}`,
  'POST /api/chronicles/:chr/modules/:mod/locations': req => `📍 Локация модуля: ${req.params.mod}`,
  'POST /api/chronicles/:chr/modules/:mod/npc/:slug/sheet/generate': req => `🤖 Генерация листа НПС: ${D(req.params.slug)} (${req.params.mod})`,
  'POST /api/chronicles/:chr/modules/:mod/npc/:slug/promote': req => `⬆  Превращение НПС в персонажа: ${D(req.params.slug)}`,
  'PUT /api/chronicles/:chr/modules/:mod/fields': req => `✏  Поля модуля: ${req.params.mod} (${req.params.chr})`,
  'PUT /api/chronicles/:chr/modules/:mod/scenario': req => `📝 Сценарий модуля: ${req.params.mod} (${req.params.chr})`,
  'PUT /api/chronicles/:chr/modules/:mod/scenario/section': req => `📝 Раздел сценария: ${req.params.mod}`,
  'PUT /api/chronicles/:chr/modules/:mod/scenario/block/fields': req => `📝 Блок сценария: ${req.params.mod}`,
  'PUT /api/chronicles/:chr/modules/:mod/finale': req => `🏁 Финал модуля: ${req.params.mod} (${req.params.chr})`,
  'PUT /api/chronicles/:chr/modules/:mod/move': req => `🔀 Перемещение модуля: ${req.params.mod}`,
  'PUT /api/chronicles/:chr/modules/:mod/session/:idx': req => `✏  Сессия #${req.params.idx}: ${req.params.mod}`,
  'PUT /api/chronicles/:chr/modules/:mod/session-notes': req => `📝 Заметки сессии: ${req.params.mod}`,
  'PUT /api/chronicles/:chr/modules/:mod/scene-note': req => `📝 Заметка сцены: ${req.params.mod}`,
  'PUT /api/chronicles/:chr/modules/:mod/npc/:slug/sheet': req => `✏  Лист НПС: ${D(req.params.slug)}`,
  'DELETE /api/chronicles/:chr/modules/:mod': req => `🗑 Удаление модуля: ${req.params.mod} (${req.params.chr})`,
  'DELETE /api/chronicles/:chr/modules/:mod/session/:idx': req => `🗑 Удаление сессии #${req.params.idx}: ${req.params.mod}`,
  'DELETE /api/chronicles/:chr/modules/:mod/npc': req => `🗑 Удаление НПС из модуля: ${req.params.mod}`,
  'DELETE /api/chronicles/:chr/modules/:mod/locations/:locSlug': req => `🗑 Удаление локации модуля: ${D(req.params.locSlug)}`,
  // ── NPC-промот (после JSON) ──────────────────────────────────────────────
  'GET /api/chronicles/:chr/modules/:mod/npc/:slug/sheet':      req => `Лист НПС: ${D(req.params.slug)} (${req.params.mod})`,
  'GET /api/chronicles/:chr/modules/:mod/npc/:slug/promote-check': req => `Проверка превращения НПС: ${D(req.params.slug)}`,
  // ── Threads ───────────────────────────────────────────────────────────────
  'GET /api/threads':                         req => `Открытые нити (${reqCity(req)})`,
  'POST /api/threads':                        req => `➕ Новая нить: ${D(req.body?.title || '?')} (${reqCity(req)})`,
  'PATCH /api/threads/:id':                   req => `✏  Нить #${req.params.id} (${reqCity(req)})`,
  // ── Archive ───────────────────────────────────────────────────────────────
  'GET /api/timeline':                        req => `Хронология (${reqCity(req)})`,
  'GET /api/factions':                        req => `Фракции (${reqCity(req)})`,
  'GET /api/visitors':                        req => `Визитёры (${reqCity(req)})`,
  'GET /api/rumors':                          req => `Слухи (${reqCity(req)})`,
  ...
  // ── Archive (structured) ──────────────────────────────────────────────────
  'GET /api/timeline/structured':             req => `Хронология — структура (${reqCity(req)})`,
  'POST /api/timeline/epoch':                 req => `➕ Эпоха: ${D(req.body?.heading || '?')} (${reqCity(req)})`,
  'POST /api/timeline/epoch/:heading/row':    req => `➕ Строка эпохи: ${D(req.params.heading)}`,
  'PUT /api/timeline/epoch/:heading/row/:index': req => `✏  Строка #${req.params.index} [${D(req.params.heading)}]`,
  'DELETE /api/timeline/epoch/:heading':      req => `🗑 Удаление эпохи: ${D(req.params.heading)}`,
  'DELETE /api/timeline/epoch/:heading/row/:index': req => `🗑 Удаление строки #${req.params.index} [${D(req.params.heading)}]`,
  'GET /api/factions/influence':              req => `Влияние фракций (${reqCity(req)})`,
  'PUT /api/factions/influence':              req => `✏  Влияние фракций (${reqCity(req)})`,
  'DELETE /api/factions/influence/:name':     req => `🗑 Фракция: ${D(req.params.name)}`,
  // ── World state ───────────────────────────────────────────────────────────
  'GET /api/world-state/structured':          req => `Мировое состояние (${reqCity(req)})`,
  'GET /api/world-state/raw':                 req => `Мировое состояние — сырой текст (${reqCity(req)})`,
  'PUT /api/world-state/raw':                 req => `✏  Мировое состояние — сохранение (${reqCity(req)})`,
  'PUT /api/world-state/last-update':         req => `✏  Дата обновления мира (${reqCity(req)})`,
  'POST /api/world-state/section':            req => `➕ Раздел мира: ${D(req.body?.heading || '?')}`,
  'POST /api/world-state/section/:heading/row': req => `➕ Строка раздела: ${D(req.params.heading)}`,
  'PUT /api/world-state/section/:heading/note': req => `✏  Заметка раздела: ${D(req.params.heading)}`,
  'PUT /api/world-state/section/:heading/row/:index': req => `✏  Строка #${req.params.index} [${D(req.params.heading)}]`,
  'DELETE /api/world-state/section/:heading': req => `🗑 Удаление раздела: ${D(req.params.heading)}`,
  'DELETE /api/world-state/section/:heading/row/:index': req => `🗑 Удаление строки #${req.params.index} [${D(req.params.heading)}]`,
  // ── Search ────────────────────────────────────────────────────────────────
  'GET /api/search':    req => `🔍 Поиск: «${req.query.q || ''}» (${reqCity(req)})`,
  // ── Audio ─────────────────────────────────────────────────────────────────
  'GET /api/audio':                           req => `Аудио — загрузка (${reqCity(req)})`,
  'POST /api/audio':                          req => `➕ Добавление аудио → ${reqCity(req)}`,
  'PUT /api/audio/:id':                       req => `✏  Аудио #${req.params.id}`,
  'DELETE /api/audio/:id':                    req => `🗑 Удаление аудио #${req.params.id}`,
  'GET /api/audio/presets':                   req => `Пресеты аудио (${reqCity(req)})`,
  'POST /api/audio/presets':                  req => `➕ Новый пресет → ${reqCity(req)}`,
  'PUT /api/audio/presets/:id':               req => `✏  Пресет #${req.params.id}`,
  'DELETE /api/audio/presets/:id':            req => `🗑 Удаление пресета #${req.params.id}`,
  // ── Backup ────────────────────────────────────────────────────────────────
  'GET /api/backup/cities-info':              () => `💾 Бэкап — список городов`,
  'POST /api/backup/city':                    req => `💾 Бэкап города: ${D(req.body?.slug || '?')}`,
  'GET /api/backup/job/:id':                  req => `💾 Статус бэкап-задачи: ${req.params.id}`,
  'GET /api/backup/job/:id/download':         req => `💾 Скачивание бэкапа: ${req.params.id}`,
  'POST /api/backup/settings':                () => `💾 Бэкап настроек`,
  'POST /api/backup/settings/restore':        () => `💾 Восстановление настроек`,
  'POST /api/backup/restore/inspect':         () => `💾 Просмотр архива перед восстановлением`,
  'POST /api/backup/restore/commit':          () => `💾 Подтверждение восстановления`,
  // ── Trash ─────────────────────────────────────────────────────────────────
  'GET /api/trash':                           req => `🗑 Удалённые (${reqCity(req)})`,
  'POST /api/trash/restore':                  req => `♻️  Восстановление из корзины`,
  'POST /api/trash/bulk':                     req => `🗑 Массовое удаление`,
  'DELETE /api/trash':                        req => `🗑 Безвозвратное удаление (${reqCity(req)})`,
  // ── Library ───────────────────────────────────────────────────────────────
  'GET /api/library/disciplines':             () => `📚 Дисциплины`,
  'GET /api/library/psychics':                () => `📚 Психические способности`,
  'GET /api/library/clans':                   () => `📚 Кланы`,
  'GET /api/library/sects':                   () => `📚 Секты`,
  'GET /api/library/titles':                  () => `📚 Титулы`,
  'GET /api/library/combo-disciplines':       () => `📚 Комбо-дисциплины`,
  'GET /api/library/merits':                  () => `📚 Достоинства`,
  'GET /api/library/merits/:category':        req => `📚 Достоинства: ${D(req.params.category)}`,
  'GET /api/library/flaws':                   () => `📚 Недостатки`,
  'GET /api/library/flaws/:category':         req => `📚 Недостатки: ${D(req.params.category)}`,
  'GET /api/library/backgrounds':             () => `📚 Происхождения`,
  'GET /api/library/backgrounds/:category':   req => `📚 Происхождения: ${D(req.params.category)}`,
  'POST /api/library/disciplines':            req => `➕ Новая дисциплина: ${D(req.body?.slug || '?')}`,
  'POST /api/library/psychics':               req => `➕ Новая псих. способность: ${D(req.body?.slug || '?')}`,
  'POST /api/library/clans':                  req => `➕ Новый клан: ${D(req.body?.slug || '?')}`,
  'POST /api/library/sects':                  req => `➕ Новая секта: ${D(req.body?.slug || '?')}`,
  'POST /api/library/titles':                 req => `➕ Новый титул: ${D(req.body?.slug || '?')}`,
  'PUT /api/library/disciplines/:slug':       req => `✏  Дисциплина: ${D(req.params.slug)}`,
  'PUT /api/library/psychics/:slug':          req => `✏  Псих. способность: ${D(req.params.slug)}`,
  'PUT /api/library/clans/:slug':             req => `✏  Клан: ${D(req.params.slug)}`,
  'PUT /api/library/sects/:slug':             req => `✏  Секта: ${D(req.params.slug)}`,
  'PUT /api/library/titles/:slug':            req => `✏  Титул: ${D(req.params.slug)}`,
  'DELETE /api/library/disciplines/:slug':    req => `🗑 Дисциплина: ${D(req.params.slug)}`,
  'DELETE /api/library/psychics/:slug':       req => `🗑 Псих. способность: ${D(req.params.slug)}`,
  'DELETE /api/library/clans/:slug':          req => `🗑 Клан: ${D(req.params.slug)}`,
  'DELETE /api/library/sects/:slug':          req => `🗑 Секта: ${D(req.params.slug)}`,
  'DELETE /api/library/titles/:slug':         req => `🗑 Титул: ${D(req.params.slug)}`,
  // ── Tools / auth ──────────────────────────────────────────────────────────
  'GET /api/auth-status':                     () => 'Статус авторизации Claude',
  'GET /api/claude/status':                   () => 'Claude — статус',
  'GET /api/claude/health':                   () => 'Claude API — проверка',
  'GET /api/guide':                           () => '📖 Онбординг-тура',
  'GET /api/settings/probe-local':            () => 'Settings — проверка локальных файлов',
  'GET /api/settings':                        () => 'Settings — загрузка',
  'POST /api/settings':                       () => 'Settings — сохранение',
  ...
  'POST /api/canon-check':                    req => `✅ Canon-check: ${D(req.body?.slug || '?')}`,
  'POST /api/claude/generate-prose':          req => `🤖 Генерация текста (${req.body?.type || '?'})`,
  'POST /api/claude/oauth/start':             () => '🔑 OAuth — начало авторизации',
  'POST /api/claude/oauth/exchange':          () => '🔑 OAuth — обмен кода',
  'POST /api/claude/oauth/refresh':           () => '🔑 OAuth — обновление токена',
  // ── AI generation (external providers) ────────────────────────────────────
  'GET /api/openrouter/models':              () => `📋 Модели OpenRouter`,
  'GET /api/lmstudio/models':                () => `📋 Модели LM Studio`,
  'POST /api/openrouter/generate-prose':     req => `🤖 Проза (OpenRouter): ${req.body?.type || '?'}`,
  'POST /api/director/propose':              req => `🎬 AI Director: ${req.body?.type || '?'}`,
};
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    console.log(`${C.dim}[web]${C.reset} ${_logTime()} ${methodStr} ${codeStr} ${timeStr}  ${label}`);
  });
...
app.use(toolsRouterFactory({
  ...
  lmStudioAvailable:  () => _lmStudioAvailable(),
  scheduleRestart: (...a) => scheduleRestart(...a),
  isSupervised:    () => SUPERVISED,
  envPath:         () => ENV_PATH,
  defaultClaudeModel: () => DEFAULT_CLAUDE_MODEL,
}));
app.use(backupRouterFactory({
  envPath:                () => ENV_PATH,
  invalidateGeminiClient: () => { _geminiClient = null; },
  scheduleRestart:        (...a) => scheduleRestart(...a),
  isSupervised:           () => SUPERVISED,
}));
app.use(trashRouterFactory());

// ── Markdown / card / chronicle parsers ───────────────────────────────────────
// parseCharacter, parseLocation, parseChronicle* and the md* helpers
...
async function _lmStudioAvailable() {
  try {
    const baseUrl = process.env.LMSTUDIO_BASE_URL || 'http://localhost:1234/v1';
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 1500);
    const resp = await fetch(baseUrl + '/models', { signal: ctrl.signal });
    clearTimeout(timer);
    return resp.ok;
  } catch { return false; }
}
...
async function generateGeminiText(systemInstruction, userPrompt, config = {}) {
  aiBusy.begin();
  try {
    return await _generateGeminiTextInner(systemInstruction, userPrompt, config);
  } finally { aiBusy.end(); }
}

async function _generateGeminiTextInner(systemInstruction, userPrompt, config = {}) {
...
function _mockGenText(system = '', user = '') {
  ...
  if (/создай полный сценарий/i.test(s))
    return `<!-- meta:npcs: Заглушка НПС; Заглушка Антагонист -->
<!-- meta:locations: Опера -->
...
Заглушка для автотестов: конфликт уже назревал до начала модуля. Старая задолженность между котерией и местным кладбищенским родом тянулась через несколько сессий, и теперь проблема выходит наружу.

## Пролог — Завязка

### Описание для игрока
Заглушка для автотестов: завязка сюжета. Котерия прибывает в указанное место по вызову, но обнаруживает, что всё не так просто, как казалось на первый взгляд.

## Сцена 1 — Место действия

### Описание для игрока
Заглушка для автотестов: первая сцена. Детали места и NPC, ожидающего героев, описаны в колорите ниже. Неожиданный поворот заставляет пересмотреть первоначальный план.

### Колорит
1. Заглушка деталь места — описание окружения для создания атмосферы.

## Сцена 2 — Развитие

### Описание для игрока
Заглушка для автотестов: вторая сцена. Противоречивые интересы участников сталкиваются; котерия должна сделать выбор, сохраняя Маскарад.

### Колорит
1. Заглушка для деталей окружения — звуки, запахи, освещение.

## Финал — Развязка

### Описание для игрока
Заглушка для автотестов: развязка сюжета. Последствия выбора котерии определяют дальнейшие отношения и открывают новые сюжетные нити.
...
| Ещё один вопрос | Другая нить |

## Колорит — три обязательные детали
1. Заглушка детали — сенсорные детали для атмосферы модуля.
2. Вторая деталь — звуковой фон сцены.
3. Третья деталь — тактильные ощущения персонажей.`;
...
async function makeGenerationClient(preferSource = null, modelOverride = null) {
  ...
  const wantLmStudio = preferSource === 'lmstudio';
  const wantClaude = preferSource === 'claude';
  const wantNonClaude = wantGemini || wantOR || wantOpenAI || wantLmStudio;
  ...
  // ── LM Studio (explicit only, локальный сервер) ─────────────
  if (wantLmStudio) {
    return { source: 'lmstudio', model: modelOverride || process.env.LMSTUDIO_MODEL || null };
  }
...
function _oaEndpoint(provider) {
  ...
  if (provider === 'lmstudio') {
    return {
      url: (process.env.LMSTUDIO_BASE_URL || 'http://localhost:1234/v1') + '/chat/completions',
...
// Парсит тело ответа OpenAI-совместимого провайдера. Для LM Studio (и OpenAI/OpenRouter в потоке)
// поддерживает SSE-стрим (`stream: true`): склеивает чанки delta.content / delta.reasoning_content
// в единый ответ, для обычного JSON просто возвращает разобранный объект.
async function _readOaBody(resp, provider) {
  const ct = (resp.headers.get('content-type') || '').toLowerCase();
  if (ct.includes('text/event-stream')) {
    const text = await _readSSE(resp);
    if (!text) throw Object.assign(new Error(`${provider === 'lmstudio' ? 'LM Studio' : 'Провайдер'} вернул пустой ответ в потоке`), { status: 502 });
    return { choices: [{ message: { content: text } }] };
  }
  return resp.json();
}

// Распарсить SSE-поток chat/completions: строки `data: {...}` до `data: [DONE]`.
// reasoning_content и content приходят разными чанками — копим их отдельно и
// возвращаем content (финальный ответ); reasoning — только как запасной вариант,
// если модель отдала лишь цепочку рассуждений (обрезанный ответ).
async function _readSSE(resp) {
  ...
  for (;;) {
    ...
    while ((idx = buffer.indexOf('\n')) !== -1) {
      ...
      if (payload === '[DONE]') return content.trim() || reasoning.trim();
      try {
...
async function _chatCompletion({ provider, model, systemPrompt, userPrompt, imageBuffers = [], timeoutMs = 75000, maxTokens = 1500 }) {
  ...
  // LM Studio (и закрытый модель-роутер) при пустом модели-имени использует
  // загруженную в настоящий момент модель — поле `model` тогда опускаем целиком,
  // иначе сервер попытается загрузить модель с дословным именем «null» и упадёт.
  if (model) payload.model = model;
  payload.max_tokens = maxTokens;
  // LM Studio (llama-server) по умолчанию рвёт соединение через 600 с (--timeout),
  // а полный сценарий генерируется дольше (~10+ мин). Streaming (SSE) шлёт чанки
  // непрерывно — idle-таймаут сервера не срабатывает, и ответ доходит целиком.
  if (provider === 'lmstudio') payload.stream = true;
  const body = JSON.stringify(payload);

  const { url, headers } = _oaEndpoint(provider);
  const label = provider === 'openai' ? 'OpenAI' : provider === 'lmstudio' ? 'LM Studio' : 'OpenRouter';

  // Abort the request if the model never responds, so callers don't hang forever.
  const ctrl  = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), timeoutMs);
  const _isAbort = e => (e?.name === 'AbortError')
    || (e?.cause?.name === 'AbortError')           // undici: TypeError: fetch failed { cause: AbortError }
    || (e?.cause?.code === 'UND_ERR_ABORTED')
    || (e?.code === 'UND_ERR_ABORTED');
  let resp;
  try {
    resp = await fetch(url, { method: 'POST', headers, body, signal: ctrl.signal });
  } catch (e) {
    if (_isAbort(e)) throw Object.assign(new Error(`Модель «${model}» не ответила за ${Math.round(timeoutMs / 1000)}с`), { status: 504 });
    // Локальный сервер (LM Studio) может оборвать соединение в середине длинной
    // генерации (ECONNRESET / «fetch failed») — это сетевой обрыв, не отказ модели.
    // Для LM Studio пробуем ещё раз; для облачных провайдеров отдаём понятную ошибку.
    if (provider === 'lmstudio') {
      await new Promise(r => setTimeout(r, 1000));
      const ctrl2  = new AbortController();
      const timer2 = setTimeout(() => ctrl2.abort(), timeoutMs);
      try {
        resp = await fetch(url, { method: 'POST', headers, body, signal: ctrl2.signal });
      } catch (e2) {
        if (_isAbort(e2)) throw Object.assign(new Error(`Модель «${model}» не ответила за ${Math.round(timeoutMs / 1000)}с`), { status: 504 });
        throw Object.assign(new Error(`LM Studio оборвал соединение во время генерации (${e2.message || e2.code || 'сеть недоступна'}). Проверьте, что сервер запущен и модель не выгружается из-за нехватки памяти.`), { status: 502 });
      } finally {
        clearTimeout(timer2);
      }
    } else {
  ...
  const data = await _readOaBody(resp, provider);
  const msg   = data.choices?.[0]?.message;
  // Some reasoning models return content: null — fall back to reasoning text.
  // reasoning_content (DeepSeek-R1 и т.п.) берём как запасной вариант —
  // если модель отдала только его (content пуст из-за таймаута), это лучше, чем пустота.
  const text  = (msg?.content || msg?.reasoning || msg?.reasoning_content || '').trim();
  if (!text) {
...
async function callOpenRouter(model, systemPrompt, userPrompt, imageBuffers, timeoutMs = 75000, maxTokens = 1500) {
  aiBusy.begin();
  try { return await _chatCompletion({ provider: 'openrouter', model, systemPrompt, userPrompt, imageBuffers: imageBuffers || [], timeoutMs, maxTokens }); }
  finally { aiBusy.end(); }
}
async function callOpenAI(model, systemPrompt, userPrompt, imageBuffers, timeoutMs = 75000, maxTokens = 1500) {
  aiBusy.begin();
  try { return await _chatCompletion({ provider: 'openai', model, systemPrompt, userPrompt, imageBuffers: imageBuffers || [], timeoutMs, maxTokens }); }
  finally { aiBusy.end(); }
}
async function callLmStudio(model, systemPrompt, userPrompt, imageBuffers, timeoutMs = 1200000, maxTokens = 16384) {
  aiBusy.begin();
  try { return await _chatCompletion({ provider: 'lmstudio', model, systemPrompt, userPrompt, imageBuffers: imageBuffers || [], timeoutMs, maxTokens }); }
  finally { aiBusy.end(); }
}

// OpenAI-compatible providers (single call shape) vs Anthropic SDK.
const _isOA     = gen => gen.source === 'openrouter' || gen.source === 'openai' || gen.source === 'lmstudio';
const _oaCall   = gen => (gen.source === 'openai' ? callOpenAI : gen.source === 'lmstudio' ? callLmStudio : callOpenRouter);
// OpenRouter gets curated free-model fallbacks; OpenAI/LM Studio use just the chosen model.
const _oaModels = gen => (gen.source === 'openai' || gen.source === 'lmstudio'
  ? [gen.model]
  : [gen.model, ...OR_FALLBACK_MODELS.filter(m => m !== gen.model)]);
...
async function genTextWithRetry(gen, { system, user, maxTokens = 900, fallbackOR = true, model = null, isValid = null }) {
  ...
  // LM Studio — локальная модель, токены бесплатны; reasoning-моделям
  // (DeepSeek-R1 и т.п.) нужен большой запас на цепочку рассуждений + ответ.
  // Кап 16384, а не 32768: при локальных 10-15 t/s вытащить 32k токенов за
  // разумный таймаут нереально, а реальные сценарии занимают 6-12k.
  const effMaxTokens = gen.source === 'lmstudio' ? Math.max(maxTokens, 16384) : maxTokens;
  ...
  if (_isOA(gen)) {
    ...
    for (const m of models) {
      try {
        const oaTimeout = gen.source === 'lmstudio' ? 1200000 : 75000;
        const text = await _oaCall(gen)(m, system, user, [], oaTimeout, effMaxTokens);
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      let m;
      try {
        m = await gen.client.messages.create({
...
function runClaude(prompt, { budget = 2, timeoutMs = 240000, allow = 'Read,Edit,Write,Grep,Glob', model = '' } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let out = '', err = '';
    const done = () => aiBusy.end();
    const timer = setTimeout(() => { ps.kill(); done(); reject(new Error('Claude: превышен таймаут')); }, timeoutMs);
    ps.stdout.on('data', d => out += d.toString('utf8'));
    ps.stderr.on('data', d => err += d.toString('utf8'));
    ps.on('error', e => { clearTimeout(timer); done(); reject(e); });
    ps.on('close', code => {
      clearTimeout(timer);
      done();
...
//
// Stress-test 2026-09-24: body-parser кидал 4xx-ошибки в общий error-net как
// «unhandled» со стеком и отдавал клиенту «Внутренняя ошибка сервера»
// (битый JSON → 400 SyntaxError, payload > лимита → 413 PayloadTooLargeError).
// Теперь клиентских ошибок достаточно: без стека в логе одной строкой
// ([client-error]), 5xx — как раньше, со стеком и [unhandled]; тело ответа
// для 400/413 — понятное сообщение об ошибке.
app.use((err, req, res, next) => {
  const status = err?.status && err.status >= 400 && err.status < 600 ? err.status : 500;
  if (status >= 500) {
    console.error(`${C.red}[unhandled]${C.reset} ${req.method} ${req.url}:`, err?.stack || err);
  } else {
    console.error(`${C.yellow}[client-error]${C.reset} ${req.method} ${req.url} → ${status}: ${err?.message || err}`);
  }
  if (res.headersSent) return next(err);

  let message;
  if (status === 413) message = 'Тело запроса слишком большое.';
  else if (status === 400 && err?.type === 'entity.parse.failed') message = 'Некорректный JSON в теле запроса.';
  else if (status >= 400 && status < 500) message = 'Некорректный запрос.';
  else message = 'Внутренняя ошибка сервера. Подробности — в логе сервера.';
