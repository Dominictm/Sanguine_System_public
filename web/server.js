...
const ACTION_MAP = {
  ...
  'POST /api/characters/:slug/upload-belonging': req => `🧳 Добавление имущества → ${decodeURIComponent(req.params.slug)}`,
  'PUT /api/characters/:slug/belonging/:filename': req => `🧳 Правка подписи имущества: ${decodeURIComponent(req.params.filename)} ← ${decodeURIComponent(req.params.slug)}`,
  'DELETE /api/characters/:slug/belonging/:filename': req => `🧳 Удаление имущества: ${decodeURIComponent(req.params.filename)} ← ${decodeURIComponent(req.params.slug)}`,
...
async function callLmStudio(model, systemPrompt, userPrompt, imageBuffers, timeoutMs = 180000, maxTokens = 1500) {
  return _chatCompletion({ provider: 'lmstudio', model, systemPrompt, userPrompt, imageBuffers: imageBuffers || [], timeoutMs, maxTokens });
}
...
async function genTextWithRetry(gen, { system, user, maxTokens = 900, fallbackOR = true, model = null, isValid = null }) {
  ...
  if (_isOA(gen)) {
    ...
    for (const m of models) {
      try {
        const oaTimeout = gen.source === 'lmstudio' ? 180000 : 75000;
        const text = await _oaCall(gen)(m, system, user, [], oaTimeout, maxTokens);
