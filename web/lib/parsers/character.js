 ...
 *   relationships: {target: string, description: string, relType: string, mutual: boolean}[], diaries: {title: string, file: string}[],
...
function parseCharacter(rawContent, folderName, lineage) {
  ...
  if (relBlock) {
    ...
    for (const line of lines) {
      ...
      // Легаси-записи (без префикса) остаются с relType='' — вид связи для них определяет
      // резолвер по документу города (web/lib/relation-types.js, §3.2 техспеки 2026-08-26).
      // Сам слаг здесь НЕ считается: парсер карточки чист и города не знает, а словарь
      // видов теперь у каждого города свой.
      const typeMatch = rest.match(/^\[([^\]]+)\]\s*(.*)$/);
      const relType = typeMatch ? typeMatch[1].trim() : '';
      const desc    = typeMatch ? typeMatch[2].trim() : rest;
      for (const tgt of targets) {
        c.relationships.push({ target: tgt, description: desc, relType, mutual });
      }
    }
  }

  // Belongings section (## 🧳 Имущество): - [Подпись](belongings/<файл>) — Описание
  // Файлы — товарные записи вкладки «Имущество», подпись и порядок — из секции.
  // Описание — опциональный хвост после ` — ` (как у Отношений), 0..1000 симв.
  // Секцию заканчивает либо следующий `## `-заголовок, либо конец файла.
  // Регекс БЕЗ флага /m: с ним `$` матчится в конце КАЖДОЙ строки, и ленивый
  // захват `[\s\S]*?` останавливался сразу после первого буллета — терялись
  // все записи, кроме первой. Без `^` обходимся: строка «## 🧳 Имущество»
  // уникальна в карточке.
  c.belongings = [];
  const belSection = content.match(/##\s*🧳\s*Имущество\s*\n([\s\S]*?)(?=\n##\s|\n?$)/);
  if (belSection) {
    for (const line of belSection[1].split('\n')) {
      const m = line.match(/^\s*-\s+\[([^\]]*)\]\(belongings\/([^)]+)\)(?:\s*—\s*(.*))?/);
      if (m) c.belongings.push({ caption: m[1].trim(), file: m[2].trim(), description: (m[3] || '').trim() });
    }
  }
...
module.exports = { parseDiary, parseCharacter };
