...
// ── Terminal colors ────────────────────────────────────────────────────────────
// Эмитим ANSI-коды только когда терминал реально поддерживает цвета:
// - NO_COLOR — отключает (стандарт no-color.org)
// - FORCE_COLOR — включает принудительно (обход для wrapper, CI)
// - hasColors() — нативная проверка Node.js (на Windows проверяет VT через
//   GetConsoleMode; на Linux/macOS — TERM и пр.). Если нет — fallback на isTTY.
// Если консоль не поддерживает VT, escape-коды не пишутся ни в терминал, ни
// в лог-файл (убирает мусор вида «␛[2m» при работе через PowerShell/cmd).
const _colorSupported = (() => {
  if (process.env.NO_COLOR !== undefined) return false;
  if (process.env.FORCE_COLOR === '1' || process.env.FORCE_COLOR === 'true') return true;
  if (typeof process.stdout.hasColors === 'function' && process.stdout.hasColors()) return true;
  return !!process.stdout.isTTY;
})();
function _c(code) { return _colorSupported ? `\x1b[${code}m` : ''; }
const C = {
  reset:  _c('0'),
  dim:    _c('2'),
  bold:   _c('1'),
