...
let _cacheLight = {};        // city → { chars, ts } — lightweight (для графа)
let _locCacheLight = {};     // city → { locs, ts } — lightweight (для грида)
const CHARS_TTL = 15_000;
const LOCS_TTL  = 15_000;
const CHARS_LIGHT_TTL = 30_000; // графу нужна меньшая актуальность
const LOCS_LIGHT_TTL  = 30_000; // гриду нужна меньшая актуальность

function invalidateChars(city) {
  if (city) { delete _cache[city]; delete _cacheLight[city]; }
  else { _cache = {}; _cacheLight = {}; }
}
function invalidateLocs(city)  {
  if (city) { delete _locCache[city]; delete _locCacheLight[city]; }
  else { _locCache = {}; _locCacheLight = {}; }
}
...
async function getAllCharacters(city = DEFAULT_CITY) {
  ...
  const loadOne = async (folder, lineage, entry) => {
    ...
    try {
      ...
      const allImages = artFiles.filter(f => imgExts.test(f)).sort()
        .map(f => `/city-img/${city}/characters/${folder}/${encodeURIComponent(entry)}/art/${encodeURIComponent(f)}`);
      char.images = allImages;
      const slugRe  = new RegExp(`^${entry}_\\d+\\.[a-z]+$`, 'i');
      const imgFile = artFiles.filter(f => slugRe.test(f)).sort().at(-1)
        || artFiles.find(f => imgExts.test(f));
      if (imgFile) {
...
// Облегчённый вариант для графа: без проверки sheet, без listing art/,
// без хранения raw-содержимого. Экономит ~40% I/O на каждом персонаже.
async function getGraphCharacters(city = DEFAULT_CITY) {
  const cc = _cacheLight[city];
  if (cc && Date.now() - cc.ts < CHARS_LIGHT_TTL) return cc.chars;
  ...
  const result = perLineage.flat();
  _cacheLight[city] = { chars: result, ts: Date.now() };
  return result;
}
...
// Лёгкий вариант getAllLocations для грида: без raw-содержимого (тяжёлая
// markdown-карточка), но со сканированием art/ — без imageUrl грид и модалка
// показывают плейсхолдер 📍 вместо арта (регресс после введения lightweight,
// восстановлено 2026-09-23). Полный список картинок карусели всё равно
// догружает /api/locations/:slug/images при открытии модалки.
async function getLightweightLocations(city = DEFAULT_CITY) {
  const lc = _locCacheLight[city];
  if (lc && Date.now() - lc.ts < LOCS_LIGHT_TTL) return lc.locs;
  ...
  await walk(locRoot);
  _locCacheLight[city] = { locs: result, ts: Date.now() };
  return result;
}
...
async function listModules(city = DEFAULT_CITY) {
  ...
  for (const ch of chrs) {
    if (!ch.isDirectory() || ch.name.startsWith('_')) continue;   // _deleted/ — корзина
...
module.exports = {
  ...
  LINEAGE_MAP,
  getAllCharacters, getGraphCharacters, getAllLocations, getLightweightLocations, findLocMdPath, listModules, getChronicleDisplay,
  readOpenThreadsRaw,
