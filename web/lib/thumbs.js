...
// машина уходит в своп раньше, чем отдаст первую карточку. На холодном старте
// (нет ни одной готовой миниатюры) поднимаем параллель до 8 — укладывает
// прогон Парижа в ~2 сек вместо ~4. После заполнения кэша снижаем до 4.
const PARALLEL_COLD = 8;
const PARALLEL_WARM = 4;
const WARMUP_TIMEOUT_MS = 60_000;
let _parallelLimit = PARALLEL_COLD;
...
function _acquire() {
  if (_running < _parallelLimit) { _running++; return Promise.resolve(); }
  return new Promise(resolve => _queue.push(resolve));
}
function _release() {
  // После первого освобождения запускаем таймер снижения параллели.
  if (!_warmupTimer && _parallelLimit > PARALLEL_WARM) {
    _warmupTimer = setTimeout(() => { _parallelLimit = PARALLEL_WARM; _warmupTimer = null; }, WARMUP_TIMEOUT_MS);
  }
