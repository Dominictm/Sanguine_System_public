...
> Копия корневого [`DESIGN.md`](../DESIGN.md). Канонический файл — корневой;
> правки вносить туда и синхронизировать сюда, иначе две копии разъедутся
> (уже случалось: расхождение в правиле про брейкпоинты, вычищено 2026-08-27).
...
### Navigation groups
- Пункты собраны в разворачивающиеся группы. Заголовок группы — Cinzel uppercase на `--fs-3xs` с **явным `line-height: 1.4`**: без него строчный бокс схлопывается ниже кегля и подписи обрезаются (реальный баг, чинился 2026-08-26).
- Состояние каждой группы живёт в `localStorage`; переход в раздел программно разворачивает его группу, чтобы активный пункт нельзя было «потерять» в свёрнутой.
- В свёрнутой панели группа представлена значком, а её пункты открываются флайаутом сбоку — закрывается по выбору, клику мимо и `Esc`.

### Segmented control (выбор провайдера)
- Радиокнопка остаётся в разметке (на ней держатся `name`/`checked`), но растянута на всю плашку через `position: absolute; inset: 0; opacity: 0` — цель клика вместо 19px по высоте становится размером плашки.
- Выбор кодируется **не только цветом**: фон, рамка и `font-weight: 600`.
- Фокус-кольцо переносится на плашку через `:has(input:focus-visible)` — у скрытого инпута родное кольцо не видно.
