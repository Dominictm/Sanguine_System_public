...
module.exports = function charactersRouter({
  ...
  router.post('/api/characters/:slug/upload-belonging', express.json({ limit: '20mb' }), async (req, res) => {
    try {
      const { base64, ext = 'jpg', caption: rawCaption } = req.body;
      const slug = decodeURIComponent(req.params.slug);
      ...
      // Последовательный номер: slug_01, slug_02, … (как у артов, коллизии исключены).
      ...
      if (card) {
        ...
        card = appendBelongingsLine(card, `- [${caption}](belongings/${filename})`);
        await writeFileAtomic(cardPath, card, 'utf-8');
      }
  ...
  // PUT: правка подписи (только текст линк-лейбла; файл не переименовывается).
  router.put('/api/characters/:slug/belonging/:filename', express.json(), async (req, res) => {
    try {
      ...
      card = card.replace(/\r\n/g, '\n');
      const targetRe = new RegExp(`- \\[[^\\]]*\\]\\(belongings/${escapeRegExp(filename)}\\)`, 'm');
      if (!targetRe.test(card)) return res.status(404).json({ error: 'Запись имущества не найдена' });
      card = card.replace(targetRe, `- [${caption}](belongings/${filename})`);
      await writeFileAtomic(cardPath, card, 'utf-8');
      invalidateChars(city);

      res.json({ success: true, filename, caption });
    } catch (e) {
      serverError(res, e);
    }
  });
