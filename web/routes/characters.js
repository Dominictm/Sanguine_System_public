...
const { binDir: trashBinDir, appendRegistry, KIND: TRASH_KIND } = require('../lib/trash');
const {
...
const { readRelationTypesDoc, mirrorTypeName, ensureAuthoredTypes } = require('../lib/relation-types');
...
// ── Имущество (вкладка «Имущество») ─────────────────────────────────────────────
// Записи имущества — изображения с подписью. Файлы лежат в <slug>/belongings/,
// подписи и порядок — единым источником истины в секции карточки ниже. Заголовок
// держим одной строковой константой (роутер и парсер должны совпадать).
const BELONGINGS_SECTION = '## 🧳 Имущество';

function escapeRegExp(s) {
  return String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// Валидатор подписи: trim, 1–200, без `[`/`]` и переводов строк (иначе ломается
// линк-лейбл теряющего MD-буллета `- [подпись](belongings/<файл>)`).
function validateBelongingCaption(raw) {
  const caption = String(raw === null || raw === undefined ? '' : raw).trim();
  if (!caption) return { error: 'Подпись обязательна' };
  if (caption.length > 200) return { error: 'Подпись не может быть длиннее 200 символов' };
  if (/[[\]\r\n]/.test(caption)) return { error: 'Подпись не может содержать символы [ ] и перевод строки' };
  return { caption };
}

function validateBelongingDescription(raw) {
  const description = String(raw === null || raw === undefined ? '' : raw).trim().replace(/\s+/g, ' ');
  if (!description) return { description: '' };
  if (description.length > 1000) return { error: 'Описание не может быть длиннее 1000 символов' };
  if (/[\r\n]/.test(raw || '')) {
    // уже схлопнуто выше, но если исходник содержал переводы — они стали пробелами, не ошибка
  }
  // | → ∣ как в остальных полях карточки (sanitizeInlineText), сохраняем в валидаторе мягко
  return { description: description.replace(/\|/g, '∣') };
}

function formatBelongingsBullet(caption, filename, description) {
  const base = `- [${caption}](belongings/${filename})`;
  return description ? `${base} — ${description}` : base;
}

// Добавляет буллет `newLine` в секцию имущества; если секции нет — создаёт её в
// конце карточки (хвостовые переводы строк нормализуем, как в upload-image).
function appendBelongingsLine(card, newLine) {
  if (card.includes(BELONGINGS_SECTION)) {
    return card.replace(new RegExp(`(${escapeRegExp(BELONGINGS_SECTION)}\\n)([\\s\\S]*?)(\\n##|\\s*$)`), (_, hdr, body, tail) => {
...
module.exports = function charactersRouter({
  ...
  // ── Export selective (ZIP с артами/вещами/листами/дневниками) ─────────────────
  // POST /api/characters/export — зеркало файловой структуры characters/<lineage>/<slug>/
  router.post('/api/characters/export', express.json({ limit: '1mb' }), async (req, res) => {
    try {
      const city = reqCity(req);
      ...
      if (slugs.length > 100) return res.status(400).json({ error: 'Слишком много персонажей (максимум 100)' });
      // Validate slugs format
      for (const s of slugs) {
        const slug = String(s || '').trim();
        if (!slug || !/^[a-z0-9_]+$/.test(slug)) return res.status(400).json({ error: `Недопустимый слаг: ${slug}` });
      }
      ...
      for (const slugRaw of slugs) {
        ...
        if (!char) return res.status(404).json({ error: `Персонаж «${slug}» не найден` });
        const folder = char.lineageFolder;
        const charDirPath = path.join(charsDir(city), folder, slug);

        // Card
        try {
          const cardRaw = await fs.readFile(path.join(charDirPath, `${slug}.md`), 'utf-8');
          const cardBuf = Buffer.from(cardRaw, 'utf-8');
          files.push({ name: `${folder}/${slug}/${slug}.md`, data: cardBuf });
          totalBytes += cardBuf.length;
        } catch { return res.status(500).json({ error: `Не удалось прочитать карточку «${slug}»` }); }

        // Sheet md/json
        for (const suffix of [`${slug}-sheet.md`, `${slug}-sheet.json`]) {
          const p = path.join(charDirPath, suffix);
          const buf = await fs.readFile(p).catch(() => null);
          if (buf) {
            files.push({ name: `${folder}/${slug}/${suffix}`, data: buf });
            totalBytes += buf.length;
          }
        }

        // Art
        const artDir = path.join(charDirPath, 'art');
        ...
        for (const f of artFiles) {
          ...
          // Zip Slip guard for reading — ensure inside charDir
          if (!path.resolve(fp).startsWith(path.resolve(charDirPath) + path.sep)) continue;
          const buf = await fs.readFile(fp).catch(() => null);
          if (buf) {
            files.push({ name: `${folder}/${slug}/art/${f}`, data: buf });
            totalBytes += buf.length;
          }
        }

        // Belongings
        const belDir = path.join(charDirPath, 'belongings');
        const belFiles = await fs.readdir(belDir).catch(() => []);
        for (const f of belFiles) {
          if (f === '.gitkeep') continue;
          if (f.startsWith('.')) continue;
          const fp = path.join(belDir, f);
          if (!path.resolve(fp).startsWith(path.resolve(charDirPath) + path.sep)) continue;
          const buf = await fs.readFile(fp).catch(() => null);
          if (buf) {
            files.push({ name: `${folder}/${slug}/belongings/${f}`, data: buf });
            totalBytes += buf.length;
          }
        }

        // Journal
        const jDir = path.join(charDirPath, 'journal');
        ...
        for (const f of jFiles) {
          ...
          const buf = await fs.readFile(fp).catch(() => null);
          if (buf) {
            files.push({ name: `${folder}/${slug}/journal/${f}`, data: buf });
            totalBytes += buf.length;
          }
        }
      }

      if (!files.length) return res.status(400).json({ error: 'Нет данных для экспорта' });
      if (files.length > MAX_FILES) return res.status(413).json({ error: `Слишком много файлов в архиве (${files.length} > ${MAX_FILES})` });
      if (totalBytes > MAX_TOTAL) return res.status(413).json({ error: `Архив слишком большой (${Math.round(totalBytes/1024/1024)} МБ > 300 МБ)` });

      const zipBuf = createZip(files);
      const dateStamp = new Date().toISOString().slice(0, 10);
      const zipName = slugs.length === 1
        ? `character_${slugs[0]}_${city}_${dateStamp}.zip`
        : `characters_${city}_${dateStamp}.zip`;
  ...
  // ── Import ZIP (селективный с артами/вещами/листами/дневниками) ───────────────
  // Отдельный роут от JSON-ветки выше — бинарный ZIP-зеркало characters/<lineage>/<slug>/
  router.post('/api/import/characters/zip', express.raw({ type: '*/*', limit: '200mb' }), async (req, res) => {
    try {
      const city = reqCity(req);
      const rawOnConflict = String(req.query.onConflict || '').trim().toLowerCase();
      const rawOverwrite = String(req.query.overwrite || '').trim().toLowerCase();
      let mode = 'skip';
      if (['skip', 'overwrite', 'rename'].includes(rawOnConflict)) mode = rawOnConflict;
      else if (rawOverwrite === '1' || rawOverwrite === 'true') mode = 'overwrite';

      const buf = req.body;
      if (!buf || !Buffer.isBuffer(buf) || !buf.length) return res.status(400).json({ error: 'Пустой ZIP-архив' });
      if (buf.length > 200 * 1024 * 1024) return res.status(413).json({ error: 'ZIP слишком большой (>200 МБ)' });

      let entries;
      try { entries = readZip(buf); }
      catch (e) { return res.status(400).json({ error: 'Не удалось разобрать ZIP: ' + e.message }); }

      if (!entries.length) return res.status(400).json({ error: 'ZIP пуст' });
      if (entries.length > 500) return res.status(413).json({ error: `Слишком много файлов в архиве (${entries.length} > 500)` });
      let totalUncompressed = 0;
      for (const ent of entries) totalUncompressed += ent.data.length;
      if (totalUncompressed > 300 * 1024 * 1024) return res.status(413).json({ error: `Распакованный размер слишком большой (${Math.round(totalUncompressed/1024/1024)} МБ > 300 МБ)` });

      // Группировка по персонажу: lineage/slug → files[]
      const groups = new Map(); // key `${folder}/${slug}` → {folder, slug, files: []}
      const errors = [];
      for (const ent of entries) {
        const name = ent.name;
        // Zip Slip guard + базовая валидация пути
        if (!name || name.includes('..') || name.startsWith('/') || name.startsWith('\\') || path.isAbsolute(name)) {
          return res.status(400).json({ error: `Недопустимый путь в архиве: ${name}` });
        }
        const parts = name.split('/');
        if (parts.length < 3) {
          return res.status(400).json({ error: `Недопустимый путь в архиве: ${name} (ожидается <lineage>/<slug>/...)` });
        }
        // Не позволяем вложенности вида characters/... — только lineage/slug/...
        const folder = parts[0];
        const slug = parts[1];
        if (!LINEAGE_MAP[folder]) return res.status(400).json({ error: `Неизвестная линейка «${folder}» в пути ${name}` });
        if (!/^[a-z0-9_]+$/.test(slug)) return res.status(400).json({ error: `Недопустимый слаг «${slug}» в пути ${name}` });
        // Имя файла не должно содержать запрещённых символов
        const rel = parts.slice(2).join('/');
        if (!rel || rel.includes('..')) return res.status(400).json({ error: `Недопустимый путь в архиве: ${name}` });
        // Разрешаем только известные подпути
        const allowedTop = rel.startsWith('art/') || rel.startsWith('belongings/') || rel.startsWith('journal/') || /^[^/]+\.md$/.test(rel) || /^[^/]+\.json$/.test(rel);
        if (!allowedTop && rel !== `${slug}.md`) {
          // Разрешаем также -sheet файлы внутри journal — уже покрыто
          // Для строгости пропускаем только заведомо подозрительные: скрытые файлы
          if (rel.split('/').some(p => p.startsWith('.'))) continue;
        }
        const key = `${folder}/${slug}`;
        if (!groups.has(key)) groups.set(key, { folder, slug, files: [] });
        groups.get(key).files.push({ rel, data: ent.data, fullName: name });
      }

      // Валидация каждой группы: должна содержать карточку <slug>.md
      for (const [key, g] of groups) {
        const hasCard = g.files.some(f => f.rel === `${g.slug}.md`);
        if (!hasCard) {
          errors.push({ slug: g.slug, error: `В архиве нет карточки ${g.slug}.md` });
          groups.delete(key);
        }
      }
      if (!groups.size && errors.length) return res.json({ ok: true, created: [], skipped: [], renamed: [], errors });
      if (!groups.size) return res.status(400).json({ error: 'В архиве нет персонажей' });

      const created = [], skipped = [], renamed = [];
      // Для проверки коллизий — существующие персонажи
      const existingChars = await getAllCharacters(city);
      const existingSet = new Set(existingChars.map(c => `${c.lineageFolder}/${c.slug}`));

      for (const [key, g] of groups) {
        const origFolder = g.folder;
        const origSlug = g.slug;
        let finalFolder = origFolder;
        let finalSlug = origSlug;
        const mdPath = path.join(charsDir(city), finalFolder, finalSlug, `${finalSlug}.md`);
        const exists = await fs.stat(mdPath).catch(() => null);

        if (exists) {
          if (mode === 'skip') { skipped.push(origSlug); continue; }
          if (mode === 'rename') {
            // Подбор свободного слага с суффиксом _1, _2...
            let counter = 1;
            let newSlug = `${origSlug}_${counter}`;
            while (counter < 100) {
              const testPath = path.join(charsDir(city), finalFolder, newSlug, `${newSlug}.md`);
              const testExists = await fs.stat(testPath).catch(() => null);
              if (!testExists && !groups.has(`${finalFolder}/${newSlug}`) && !existingSet.has(`${finalFolder}/${newSlug}`)) break;
              counter++; newSlug = `${origSlug}_${counter}`;
            }
            if (counter >= 100) { errors.push({ slug: origSlug, error: 'Не удалось подобрать свободный слаг для переименования' }); continue; }
            finalSlug = newSlug;
          }
          // overwrite — пишем поверх, без переименования
        }

        const isRename = finalSlug !== origSlug;
        const destBase = path.join(charsDir(city), finalFolder, finalSlug);

        try {
          // Создаём директории
          await fs.mkdir(path.join(destBase, 'art'), { recursive: true });
          await fs.mkdir(path.join(destBase, 'belongings'), { recursive: true });
          await fs.mkdir(path.join(destBase, 'journal'), { recursive: true });

          // Запись файлов группы
          let cardRawForIndex = null;
          for (const f of g.files) {
            let rel = f.rel;
            let data = f.data;
            let destRel = rel;
            // При rename: переименование путей и содержимого карточки
            if (isRename) {
              if (rel === `${origSlug}.md`) destRel = `${finalSlug}.md`;
              else if (rel === `${origSlug}-sheet.md`) destRel = `${finalSlug}-sheet.md`;
              else if (rel === `${origSlug}-sheet.json`) destRel = `${finalSlug}-sheet.json`;
              // art/belongings/journal — имя файла оставляем как в архиве (совместимо),
              // но можно было бы переименовать префикс oldSlug_ → newSlug_ — оставляем исходное для простоты
            }
            const destPath = path.join(destBase, destRel);
            // Zip Slip guard на dest
            if (!path.resolve(destPath).startsWith(path.resolve(destBase) + path.sep) && path.resolve(destPath) !== path.resolve(destBase)) {
              throw new Error(`Недопустимый путь записи: ${destRel}`);
            }
            await fs.mkdir(path.dirname(destPath), { recursive: true });

            // Обновить содержимое карточки при rename: поле Слаг + возможно header
            if (rel === `${origSlug}.md` && isRename) {
              let text = data.toString('utf-8');
              // Замена - **Слаг:** old → new
              text = text.replace(/^(- \*\*Слаг:\*\*\s*).+$/m, `$1${finalSlug}`);
              data = Buffer.from(text, 'utf-8');
              cardRawForIndex = text;
            } else if (rel === `${origSlug}.md`) {
              cardRawForIndex = data.toString('utf-8');
            } else if (rel === `${origSlug}-sheet.json` && isRename) {
              // В sheet.json header.sire/clan могут ссылаться, но слаг там не хранится напрямую — оставляем
            }
            await writeFileAtomic(destPath, data);
          }

          // Индекс characters_index.md
          const idxPath = path.join(archiveDir(city), 'characters_index.md');
          const idxRaw = await fs.readFile(idxPath, 'utf-8').catch(() => null);
          if (idxRaw !== null) {
            const relLink = `../characters/${finalFolder}/${finalSlug}/${finalSlug}.md`;
            if (!idxRaw.includes(relLink)) {
              const rawForParse = cardRawForIndex || await fs.readFile(path.join(destBase, `${finalSlug}.md`), 'utf-8').catch(() => '');
              const parsed = parseCharacter(rawForParse, finalSlug, LINEAGE_MAP[finalFolder]);
              const bom = idxRaw.charCodeAt(0) === 0xFEFF;
              const body = (bom ? idxRaw.slice(1) : idxRaw).replace(/\s*$/, '') +
                `\n- [${parsed.name}](${relLink}) — ${parsed.lineageLabel || finalFolder}${parsed.clan ? `, ${parsed.clan}` : ''}\n`;
              await writeFileAtomic(idxPath, (bom ? '﻿' : '') + body, 'utf-8');
            }
          }

          // Инвалидация thumbs кэша для этого персонажа (если был overwrite/rename)
          const thumbsBase = path.join(cityDir(city), '.thumbs', 'characters', finalFolder, finalSlug);
          await fs.rm(thumbsBase, { recursive: true, force: true }).catch(() => {});
          if (isRename) {
            // При rename старый thumbs не трогаем — он относится к оригиналу
          } else if (exists) {
            // При overwrite старого — чистим и старый путь (тот же)
            const oldThumbs = path.join(cityDir(city), '.thumbs', 'characters', finalFolder, origSlug);
            if (origSlug !== finalSlug) await fs.rm(oldThumbs, { recursive: true, force: true }).catch(() => {});
          }

          if (isRename) renamed.push({ from: origSlug, to: finalSlug });
          else created.push(finalSlug);
          existingSet.add(`${finalFolder}/${finalSlug}`);
        } catch (e) {
          errors.push({ slug: origSlug, error: e.message });
        }
      }

      invalidateChars(city);
      console.log(`[import-characters-zip] ${city}: created=${created.length} skipped=${skipped.length} renamed=${renamed.length} errors=${errors.length} mode=${mode}`);
      res.json({ ok: true, created, skipped, renamed, errors });
    } catch (e) { serverError(res, e); }
  });
  ...
  // Авто-парные виды связи (2026-08-08, Фаза 3, п.5-7 запроса) — при взаимной синхронизации
  // зеркальная сторона получает НЕ тот же вид, а парный (Сир↔Чайлд, Домитор↔Гуль); Брат/Сестра —
  // в зависимости от пола ЦЕЛИ (кому пишем зеркальную строку). Остальные виды зеркалятся тем же
  // именем — симметричны по умолчанию. С 2026-08-26 пары не хардкод, а колонка «Пара» документа
  // города (cities/<город>/rules/relation_types.md): прежние REL_AUTO_PAIRS здесь и
  // REL_AUTO_PAIR_TYPES в char-detail.js были двумя копиями одного списка «держать в синхроне».
  // Побочный эффект, которого раньше не было: пару может получить и авторский вид.
  ...
  async function _syncMutualRelations(city, chars, aName, oldRels, newRels) {
    ...
    const relDoc = await readRelationTypesDoc(city);
    ...
    for (const [targetName, rel] of newMutual) {
      ...
      try {
        ...
        const tChar = fresh.find(c => c.slug === target.slug);
        if (!tChar) continue;
        const mirrorType = mirrorTypeName(relDoc, rel.relType, tChar.gender);
  ...
  router.put('/api/characters/:slug/relations', express.json(), async (req, res) => {
    try {
      ...
      // Новый вид связи, которого нет ни в одном разделе документа города, дописывается
      // в «Авторские» (§4.4 техспеки 2026-08-26). Это замена прежней агрегации на лету:
      // раздел ведёт себя как журнал того, что Рассказчик действительно использует, и
      // переживает перезагрузку. Идёт ДО зеркальной синхронизации — чтобы вид уже
      // существовал, когда та полезет за его парой.
      let addedTypes = [];
      try {
        addedTypes = await ensureAuthoredTypes(city, (freshChar?.relationships || []).map(r => r.relType));
      } catch (e) {
        console.error('[relation-types]', e.message);
      }

      const warnings = await _syncMutualRelations(city, freshChars, char.name, oldRels, freshChar?.relationships || []);

      res.json({ ok: true, warnings, ...(addedTypes.length ? { addedRelationTypes: addedTypes } : {}) });
    } catch (e) {
      serverError(res, e);
    }
  });
  ...
  router.delete('/api/characters/:slug', express.json(), async (req, res) => {
    try {
      ...
      // Запись происхождения в реестр корзины (для вкладки «Удалённые»).
      const trashDir = trashBinDir(TRASH_KIND.CHARACTER, city);
      await appendRegistry(trashDir, {
        storedName: path.basename(dst), kind: TRASH_KIND.CHARACTER, slug, city,
        deletedAt: new Date().toISOString(), lineage: lineageFolder, display: char.name || slug,
      });
  ...
  // Upload: изображение + подпись → файл в belongings/ + буллет в секции карточки.
  // Зеркало upload-image (тело {base64, ext}, валидация magic bytes, последовательные имена).
  router.post('/api/characters/:slug/upload-belonging', express.json({ limit: '20mb' }), async (req, res) => {
    try {
      const { base64, ext = 'jpg', caption: rawCaption, description: rawDescription } = req.body;
      const slug = decodeURIComponent(req.params.slug);
      ...
      const capCheck = validateBelongingCaption(rawCaption);
      if (capCheck.error) return res.status(400).json({ error: capCheck.error });
      const caption = capCheck.caption;
      const descCheck = validateBelongingDescription(rawDescription);
      ...
      const belDir = path.join(charsDir(city), char.lineageFolder, char.slug, 'belongings');
      await fs.mkdir(belDir, { recursive: true });

      // Имя файла: <slug>_<название>_<счётчик> — название из подписи вещи, счётчик общий по персонажу.
      const rawCaptionSlug = slugify(caption) || 'item';
      const safeCaptionSlug = rawCaptionSlug.slice(0, 40).replace(/_+$/,'') || 'item';
      const existing = await fs.readdir(belDir).catch(() => []);
      // Счётчик — общий по персонажу (совместим со старым форматом slug_01.* и новым slug_title_01.*).
      const nums = existing
        .filter(f => f.startsWith(char.slug + '_'))
        .map(f => { const m = f.match(/_(\d+)\.[a-z0-9]+$/i); return m ? parseInt(m[1], 10) : 0; })
        .filter(n => n > 0);
      let nextNum = nums.length ? Math.max(...nums) + 1 : 1;
      let filename = `${char.slug}_${safeCaptionSlug}_${String(nextNum).padStart(2, '0')}.${safeExt}`;
      // Защита от коллизии, если файл с таким именем уже есть (гонка или ручное копирование).
      let attempt = 0;
      while (existing.includes(filename) && attempt < 100) {
        attempt++;
        filename = `${char.slug}_${safeCaptionSlug}_${String(nextNum + attempt).padStart(2, '0')}.${safeExt}`;
      }

      await writeFileAtomic(path.join(belDir, filename), validated.buffer);

      const cardPath = path.join(charsDir(city), char.lineageFolder, char.slug, `${char.slug}.md`);
      let card = await fs.readFile(cardPath, 'utf-8').catch(() => null);
      if (card) {
        card = card.replace(/\r\n/g, '\n');  // Windows checkout (core.autocrlf) → CRLF на диске
        card = appendBelongingsLine(card, formatBelongingsBullet(caption, filename, description));
        await writeFileAtomic(cardPath, card, 'utf-8');
      }
      ...
      res.json({
        ...
        caption,
        description,
        url: `/city-img/${city}/characters/${char.lineageFolder}/${encodeURIComponent(char.slug)}/belongings/${encodeURIComponent(filename)}`
      });
    } catch (e) {
      serverError(res, e);
    }
  ...
  // PUT: правка подписи/описания (только тексты буллета; файл не переименовывается).
  router.put('/api/characters/:slug/belonging/:filename', express.json(), async (req, res) => {
    try {
      const slug     = decodeURIComponent(req.params.slug);
      const filename = decodeURIComponent(req.params.filename);
      const city     = reqCity(req);

      if (/[/\\]|^\./.test(filename)) return res.status(400).json({ error: 'Недопустимое имя файла' });

      const capCheck = validateBelongingCaption(req.body?.caption);
      if (capCheck.error) return res.status(400).json({ error: capCheck.error });
      const caption = capCheck.caption;
      ...
      let description = '';
      if (hasDesc) {
        const descCheck = validateBelongingDescription(req.body?.description);
        if (descCheck.error) return res.status(400).json({ error: descCheck.error });
      ...
      const cardPath = path.join(charsDir(city), char.lineageFolder, char.slug, `${char.slug}.md`);
      let card = await fs.readFile(cardPath, 'utf-8').catch(() => null);
      if (!card) return res.status(404).json({ error: 'Карточка персонажа не найдена' });

      card = card.replace(/\r\n/g, '\n');
      // Если description не передан — сохранить существующий хвост " — описание"
      if (!hasDesc) {
        const existing = card.match(new RegExp(`- \\[[^\\]]*\\]\\(belongings/${escapeRegExp(filename)}\\)(?:\\s*—\\s*(.*))?`, 'm'));
        description = existing && existing[1] ? existing[1].trim() : '';
      }
      const targetRe = new RegExp(`^-\\s*\\[[^\\]]*\\]\\(belongings/${escapeRegExp(filename)}\\)(?:\\s*—\\s*.*)?`, 'm');
      if (!targetRe.test(card)) return res.status(404).json({ error: 'Запись имущества не найдена' });
      card = card.replace(targetRe, formatBelongingsBullet(caption, filename, description));
      await writeFileAtomic(cardPath, card, 'utf-8');
      invalidateChars(city);

      res.json({ success: true, filename, caption, description });
    } catch (e) {
      serverError(res, e);
    }
  });

  // DELETE: удалить запись (буллет из секции + файл). Идемпотентен по арт-конвенции:
  // файл ENOENT + нет ссылки → 404, иначе чистим то, что есть.
  router.delete('/api/characters/:slug/belonging/:filename', async (req, res) => {
    try {
      const slug     = decodeURIComponent(req.params.slug);
      const filename = decodeURIComponent(req.params.filename);
      const city     = reqCity(req);
      ...
      const belDir   = path.join(charsDir(city), char.lineageFolder, char.slug, 'belongings');
      const filePath = path.resolve(belDir, filename);
      if (!filePath.startsWith(path.resolve(belDir) + path.sep)) {
        return res.status(400).json({ error: 'Недопустимый путь' });
      }
      ...
      // Убрать строку записи; если буллетов имущества не осталось — удалить и заголовок секции.
      const cardPath = path.join(charsDir(city), char.lineageFolder, char.slug, `${char.slug}.md`);
      let card = await fs.readFile(cardPath, 'utf-8').catch(() => null);
      let refRemoved = false;
      if (card) {
        ...
        const target = lines.findIndex(l => l.includes(`belongings/${filename}`));
        if (target !== -1) {
          lines.splice(target, 1);
          const remaining = lines.join('\n');
          // Нет ни одного буллета имущества — убираем и пустой заголовок секции
          // (только если он реально стал пустым: без этого инфо был навсегда).
          if (!/^- \[[^\]]*\]\(belongings\//m.test(remaining)) {
            card = remaining
              .replace(/^## 🧳 Имущество\n/m, '')
              .replace(/\n{3,}/g, '\n\n');
          } else {
  ...
  router.post('/api/characters/:slug/diary/generate', aiRateLimit, express.json(), async (req, res) => {
    try {
      ...
      let eventsText = '';
      try {
        const chrs = (await fs.readdir(chroniclesDir(city), { withFileTypes: true })).filter(e => e.isDirectory() && !e.name.startsWith('_'));
