...
router.delete('/api/cities/:slug', async (req, res) => {
  try {
    ...
    await appendRegistry(trashBinDir(TRASH_KIND.CITY), {
      storedName: path.basename(dest), kind: TRASH_KIND.CITY, slug,
      deletedAt: new Date().toISOString(), display: slug,
    });
...
router.delete('/api/cities/:slug/districts/:districtSlug', async (req, res) => {
  try {
    ...
    const districtStoredName = `district_${districtSlug}_${Date.now()}`;
    await fs.rename(districtDir, path.join(trashRoot, districtStoredName));
    // Запись происхождения в реестр корзины (для вкладки «Удалённые»): slug — сам
    // район (record.slug у DISTRICT читается как отображаемый слайз-слаг), а не город.
    await appendRegistry(trashBinDir(TRASH_KIND.DISTRICT, slug), {
      storedName: districtStoredName, kind: TRASH_KIND.DISTRICT, slug: districtSlug, city: slug, districtSlug,
      deletedAt: new Date().toISOString(), display: districtSlug,
    });
    const warning = await _syncCityDistrictsList(slug);
