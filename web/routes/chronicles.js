...
const { moveToTrash, KIND: TRASH_KIND } = require('../lib/trash');
const {
...
async function buildChronicleDeletePreview(city, slug) {
  ...
  const otherDirs  = allChrs.filter(c => c.isDirectory() && c.name !== slug && !c.name.startsWith('_'));
...
module.exports = function chroniclesRouter({
  ...
  router.delete('/api/chronicles/:slug', express.json(), async (req, res) => {
    try {
      ...
      // 3. Soft-delete: move chronicle directory to chronicles/_deleted/<slug>_<ts>
      //    + запись происхождения в _registry.json (вкладка «Удалённые», D2).
      await moveToTrash({
        src: chrDir,
        kind: TRASH_KIND.CHRONICLE,
        city,
        slug,
        extra: {},
        storedName: `${slug}_${Date.now()}`,
      });
