...
async function _checkNpcPromotion(city, chr, mod, npcSlug) {
  ...
  try {
    ...
    for (const cEntry of chrs) {
      if (!cEntry.isDirectory() || cEntry.name.startsWith('_')) continue;   // _deleted/ — корзина
...
// ── Generation mode helper ──────────────────────────────────────────────────
// Reads the current generation mode from process.env (synced by POST /api/settings).
// Returns 'b' (default) or 'c'.
function getGenerationMode() {
  const mode = (process.env.GENERATION_MODE || 'b').toLowerCase();
  return mode === 'c' ? 'c' : 'b';
}
...
module.exports = {
  ...
  diaryToneFor, getGenerationMode,
};
