...
const {
  ...
  unescapeFreeformBody, isBogusGeneration, getGenerationMode,
} = require('./shared');
...
module.exports = function fillRouter({ makeGenerationClient, genTextWithRetry }) {
  ...
  router.post('/api/chronicles/:chr/modules/:mod/fill', aiRateLimit, express.json(), async (req, res) => {
    try {
      ...
      // Read module rules + scenario template (canonical structure norm)
      const moduleRules = await fs.readFile(
        path.join(ROOT, 'system', 'rules', 'module_rules.md'), 'utf-8').catch(() => '');
      const scenarioTemplate = await fs.readFile(
        path.join(ROOT, 'system', 'rules', 'scenario_template.md'), 'utf-8').catch(() => '');
  ...
  # ЭТАЛОН СТРУКТУРЫ СЦЕНАРИЯ [[SCENARIO_TEMPLATE]]
  ${scenarioTemplate}
  ...
  Язык: русский. Стиль: готический нуар, VtM атмосфера.`;
      ...
      // ── Mode B: fact-ledger — compressed state snapshot for the prompt ──────
      const warnings = [];
      if (genMode === 'b') {
        try {
          const { compressChronicleEvents, parseEventsText } = require('../../lib/context_builder');
          const evPath = path.join(chroniclesDir(city), chr, 'events.md');
          const evRaw = await fs.readFile(evPath, 'utf-8').catch(() => '');
          if (evRaw) {
            const parsed = parseEventsText(evRaw);
            const state = compressChronicleEvents(parsed);
            const factSnapshot = [
              `Напряжение: ${state.tension}/10`,
              `Активные персонажи: ${Object.keys(state.characters).join(', ') || 'нет'}`,
              `Мировые флаги: ${Object.keys(state.world_flags).join('; ') || 'нет'}`,
            ].join('\n');
            // Prepend fact snapshot to the system prompt
            systemPrompt = `# СЛЕПОК ФАКТОВ (актуальное состояние хроники — не противоречь этим фактам)\n${factSnapshot}\n\n${systemPrompt}`;
          }
        } catch (flErr) {
          console.warn('[fill-module] fact-ledger error:', flErr.message);
        }
      }

      // Use makeGenerationClient (respects OpenRouter/Claude preference)
      const scenSource = req.body?.preferSource || null;
      const scenModel  = req.body?.orModel      || null;
      const gen = await makeGenerationClient(scenSource, scenModel).catch(() => null);
      ...
      // Нормализация уровней заголовков: модель может сдвинуть уровни на +1
      // (сцены `#` вместо `##`, поля `##` вместо `###`) — детерминировано
      // демоутим на один уровень, титул `# Сценарий` не трогаем.
      const preNormalized = scenarioText;
      scenarioText = normalizeScenarioHeadings(scenarioText);
      const normalized = scenarioText !== preNormalized;

      // Грубая проверка после нормализации: если отсутствуют обязательные
      // разделы (Пролог/Сцены/Финал) — 502, файл не пишем.
      const grossMissing = checkScenarioStructure(scenarioText).missing;
      if (grossMissing.length > 0) {
        _logAiFail(`fill/${chr}/${mod}: сценарий`, new Error('non-canonical structure: missing ' + grossMissing.map(g => g.key).join(', ')), gen);
        return res.status(502).json({ ok: false,
          error: 'Структура сценария не эталонна — отсутствуют обязательные разделы. Перегенерируй или попробуй другого AI-провайдера в Инструменты → Модели AI.' });
      }

      // Мягкая проверка каноничности (не блок): титул, top-level блоки, Колорит
      // в сценах, meta-строки, шапка — показываем Мастеру предупреждениями.
      const structureWarnings = checkScenarioCanonical(scenarioText).errors;
      ...
      if (genMode === 'b') {
        try {
          const canonRes = await fetch(`http://127.0.0.1:${process.env.PORT || 4295}/api/canon-check`, {
            ...
            body: JSON.stringify({ text: scenarioText.slice(0, 8000) }),
          ...
          console.warn('[fill-module] canon-check error:', ccErr.message);
        }
      }
      ...
      if (genMode === 'c') {
        try {
          const existingScenario = await fs.readFile(scenarioPath, 'utf-8').catch(() => null);
          if (existingScenario) {
            const prevPath = path.join(modDir, 'scenario.prev.md');
            await writeFileAtomic(prevPath, existingScenario, 'utf-8');
          }
        } catch (vErr) {
          console.warn('[fill-module] versioning error:', vErr.message);
        }
      }

      // Save as scenario.md
      const scenarioPath = path.join(modDir, 'scenario.md');
      const header = `# Сценарий — ${modTitle}\n\n> 🔗 [Модуль](${mod}.md) | [Хроника](../../chronicle.md) | [НПС](npc.md)\n\n<!-- meta:genmode: ${genMode} -->\n\n---\n\n`;
