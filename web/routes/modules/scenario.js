...
module.exports = function scenarioRouter({ makeGenerationClient, genTextWithRetry }) {
  ...
  router.post('/api/chronicles/:chr/modules/:mod/scenario/section/regenerate', aiRateLimit, express.json(), async (req, res) => {
    try {
      ...
      const scenarioTemplate = await fs.readFile(path.join(ROOT, 'system', 'rules', 'scenario_template.md'), 'utf-8').catch(() => '');
...
- Верни ТОЛЬКО новый текст раздела — без строки заголовка «## ${heading}» и без обрамляющих markdown-разделителей «---».`;
      ...
      const gen = await makeGenerationClient(req.body?.preferSource || null, req.body?.orModel || null).catch(() => null);
      ...
      if (genMode === 'c') {
        try {
          ...
          await writeFileAtomic(prevPath, raw, 'utf-8');
        } catch (vErr) {
          console.warn('[scenario-section-regen] versioning error:', vErr.message);
        }
      }

      const sectionUpdated = replaceScenarioSection(raw, heading, newBody, parent);
      const updated = normalizeScenarioHeadings(sectionUpdated);
      await writeFileAtomic(scenarioPath, updated, 'utf-8');
      invalidateChars(city);
      console.log(`[scenario-section] ${city}/${chr}/${mod} → «${heading}» перегенерирован`);
      res.json({ ok: true, scenario: updated, genMode, warnings });
    } catch (e) {
...
- Верни ТОЛЬКО новый текст блока начиная СРАЗУ с первого «### » (без строки заголовка «## ${heading}», без обрамляющих markdown-разделителей «---»). Если нужен вводный текст перед первым «### » — начни с него.`;
      ...
      if (genMode === 'c') {
        try {
          ...
          console.warn('[scenario-block-regen] versioning error:', vErr.message);
        }
      }
      ...
      updated = normalizeScenarioHeadings(updated);
      await writeFileAtomic(scenarioPath, updated, 'utf-8');
      invalidateChars(city);
