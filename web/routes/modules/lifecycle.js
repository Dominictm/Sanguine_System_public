...
module.exports = function lifecycleRouter({ makeGenerationClient, genTextWithRetry }) {
  ...
  router.delete('/api/chronicles/:chr/modules/:mod', express.json(), async (req, res) => {
    try {
      ...
      // 5. Soft-delete: move module dir to chronicles/_deleted/<chr>__<mod>_<ts>
      //    + запись происхождения в _registry.json (вкладка «Удалённые», D3).
      await moveToTrash({
        src: modDir,
        kind: TRASH_KIND.MODULE,
        city,
        slug: mod,
        extra: { chr, mod },
        storedName: `${chr}__${mod}_${Date.now()}`,
      });
