...
const {
  ...
  getAllCharacters, getGraphCharacters, listModules, countMdFiles, readOpenThreadsRaw,
  aggregateEvents, makeNameResolver, getDiaryIndex, eventMonthKey,
  getBrokenLinks, getChronicleDisplay,
} = require('../lib/db');
const { getRelationResolver } = require('../lib/relation-types');
...
router.get('/api/status', async (req, res) => {
  try {
    ...
    res.json({
      ...
      aiBusy: aiBusy.snapshot()        // {active, since} — in-flight AI generations
    });
...
router.get('/api/graph', async (req, res) => {
  try {
    const city  = reqCity(req);
    const chars = await getGraphCharacters(city);
    ...
    // Вид связи — единый резолвер над документом города (§3.2 техспеки 2026-08-26,
    // cities/<город>/rules/relation_types.md). До него тип выбирался двумя независимыми
    // путями сразу: буквальное совпадение с именем глобальной библиотеки давало слаг
    // библиотеки, всё остальное — категорию categorizeRel() по ключевым словам. Одно
    // понятие получало два ключа и рисовало на графе ДВА одинаковых чипа («Союзник» ×2).
    const { resolve: matchRelType } = await getRelationResolver(city);
    ...
    for (const c of chars) {
      for (const r of c.relationships) {
        ...
        // REL_NEUTRAL графа — не вид связи, а признак «резолвер ничего не узнал» (§3.3);
        // в документе города его нет и быть не должно.
        const resolvedType = matched ? matched.slug : 'neutral';
    ...
    const walkMd = async (dir, filterFn) => {
      ...
      const walk = async d => {
        ...
        for (const e of entries) {
          if (e.name.startsWith('_')) continue;   // _deleted/_registry — корзины и мусор
