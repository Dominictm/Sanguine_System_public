...
module.exports = function toolsRouter({
  ...
  invalidateGeminiClient, geminiAvailable, lmStudioAvailable,
  ...
  // ── Probe local AI servers (async, non-blocking) ───────────────────────────
  router.get('/api/settings/probe-local', async (req, res) => {
    try {
      const lmStudioAvailable = await (typeof lmStudioAvailable === 'function' ? lmStudioAvailable() : Promise.resolve(false));
      res.json({ ok: true, lmStudioAvailable });
    } catch (e) {
      res.json({ ok: true, lmStudioAvailable: false });
    }
  });
  ...
  router.get('/api/settings', async (req, res) => {
    try {
      ...
      res.json({
        ...
        GENERATION_MODE:       env.GENERATION_MODE    || 'b',
        claudeOauth,
      });
    } catch (e) { serverError(res, e); }
  });
  ...
  router.post('/api/settings', express.json(), async (req, res) => {
    try {
      const {
        ...
        LMSTUDIO_BASE_URL, LMSTUDIO_MODEL, GENERATION_MODE,
      } = req.body || {};
      ...
      if (LMSTUDIO_BASE_URL !== undefined) {
        if (LMSTUDIO_BASE_URL.trim()) env.LMSTUDIO_BASE_URL = LMSTUDIO_BASE_URL.trim();
        else delete env.LMSTUDIO_BASE_URL;
      }
      if (LMSTUDIO_MODEL !== undefined) {
        if (LMSTUDIO_MODEL.trim()) env.LMSTUDIO_MODEL = LMSTUDIO_MODEL.trim();
        else delete env.LMSTUDIO_MODEL;
      }
      if (GENERATION_MODE !== undefined) {
        const mode = String(GENERATION_MODE).trim().toLowerCase();
        if (mode === 'b' || mode === 'c') {
          env.GENERATION_MODE = mode;
          process.env.GENERATION_MODE = mode; // sync in-memory for immediate effect
        } else if (mode === '') {
          delete env.GENERATION_MODE;
          process.env.GENERATION_MODE = 'b'; // reset to default
        } else {
          return res.status(400).json({ ok: false, error: 'GENERATION_MODE: допустимые значения — "b" или "c".' });
        }
      }
      ...
      res.json({ ok: true, needsRestart, supervised: isSupervised(), GENERATION_MODE: env.GENERATION_MODE || 'b' });
