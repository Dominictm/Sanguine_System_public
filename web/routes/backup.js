...
const {
  ...
  invalidateChars, invalidateLocs, writeFileAtomic,
} = require('../lib/db');
...
// Приём тела запроса сырым потоком в файл, с пределом размера — общее между
// restore/inspect (города) и settings/restore: без express.json() (см. комментарии
// на обоих роутах — тело идёт мимо общего express.json() выше по цепочке), поэтому
// у каждого свой предел вместо унаследованного 20mb. Разный maxBytes на вызов
// (1 ГБ для городов, 20 МБ для настроек), сама механика приёма — одна.
function receiveUploadToFile(req, uploadPath, maxBytes) {
  let uploadedBytes = 0;
  ...
  return new Promise((resolve, reject) => {
    req.on('data', chunk => {
      ...
      if (uploadedBytes > maxBytes) {
        req.destroy();
        writeStream.destroy();
        reject(new Error(`файл слишком большой (>${Math.round(maxBytes / 1024 / 1024)} МБ)`));
      }
    });
...
// GOOGLE_APPLICATION_CREDENTIALS в .env — АБСОЛЮТНЫЙ путь к service-account ключу
// (его пишет туда routes/tools.js при сохранении настроек). В архиве настроек путь
// приезжает с той машины, где делался бэкап, а сам ключ при восстановлении ложится
// рядом с локальным .env — поэтому значение переписываем на локальное. Иначе после
// переноса на другую машину/в другую папку (главный сценарий этого архива) ADC
// указывает в никуда, Vertex падает на каждой генерации, а restore отчитался «ok».
function retargetAdcPath(envText, localKeyPath) {
  return envText.replace(/^([ \t]*GOOGLE_APPLICATION_CREDENTIALS[ \t]*=).*$/gm,
    (_m, prefix) => prefix + localKeyPath);
}

module.exports = function backupRouterFactory({ envPath, invalidateGeminiClient, scheduleRestart, isSupervised }) {
  ...
  // ── Восстановление настроек — синхронно, один POST (архив настроек — единицы
  // файлов, не сотни мегабайт городского архива; двухшаговый inspect→commit,
  // оправданный у городов размером и риском, здесь избыточен — подтверждение
  // происходит на клиенте до отправки файла). .env и vertex-key пишутся независимо
  // друг от друга (не транзакционно — это разные, не связанные артефакты), каждый
  // через writeFileAtomic. ai-feature-prefs физически не существует на сервере
  // (это localStorage браузера) — сервер только возвращает его клиенту, запись
  // делает сам клиент.
  router.post('/api/backup/settings/restore', async (req, res) => {
    let tmpDir;
    try {
      tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'vtm_settings_restore_'));
      const uploadPath = path.join(tmpDir, 'upload.zip');

      // 20 МБ — щедрый запас над реальным размером архива настроек (единицы
      // килобайт), но не 1 ГБ, как у городов: файлу из трёх текстовых/JSON
      // документов взяться большим неоткуда, разумный предел — просто барьер
      // от неограниченной записи на диск (см. тот же приём в restore/inspect
      // городов, тот же receiveUploadToFile с другим maxBytes).
      await receiveUploadToFile(req, uploadPath, 20 * 1024 * 1024);

      const extractDir = path.join(tmpDir, 'extracted');
      // Битый/не-zip файл — ошибка ВВОДА пользователя, а не сбой сервера: отвечаем
      // 400 с внятным текстом, как это делает restore/inspect у городов, а не
      // выпадаем в generic 500 через serverError.
      try {
        await runPowerShell(`Expand-Archive -Path ${psQuote(uploadPath)} -DestinationPath ${psQuote(extractDir)} -Force`);
      } catch (e) {
        return res.status(400).json({ error: 'Не удалось разобрать архив: ' + e.message });
      }

      const envSrc    = path.join(extractDir, '.env');
      const vertexSrc = path.join(extractDir, '.gemini-vertex-key.json');
      const prefsSrc  = path.join(extractDir, 'ai-feature-prefs.json');

      const hasEnv = await fs.stat(envSrc).then(() => true).catch(() => false);
      if (!hasEnv) return res.status(400).json({ error: 'В архиве нет .env — это не похоже на архив настроек' });

      // Откат прежней версии — один слот, перезаписывается при каждом восстановлении
      // (не растущая история, как cities/_deleted/: секреты не должны бесконтрольно
      // копиться на диске). web/.backups/ — директория целиком в .gitignore (не по
      // имени файла, как web/.env/web/.gemini-vertex-key.json сейчас) — иначе файл
      // отката с другим именем мог бы случайно уйти в git при широком git add.
      const backupDir = path.join(ROOT, 'web', '.backups', 'env');
      await fs.mkdir(backupDir, { recursive: true });

      const results = {};
      // Частичный сбой (.env лёг, а vertex-key/prefs — нет) не должен выглядеть как
      // полный успех: собираем warnings тем же приёмом, что уже используется на
      // других эндпоинтах (cities.js), клиент показывает их warning-тостами.
      const warnings = [];

      await fs.copyFile(envPath(), path.join(backupDir, '.env.previous')).catch(() => {});
      let envText = null;
      try {
        envText = retargetAdcPath(await fs.readFile(envSrc, 'utf-8'), vertexKeyPath());
        await writeFileAtomic(envPath(), envText, 'utf-8');
        results.env = 'ok';
      } catch (e) {
        console.error('[settings-restore] .env:', e.message);
        results.env = 'error: ' + e.message;
      }

      const hasVertex = await fs.stat(vertexSrc).then(() => true).catch(() => false);
      if (hasVertex) {
        await fs.copyFile(vertexKeyPath(), path.join(backupDir, '.gemini-vertex-key.json.previous')).catch(() => {});
        try {
          const vertexRaw = await fs.readFile(vertexSrc, 'utf-8');
          let parsed;
          try { parsed = JSON.parse(vertexRaw); }
          catch { throw new Error('не удалось разобрать JSON — файл в архиве повреждён или не является service-account ключом'); }
          if (!parsed || parsed.type !== 'service_account')
            throw new Error('ожидается ключ типа "service_account" (в архиве — не он)');
          // Канонический формат независимо от того, как файл был отформатирован в
          // архиве — то же самое уже делает ручное редактирование (tools.js), одним
          // способом записи ключа на диск меньше расхождений.
          await writeFileAtomic(vertexKeyPath(), JSON.stringify(parsed, null, 2), 'utf-8');
          results.vertexKey = 'ok';
        } catch (e) {
          console.error('[settings-restore] vertex-key:', e.message);
          results.vertexKey = 'error: ' + e.message;
          warnings.push('Ключ Vertex AI не восстановлен: ' + e.message);
        }
      }

      let aiFeaturePrefs = null;
      const prefsRaw = await fs.readFile(prefsSrc, 'utf-8').catch(() => null);
      if (prefsRaw !== null) {
        // Битый JSON не блокирует остальное, но и не проходит молча: «файла в архиве
        // не было» (старый архив/пустой localStorage — норма) и «файл был, но битый»
        // (сбой) раньше одинаково давали aiFeaturePrefs:null и не различались.
        try { JSON.parse(prefsRaw); aiFeaturePrefs = prefsRaw; }
        catch { warnings.push('Настройки моделей по функциям не восстановлены: ai-feature-prefs.json в архиве повреждён'); }
      }

      // Настройки требуют Vertex, а ключа на диске так и нет (в архиве его не было
      // или он не прошёл валидацию): .env применился, но генерация упадёт при первом
      // же обращении — предупреждаем сразу, а не оставляем «AI сломался без причины».
      if (results.env === 'ok' && /^[ \t]*GEMINI_AUTH_TYPE[ \t]*=[ \t]*vertex[ \t]*$/m.test(envText || '')) {
        const keyOnDisk = await fs.stat(vertexKeyPath()).then(() => true).catch(() => false);
        if (!keyOnDisk) warnings.push(
          'В настройках выбран Vertex AI, но файла service-account ключа нет — загрузи ключ заново в «Инструменты → Модели AI».');
      }

      // .env — единственный ОБЯЗАТЕЛЬНЫЙ артефакт архива; если именно его запись
      // не удалась, restart бессмыслен (ничего не изменилось для перезагрузки) и
      // клиент не должен получить впечатление успеха только по HTTP 200 — ok
      // отражает исход записи .env, а не просто «запрос обработан без 500».
      const envOk = results.env === 'ok';
      invalidateGeminiClient();
      res.json({
        ok: envOk, results, aiFeaturePrefs,
        ...(warnings.length ? { warnings } : {}),
        needsRestart: envOk, supervised: isSupervised(),
      });
      if (envOk) scheduleRestart('[settings-restore]');
    } catch (e) {
      serverError(res, e);
    } finally {
      if (tmpDir) await fs.rm(tmpDir, { recursive: true, force: true }).catch(() => {});
  ...
  router.post('/api/backup/restore/inspect', (req, res) => {
    ...
    (async () => {
      ...
      await receiveUploadToFile(req, uploadPath, 1024 * 1024 * 1024);
