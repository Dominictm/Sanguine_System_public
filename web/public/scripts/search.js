...
async function _runSearch() {
  ...
  if (q.length < 3) {
    resultsEl.innerHTML = `<div class="srch-hint">${t('search.hint')}</div>`;
    if (subEl) subEl.textContent = '';
    return;
  }
  resultsEl.innerHTML = '<div class="loading-state"><div class="spinner"></div>' + t('common.search') + '</div>';
  try {
    const data = await fetch(`/api/search?q=${encodeURIComponent(q)}`).then(r => r.json());
    _renderSearchResults(data, resultsEl, subEl);
  } catch (e) {
    resultsEl.innerHTML = `<div class="srch-hint">⚠ ${t('ui.err')}${escHtml(e.message)}</div>`;
  }
}

function _renderSearchResults(data, el, subEl) {
  if (!data.results || data.total === 0) {
    el.innerHTML = `<div class="srch-hint">${t('search.empty', { query: escHtml(data.query) })}</div>`;
    if (subEl) subEl.textContent = '';
    return;
  }
  if (subEl) subEl.textContent = `${data.total} ${t('search.count')}`;
  const hl = (txt) => {
    if (!txt) return '';
  ...
  if ((data.results.characters || []).length) {
    const rows = data.results.characters.map(c => `
      <div class="srch-row srch-char" data-slug="${escHtml(c.slug)}">
        ...
        <button class="srch-open-btn">${t('search.open')}</button>
      </div>`).join('');
    sections.push(`<div class="srch-section"><div class="srch-sec-title">${t('search.sec.characters')} <span class="srch-sec-count">${data.results.characters.length}</span></div>${rows}</div>`);
  }
  ...
  if ((data.results.locations || []).length) {
    ...
    sections.push(`<div class="srch-section"><div class="srch-sec-title">${t('search.sec.locations')} <span class="srch-sec-count">${data.results.locations.length}</span></div>${rows}</div>`);
  }
  ...
  if ((data.results.modules || []).length) {
    ...
    sections.push(`<div class="srch-section"><div class="srch-sec-title">${t('search.sec.modules')} <span class="srch-sec-count">${data.results.modules.length}</span></div>${rows}</div>`);
  }
  ...
  if ((data.results.events || []).length) {
    ...
    sections.push(`<div class="srch-section"><div class="srch-sec-title">${t('search.sec.events')} <span class="srch-sec-count">${data.results.events.length}</span></div>${rows}</div>`);
  }
  ...
  if ((data.results.archive || []).length) {
    ...
    sections.push(`<div class="srch-section"><div class="srch-sec-title">${t('search.sec.archive')} <span class="srch-sec-count">${data.results.archive.length}</span></div>${rows}</div>`);
  }
