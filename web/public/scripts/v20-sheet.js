...
const V20_ATTR_GROUP_LABELS = { physical: t('v20.groupAttr.physical'), social: t('v20.groupAttr.social'), mental: t('v20.groupAttr.mental') };
const V20_ABILITIES = {
...
const V20_ABILITY_GROUP_LABELS = { talents: t('v20.groupAbility.talents'), skills: t('v20.groupAbility.skills'), knowledges: t('v20.groupAbility.knowledges') };
const V20_HEALTH = [
...
function _v20NamedDotRow(namePath, nameVal, dpath, val, max = 5, removeKey = '') {
  const rm = removeKey ? `<button type="button" class="v20-row-remove-btn" data-v20-remove="${escAttr(removeKey)}" data-v20-remove-idx="${dpath.match(/(\d+)/)?.[1] ?? ''}" title="${t('v20.removeRowTip')}">×</button>` : '';
...
function _v20AddRowBtn(removeKey) {
  return `<button type="button" class="v20-mini-action v20-add-row-btn" data-v20-add="${escAttr(removeKey)}" title="${t('v20.addRowTip')}">+</button>`;
}
...
function _v20AutoBadge(actual, computed, path, kind) {
  ...
  const title = match ? t('v20.autoMatch') : t('v20.autoComputed', { n: computed });
  return `<button type="button" class="v20-auto-badge${match ? ' is-match' : ''}" data-auto-path="${path}" data-auto-kind="${kind}" data-auto-val="${computed}" title="${escAttr(title)}">${match ? t('v20.autoLabel') : `↺ ${computed}`}</button>`;
}
...
function _v20UpdateRollPoolInfo() {
  ...
  if (info) info.textContent = t('v20.rollPoolInfo', { a: attrVal, b: abilVal, c: bonus, penalty: penalty ? t('v20.rollPenalty', { n: penalty }) : '', total: pool });
  return pool;
}
...
function _v20DoRoll() {
  ...
  const label = [optLabel(attrSel), optLabel(abilSel)].filter(Boolean).join(' + ') || `${t('v20.rollPoolLabel')} ${pool}`;
  const resultText = r.botch ? t('v20.rollBotch') : r.successes > 0 ? t('v20.rollSucceed', { n: r.successes }) : t('v20.rollFail');
  ...
  const diceHtml = r.dice.length
    ...
    : '<span class="v20-roll-die-empty">' + t('v20.rollPoolZero') + '</span>';
  ...
  _v20RollLog.unshift({ label, text: t('v20.rollLogText', { result: resultText, diff, pool }), successes: r.successes, botch: r.botch });
...
function _v20OpenRollModal(seed) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `<div class="v20-disc-modal v20-roll-modal">
      <button type="button" class="v20-disc-modal-close" id="v20-roll-modal-close" aria-label="${t('v20.rollCloseAria')}">✕</button>
      <h3>${t('v20.rollTitle')}</h3>
      <div class="v20-roll-row">
        <label class="v20-roll-field">${t('v20.rollAttr')}<select id="v20-roll-attr">${attrOpts}</select></label>
        <label class="v20-roll-field">${t('v20.rollAbil')}<select id="v20-roll-abil">${abilOpts}</select></label>
      </div>
      <div class="v20-roll-row">
        <label class="v20-roll-field v20-roll-field-sm">${t('v20.rollBonus')}<input type="number" id="v20-roll-bonus" value="0" class="v20-mini-input"></label>
        <label class="v20-roll-field v20-roll-field-sm">${t('v20.rollDiff')}<input type="number" id="v20-roll-diff" value="6" class="v20-mini-input"></label>
        <label class="v20-roll-spec"><input type="checkbox" id="v20-roll-spec"> ${t('v20.rollSpec')}</label>
      </div>
      <div class="v20-roll-pool-info" id="v20-roll-pool-info"></div>
      <button type="button" class="cdet-sheet-btn primary v20-roll-go" id="v20-roll-go">${t('v20.rollGo')}</button>
      <div class="v20-roll-result" id="v20-roll-result"></div>
      <div class="v20-roll-log" id="v20-roll-log"></div>
    </div>`;
    document.body.appendChild(modal);
    bindBackdropDblClickClose(modal, _v20CloseRollModal);
...
function _libLevelDots(n) {
  ...
  return `<span class="lib-dots" title="${t('v20.level', { n: v })}" aria-label="${t('v20.level', { n: v })}">${'<span class="lib-dot"></span>'.repeat(v)}</span>`;
}
...
function _libPowerHtml(p) {
  return `<div class="lib-power">
    ...
    ${p.literary ? `<div class="lib-power-sec"><div class="lib-power-label">${t('v20.literaryDesc')}</div><p class="lib-power-text">${escHtml(p.literary)}</p></div>` : ''}
    ${p.system ? `<div class="lib-power-sec"><div class="lib-power-label">${t('v20.system')}</div><p class="lib-power-text lib-power-sys">${escHtml(p.system)}</p></div>` : ''}
  </div>`;
}
...
function _libDisciplineDetailHtml(d) {
  if (!d) return '<div class="v20-disc-empty">' + t('v20.discEmpty') + '</div>';
  let body;
  if (d.noLevels && d.paths?.length) {
    ...
    body = `${d.note ? `<div class="v20-disc-note">${escHtml(d.note)}</div>` : ''}` +
      `<p class="lib-power-text">${t('v20.pathsCount', { n: d.paths.length })}</p>`;
  } else {
...
function _libDisciplineCardsHtml() {
  return `<div class="lib-cards">${(_disciplinesCache || []).map(d => {
    const art = d.hasArt
      ...
      : '';
    const badge = d.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
...
function _libSorceryPathsHtml(group) {
  ...
  if (!cards.length) return '<div class="v20-disc-empty">' + t('v20.pathsEmpty') + '</div>';
...
function _libComboCardsHtml() {
  ...
  if (!list.length) return '<div class="v20-disc-empty">' + t('v20.comboEmpty') + '</div>';
...
function _v20RenderDisciplineLibrary() {
  ...
  if (!body) return;
  body.innerHTML = `<h3>${t('v20.discLibTitle')}</h3>${_libDisciplineListHtml()}`;
}
function _v20RenderDisciplineDetail(slug) {
  const d = _discBySlug(slug);
  const html = `<button type="button" class="v20-disc-back" data-disc-back>${t('v20.libBack')}</button>${_libDisciplineDetailHtml(d)}`;
...
function _v20RenderDisciplinePathDetail(discSlug, pathName) {
  ...
  if (!p) { _v20SetLibDetailBody('<div class="v20-disc-empty">' + t('v20.pathEmpty') + '</div>', null); return; }
...
function _v20RenderComboDetail(slug) {
  ...
  if (!c) { _v20SetLibDetailBody('<div class="v20-disc-empty">' + t('v20.comboEmptyDetail') + '</div>', null); return; }
  const html = `<div class="v20-disc-detail-head"><h3>${escHtml(c.name)}</h3>` +
    `<span class="v20-disc-clans">${escHtml(c.clans || '')}</span></div>` +
    `<div class="v20-disc-note">${t('v20.requires', { value: escHtml(c.prereq || '—') })}</div>` +
    `${c.literary ? `<div class="lib-power-sec"><div class="lib-power-label">${t('v20.literaryDesc')}</div><p class="lib-power-text">${escHtml(c.literary)}</p></div>` : ''}` +
    `${c.system ? `<div class="lib-power-sec"><div class="lib-power-label">${t('v20.system')}</div><p class="lib-power-text lib-power-sys">${escHtml(c.system)}</p></div>` : ''}`;
  _v20SetLibDetailBody(html, null);
}
...
function _v20DetailActionsHtml(custom) {
  ...
  const artBtn = `<button type="button" class="cdet-edit-btn cdet-secondary-btn" id="v20-disc-art-btn">${t('v20.artBtn')}</button>`;
  if (!custom) {
    return `<div class="v20-disc-detail-actions">
      <button type="button" class="cdet-edit-btn" id="v20-disc-fork-btn">${t('v20.forkCopy')}</button>
      ${artBtn}
    </div>`;
  }
  return `<div class="v20-disc-detail-actions">
    <button type="button" class="cdet-edit-btn" id="v20-disc-edit-btn">${t('v20.editBtn')}</button>
    <button type="button" class="cdet-delete-btn" id="v20-disc-delete-btn" title="${t('v20.deleteBtn')}">🗑</button>
    ${artBtn}
  </div>`;
}
...
async function _v20EnterLibEdit() {
  ...
  body.querySelector('#v20-disc-edit-btn').textContent = t('v20.saveBtn');
...
async function _v20SaveLibEdit() {
  ...
  if (!body.name) { errEl.textContent = t('v20.nameRequired'); errEl.style.display = ''; return false; }
  ...
  try {
    ...
    if (!r.ok) { errEl.textContent = r.error || t('v20.saveError'); errEl.style.display = ''; return false; }
...
async function _v20DeleteCurrentLibRecord() {
  ...
  const cfg = _LIB_KIND_CONFIG[v.kind];
  const rec = _libFindRecord(v.kind, v.slug, v.category);
  const ok = await showConfirm(t('v20.deleteRecordConfirm', { name: rec?.name || v.slug }), { danger: true, confirmText: t('v20.deleteBtn') });
  if (!ok) return;
  const isJsonTrack = !!cfg.categories;
  const url = isJsonTrack
    ? `/api/library/${v.kind}/${encodeURIComponent(v.category)}/${encodeURIComponent(v.slug)}`
  ...
  if (!r.ok) { showToast(r.error || t('v20.deleteFailed'), 'error'); return; }
...
function _v20UploadCurrentLibArt() {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      ...
      if (!r.ok) { showToast(r.error || t('v20.errUploadImage'), 'error'); return; }
      const cfg = _LIB_KIND_CONFIG[v.kind];
      await cfg.reload(v.category);
      _v20ReopenLibDetail(v.kind, v.slug, v.category);
      showToast(t('v20.imageUpdated'), 'success');
...
function _libImageToPngBase64(file) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => { URL.revokeObjectURL(url); reject(new Error(t('v20.errReadingImage'))); };
    img.src = url;
  });
}
...
async function _v20CloseDisciplineModal() {
  ...
  if (_v20LibEditMode && _v20LibEditDirty) {
    const save = await showConfirm(t('v20.unsavedChanges'),
      { confirmText: t('v20.save'), cancelText: t('v20.dontSave') });
    if (save) {
...
function _v20EnsureLibModal() {
  ...
  modal.className = 'v20-disc-modal-backdrop';
  modal.innerHTML = `<div class="v20-disc-modal" role="dialog" aria-modal="true" aria-label="${t('v20.libModalAria')}">
    <button type="button" class="v20-disc-modal-close" id="v20-disc-modal-close" aria-label="${t('v20.libModalCloseAria')}">✕</button>
    <div id="v20-disc-modal-body"></div>
  </div>`;
  document.body.appendChild(modal);
  bindBackdropDblClickClose(modal, _v20CloseDisciplineModal);
...
async function _v20OpenDisciplineModal(name, preresolvedSlug) {
  _v20OpenLibModalShell();
  const body = document.getElementById('v20-disc-modal-body');
  if (body) body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.libLoading')}</div>`;
  await ensureDisciplines();
...
async function loadLibrary(group) {
  ...
  const body = document.getElementById('lib-disciplines-body');
  if (body) body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loading')}</div>`;
  await ensureDisciplines();
...
function _libPsyDetailHtml(p) {
  if (!p) return '<div class="v20-disc-empty">' + t('v20.psyEmpty') + '</div>';
  const body = `${p.note ? `<div class="v20-disc-note">${escHtml(p.note)}</div>` : ''}${(p.levels || []).map(_libPowerHtml).join('')}`;
  const meta = [p.category, p.roll ? `${t('v20.rollValue', { roll: p.roll })}` : ''].filter(Boolean).join(' · ');
...
function _libPsychicCardsHtml() {
  return `<div class="lib-cards">${(_psychicsCache || []).map(p => {
    const art = p.hasArt
      ...
      : '';
    const badge = p.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
...
function _libMeritDetailHtml(m) {
  if (!m) return '<div class="cdet-empty">' + t('v20.meritEmpty') + '</div>';
  return `<div class="v20-disc-detail-head"><h3>${escHtml(m.name)}</h3></div><div class="merit-lib-detail"><div class="merit-lib-cost">${'●'.repeat(m.points)} ${t('v20.pts')}</div><div class="merit-lib-desc">${escHtml(m.description)}</div></div>`;
}
...
function _libMeritCardsHtml(category) {
  ...
  return `<div class="lib-cards">${merits.map(m => {
    ...
    const inner = `<div class="lib-card-name">${escHtml(m.name)}</div>
      ...
      ${m.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : ''}`;
...
function _v20RenderMeritDetail(slug, category) {
  ...
  const html = `<button type="button" class="v20-disc-back" data-modal-close>${t('v20.libClose')}</button>${_libMeritDetailHtml(merit)}`;
...
async function loadMeritsLibrary(category) {
  ...
  body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loading')}</div>`;
  ...
  try {
    ...
    body.innerHTML = `<div class="cdet-empty">${t('v20.loadError', { msg: escHtml(e.message) })}</div>`;
  }
}
...
function _libFlawDetailHtml(m) {
  if (!m) return '<div class="cdet-empty">' + t('v20.flawEmpty') + '</div>';
  return `<div class="v20-disc-detail-head"><h3>${escHtml(m.name)}</h3></div><div class="merit-lib-detail"><div class="merit-lib-cost">${'●'.repeat(m.points)} ${t('v20.pts')}</div><div class="merit-lib-desc">${escHtml(m.description)}</div></div>`;
}
...
function _v20RenderFlawDetail(slug, category) {
  ...
  const html = `<button type="button" class="v20-disc-back" data-modal-close>${t('v20.libClose')}</button>${_libFlawDetailHtml(flaw)}`;
...
function _libBackgroundDetailHtml(b) {
  if (!b) return '<div class="cdet-empty">' + t('v20.bgEmpty') + '</div>';
  const sect = b.sectOnly ? `<span class="lib-card-sect">${t('v20.only', { name: escHtml(b.sectOnly) })}</span>` : '';
...
function _libBackgroundCardsHtml(category) {
  ...
  return `<div class="lib-cards">${items.map(b => {
    ...
    const inner = `<div class="lib-card-name">${escHtml(b.name)}</div>
      ${b.sectOnly ? `<div class="lib-card-sect">${t('v20.only', { name: escHtml(b.sectOnly) })}</div>` : ''}
      ${b.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : ''}`;
...
function _v20RenderBackgroundDetail(slug, category) {
  ...
  const html = `<button type="button" class="v20-disc-back" data-modal-close>${t('v20.libClose')}</button>${_libBackgroundDetailHtml(bg)}`;
...
function _libClanDetailHtml(c) {
  if (!c) return '<div class="cdet-empty">' + t('v20.clanEmpty') + '</div>';
  const sect = c.sect ? `<span class="lib-card-sect">${escHtml(c.sect)}</span>` : '';
  const rows = [
    c.disciplines ? `<div class="lib-power-sec"><div class="lib-power-label">${t('v20.clanDiscLabel')}</div><p class="lib-power-text">${escHtml(c.disciplines)}</p></div>` : '',
    c.weakness ? `<div class="lib-power-sec"><div class="lib-power-label">${t('v20.clanWeakLabel')}</div><p class="lib-power-text">${escHtml(c.weakness)}</p></div>` : '',
  ].join('');
...
function _libClanCardsHtml() {
  return `<div class="lib-cards">${(_clansCache || []).map(c => {
    const art = c.hasArt
      ...
      : '';
    const badge = c.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
...
function _libSectDetailHtml(s) {
  if (!s) return '<div class="cdet-empty">' + t('v20.sectEmpty') + '</div>';
...
function _libSectCardsHtml() {
  return `<div class="lib-cards">${(_sectsCache || []).map(s => {
    const art = s.hasArt
      ...
      : '';
    const badge = s.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
...
function _libTitleDetailHtml(ttl) {
  if (!ttl) return '<div class="cdet-empty">' + t('v20.titleEmpty') + '</div>';
  const aff = ttl.affiliation ? `<span class="lib-card-sect">${escHtml(ttl.affiliation)}</span>` : '';
  const negBadge = ttl.negative ? `<span class="lib-card-negative-badge">${t('v20.negativeTitleBadge')}</span>` : '';
  const note = ttl.note ? `<div class="v20-disc-note">${escHtml(ttl.note)}</div>` : '';
  const desc = ttl.description ? `<p class="lib-power-text">${escHtml(ttl.description)}</p>` : '';
  return `<div class="v20-disc-detail-head"><h3>${escHtml(ttl.name)}</h3>${aff}</div>${negBadge}${note}${desc}`;
}

function _libTitleCardsHtml() {
  return `<div class="lib-cards">${(_titlesCache || []).map(ttl => {
    const art = ttl.hasArt
      ? `<img class="lib-card-art" loading="lazy" decoding="async" src="/img/system/library/titles/${escAttr(ttl.slug)}.png" alt="">`
      : '';
    const badge = ttl.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
    ...
    const negBadge = ttl.negative ? `<span class="lib-card-negative-badge">${t('v20.negativeBadge')}</span>` : '';
    const aff = ttl.affiliation ? `<div class="lib-card-sect">${escHtml(ttl.affiliation)}</div>` : '';
    const inner = `<div class="lib-card-name">${escHtml(ttl.name)}</div>${aff}${badge}`;
    return `<button type="button" class="lib-card${ttl.hasArt ? ' has-art' : ''}" data-title-slug="${escAttr(ttl.slug)}">
      ${art}${negBadge}${ttl.hasArt ? `<div class="lib-card-overlay">${inner}</div>` : inner}
    </button>`;
  }).join('')}</div>`;
}
...
async function loadKindred(which) {
  if (!which || which === 'clans') {
    const body = document.getElementById('lib-clans-body');
    if (body && !_clansCache) body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loading')}</div>`;
    await ensureClans();
    _libRenderClanList();
  }
  if (!which || which === 'sects') {
    const body = document.getElementById('lib-sects-body');
    if (body && !_sectsCache) body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loading')}</div>`;
    await ensureSects();
    _libRenderSectList();
  }
  if (!which || which === 'titles') {
    const body = document.getElementById('lib-titles-body');
    if (body && !_titlesCache) body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loading')}</div>`;
    await ensureTitles();
    _libRenderTitleList();
  }
}
...
function _libMortCardsHtml(group) {
  ...
  return `<div class="lib-cards">${list.map(r => {
    const art = r.hasArt
      ...
      : '';
    const badge = r.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
...
function _libMortDetailHtml(r) {
  if (!r) return '<div class="v20-disc-empty">' + t('v20.empty') + '</div>';
  const badge = r.custom ? `<span class="lib-card-custom-badge">${t('v20.authorBadge')}</span>` : '';
  return `<div class="v20-disc-detail-head"><h3>${escHtml(r.name)}</h3>${badge}</div>
    ${r.source ? `<div class="v20-disc-note">${t('v20.sourceValue', { source: escHtml(r.source) })}</div>` : ''}
...
async function loadMortalLib(which) {
  for (const group of MORT_GROUPS) {
    ...
    const body = document.getElementById(`lib-mortal-${group}-body`);
    if (body && !_mortLibCache.has(group)) body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loading')}</div>`;
    await ensureMortLib(group);
...
function _v20RenderPsychicLibrary() {
  const body = document.getElementById('v20-disc-modal-body');
  if (!body) return;
  body.innerHTML = `<h3>${t('v20.psyLibTitle')}</h3>${_libPsyListHtml()}`;
}
function _v20RenderPsychicDetail(slug) {
  const p = _psyBySlug(slug);
  const html = `<button type="button" class="v20-disc-back" data-psy-back>${t('v20.libBack')}</button>${_libPsyDetailHtml(p)}`;
...
function _v20LabelFromPath(path, m) {
  ...
  if (parts[0] === 'abilities') return m.abilities[parts[1]][+parts[2]]?.name || t('v20.abilityFallback');
  return path;
}
...
function _v20XpKindFromPath(path, m) {
  ...
  if (path.startsWith('disciplines.')) {
    const name = m.disciplines[+path.split('.')[1]]?.name || t('v20.disciplineFallback');
  ...
  if (path === 'virtues.conscience')  return { kind: 'virtue', label: t('v20.virtueConsc') };
  if (path === 'virtues.selfcontrol') return { kind: 'virtue', label: t('v20.virtueSelf') };
  if (path === 'virtues.courage')     return { kind: 'virtue', label: t('v20.virtueCourage') };
  if (path === 'humanity')            return { kind: 'humanity', label: m.path || t('v20.humanity') };
  if (path === 'willpower.permanent') return { kind: 'willpower', label: t('v20.willpower') };
...
async function _v20RunAction(action) {
  ...
  if (action === 'fill-clan-disc') {
    ...
    if (m.flaw.trim() && !await showConfirm(t('v20.weaknessConfirm'), { confirmText: t('v20.weaknessConfirmBtn') })) return;
...
function _v20RenderV20LibList(kind, lib, pickerEl) {
  ...
  listEl.innerHTML = (lib || []).map(item => {
    ...
    const hint = kind === 'meritflaw'
      ? `${item._kind === 'flaw' ? t('v20.kindFlaw') : t('v20.kindMerit')} · ${item.category || ''} · ${item.points ?? 0} ${t('v20.ptsShort')}`
      : kind === 'background'
      ? [item.category, item.sectOnly ? t('v20.onlyLower', { name: item.sectOnly }) : ''].filter(Boolean).join(' · ')
      : ((item.levels || []).length ? t('v20.levelsCount', { n: (item.levels || []).length }) : '');
...
function _v20Validate(m) {
  ...
  const budget = lineage === 'vampires' ? RULES_V20.creation.vampires.kamarilla
    ...
    : null;
  if (!budget) return { items: [], warnings: [{ text: t('v20.valNoBudget'), level: 'info' }] };
  ...
  const items = [
    { label: t('v20.bandAttrs'), used: attrTotal - 9, max: budget.attrs.reduce((a, b) => a + b, 0) },
    { label: t('v20.bandAbilities'), used: abilTotal, max: budget.abilities.reduce((a, b) => a + b, 0) },
    { label: t('v20.colVirtues'), used: virtTotal - 3, max: budget.virtues },
    { label: t('v20.colBackgrounds'), used: bgTotal, max: budget.backgrounds },
  ];
  if (lineage === 'vampires') items.push({ label: t('v20.valDiscStart'), used: discTotal, max: budget.disciplines });

  const warnings = [];
  const stillChargen = _num(m.experience.spent, 0) === 0;
  if (stillChargen) {
    for (const g of ['talents', 'skills', 'knowledges']) for (const s of m.abilities[g]) {
      if (_num(s.val, 0) > 3 && s.name) warnings.push({ text: t('v20.valCharGenWarn', { name: s.name }), level: 'warn' });
    }
  }
  if (lineage === 'vampires') {
    ...
    for (const g of ['physical', 'social', 'mental']) for (const [k, ru] of V20_ATTRS[g]) {
      if (m.attributes[g][k] > maxDots) warnings.push({ text: t('v20.valAboveMax', { name: ru, val: m.attributes[g][k], max: maxDots, gen: m.header.generation || '—' }), level: 'error' });
    }
    for (const g of ['talents', 'skills', 'knowledges']) for (const s of m.abilities[g]) {
      if (s.name && _num(s.val, 0) > maxDots) warnings.push({ text: t('v20.valAboveMaxSimple', { name: s.name, val: s.val, max: maxDots }), level: 'error' });
    }
    for (const d of m.disciplines) {
      if (d.name && d.val > maxDots) warnings.push({ text: t('v20.valAboveMaxSimple', { name: d.name, val: d.val, max: maxDots }), level: 'error' });
    }
    for (const b of m.backgrounds) {
      if (b.name && b.val > maxDots) warnings.push({ text: t('v20.valAboveMaxSimple', { name: b.name, val: b.val, max: maxDots }), level: 'error' });
    }
  } else {
    for (const g of ['physical', 'social', 'mental']) for (const [k, ru] of V20_ATTRS[g]) {
      if (m.attributes[g][k] > 5) warnings.push({ text: t('v20.valAbove5', { name: ru }), level: 'error' });
    }
  }
...
function _v20RenderValidationReport(m) {
  ...
  const warn = r.warnings.length
    ...
    : `<div class="v20-val-ok">${t('v20.valOk')}</div>`;
  return `<div class="v20-val-budgets">${rows}</div>${warn}<div class="v20-val-note">${t('v20.valNote')}</div>`;
}
...
function _v20RenderSheet(panel, charName) {
  ...
  const rollIcon = (kind, path) => `<button type="button" class="v20-roll-icon-btn" data-roll-seed="${kind}:${path}" title="${t('v20.rollTitle')}">🎲</button>`;
  ...
  const header = `
    <div class="v20-header">
      ${_v20Field(t('v20.fName'), 'header.name', m.header.name)}
      ${_v20Field(t('v20.fNature'), 'header.nature', m.header.nature)}
      ${isVamp ? _v20Field(t('v20.fClan'), 'header.clan', m.header.clan) : _v20Field(t('v20.fConcept'), 'header.concept', m.header.concept)}
      ${_v20Field(t('v20.fPlayer'), 'header.player', m.header.player)}
      ${_v20Field(t('v20.fMask'), 'header.demeanor', m.header.demeanor)}
      ${isVamp ? _v20Field(t('v20.fGen'), 'header.generation', m.header.generation) : _v20Field('—', 'header._x1', '')}
      ${_v20Field(t('v20.fChronicle'), 'header.chronicle', m.header.chronicle)}
      ${_v20Field(t('v20.fArchetype'), 'header.concept', m.header.concept)}
      ${isVamp ? _v20Field(t('v20.fSire'), 'header.sire', m.header.sire) : _v20Field('—', 'header._x2', '')}
    </div>`;

  const attributes = `
    <div class="v20-band">${t('v20.bandAttrs')}</div>
    <div class="v20-grid3">${attrCol('physical')}${attrCol('social')}${attrCol('mental')}</div>`;

  const abilities = `
    <div class="v20-band">${t('v20.bandAbilities')}</div>
  ...
  const discFillBtn = clanInfo && m.disciplines.slice(0, 3).every(d => !d.name)
    ? `<button type="button" class="v20-mini-action" data-v20-action="fill-clan-disc" title="${t('v20.fillClanDiscTip')}">+ ${t('v20.fillClanDiscBtn')}</button>` : '';
  const discLibBtn = `<button type="button" class="v20-mini-action v20-lib-add-btn" data-v20-lib-kind="discipline" title="${t('v20.libDiscTip')}">+ ${t('v20.addFromLibBtn')}</button>`;
  const discRow = (d, i) => {
    const known = !!v20DisciplineKey(d.name);
    const infoBtn = known ? `<button type="button" class="v20-disc-info-btn" data-disc-view="${escAttr(d.name)}" title="${t('v20.rollPowerTitle', { name: escAttr(d.name) })}">ℹ</button>` : '';
    const rm = i >= _V20_BASELINE_LEN.disciplines ? `<button type="button" class="v20-row-remove-btn" data-v20-remove="disciplines" data-v20-remove-idx="${i}" title="${t('v20.removeRowTip')}">×</button>` : '';
  ...
  const discCol = `<div class="v20-col">
    <div class="v20-col-title">${t('v20.colDisciplines')}${discFillBtn}${discLibBtn}${_v20AddRowBtn('disciplines')}</div>
    <div class="v20-lib-picker" data-v20-lib-kind="discipline" hidden>
      <input type="text" class="v20-lib-search" placeholder="${t('v20.sheetSearchPh')}" data-v20-lib-kind="discipline">
      <div class="v20-lib-list" data-v20-lib-kind="discipline"></div>
    </div>
    ${m.disciplines.map(discRow).join('')}
  </div>`;
  const bgLibBtn = `<button type="button" class="v20-mini-action v20-lib-add-btn" data-v20-lib-kind="background" title="${t('v20.libBgTip')}">+ ${t('v20.addFromLibBtn')}</button>`;
  const bgCol = `<div class="v20-col"><div class="v20-col-title">${t('v20.colBackgrounds')}${bgLibBtn}${_v20AddRowBtn('backgrounds')}</div>
    <div class="v20-lib-picker" data-v20-lib-kind="background" hidden>
      <input type="text" class="v20-lib-search" placeholder="${t('v20.sheetSearchPh')}" data-v20-lib-kind="background">
  ...
  const virtCol = `<div class="v20-col"><div class="v20-col-title">${t('v20.colVirtues')}</div>
      ${_v20DotRow(t('v20.virtueConsc'), 'virtues.conscience', m.virtues.conscience)}
      ${_v20DotRow(t('v20.virtueSelf'), 'virtues.selfcontrol', m.virtues.selfcontrol)}
      ${_v20DotRow(t('v20.virtueCourage'), 'virtues.courage', m.virtues.courage)}</div>`;
  const advantages = `
    <div class="v20-band">${t('v20.bandAdvantages')}</div>
  ...
  const psyLibBtn = `<button type="button" class="v20-mini-action v20-lib-add-btn" data-v20-lib-kind="numina" title="${t('v20.libPsyTip')}">+ ${t('v20.addFromLibBtn')}</button>`;
  const psyRow = (p, i) => {
    const known = !!v20PsychicKey(p.name);
    const infoBtn = known ? `<button type="button" class="v20-disc-info-btn" data-psy-view="${escAttr(p.name)}" title="${t('v20.rollPowerTitle', { name: escAttr(p.name) })}">ℹ</button>` : '';
    const rm = i >= _V20_BASELINE_LEN.psychicPowers ? `<button type="button" class="v20-row-remove-btn" data-v20-remove="psychicPowers" data-v20-remove-idx="${i}" title="${t('v20.removeRowTip')}">×</button>` : '';
  ...
  const psychics = isMortal ? `
    <div class="v20-band">${t('v20.bandPsychic')}</div>
    <div class="v20-grid3">
      <div class="v20-col">
        <div class="v20-col-title">${t('v20.colNumina')}${psyLibBtn}${_v20AddRowBtn('psychicPowers')}</div>
        <div class="v20-lib-picker" data-v20-lib-kind="numina" hidden>
          <input type="text" class="v20-lib-search" placeholder="${t('v20.sheetSearchPh')}" data-v20-lib-kind="numina">
  ...
  const mfLibBtn = `<button type="button" class="v20-mini-action v20-lib-add-btn" data-v20-lib-kind="meritflaw" title="${t('v20.libMfTip')}">+ ${t('v20.addFromLibBtn')}</button>`;
  const mfRows = m.meritsFlaws.map((mf, i) => {
    const rm = `<button type="button" class="v20-row-remove-btn" data-v20-remove="meritsFlaws" data-v20-remove-idx="${i}" title="${t('v20.removeRowTip')}">×</button>`;
    return `<div class="v20-row v20-named v20-mf-row">${rm}
      <input class="v20-line-input" data-tpath="meritsFlaws.${i}.name" value="${escAttr(mf.name)}" placeholder="${t('v20.mfNamePh')}">
      <select class="v20-mf-kind" data-tpath="meritsFlaws.${i}.kind">
        <option value="merit"${mf.kind !== 'flaw' ? ' selected' : ''}>${t('v20.kindMerit')}</option>
        <option value="flaw"${mf.kind === 'flaw' ? ' selected' : ''}>${t('v20.kindFlaw')}</option>
      </select>
  ...
  const bottom = `
    <div class="v20-band">${t('v20.bandAdvState')}</div>
    <div class="v20-grid3 v20-bottom">
      <div class="v20-col">
        <div class="v20-col-title">${t('v20.colMf')}${mfLibBtn}${_v20AddRowBtn('meritsFlaws')}</div>
        <div class="v20-lib-picker" data-v20-lib-kind="meritflaw" hidden>
          <input type="text" class="v20-lib-search" placeholder="${t('v20.sheetSearchPh')}" data-v20-lib-kind="meritflaw">
          <div class="v20-lib-list" data-v20-lib-kind="meritflaw"></div>
        </div>
        ${mfRows}
        <div class="v20-col-title" style="margin-top:12px">${t('v20.colFlaw')}${clanInfo && clanInfo.weakness ? `<button type="button" class="v20-mini-action" data-v20-action="insert-clan-weakness" title="${t('v20.insertWeaknessTip')}">+ ${t('v20.insertWeaknessBtn')}</button>` : ''}</div>
        <input class="v20-field-input" data-tpath="flaw" value="${escAttr(m.flaw)}">
      </div>
      <div class="v20-col">
        <div class="v20-stat-block v20-stat-block--centered">
          <div class="v20-stat-title"><input class="v20-line-input v20-path-title" data-tpath="path" value="${escAttr(m.path)}" placeholder="${t('v20.humanity')}">${_v20AutoBadge(m.humanity, derived.humanity, 'humanity', 'dot')}</div>
          ${_v20DotsHtml('humanity', m.humanity, 10)}
        </div>
        <div class="v20-stat-block v20-stat-block--centered">
          <div class="v20-stat-title">${t('v20.willpower')} ${_v20AutoBadge(m.willpower.permanent, derived.willpower, 'willpower.permanent', 'dot')}<button type="button" class="v20-mini-action" data-v20-action="spend-willpower" title="${t('v20.spendWillpowerTip')}">−1</button></div>
          ${_v20DotsHtml('willpower.permanent', m.willpower.permanent, 10)}
          <div class="v20-stat-title v20-stat-subtitle">${t('v20.statWillTemp')}</div>
          ${_v20BoxesHtml('willpower.temp', m.willpower.temp)}
        </div>
        ${isVamp ? `<div class="v20-stat-block v20-stat-block--centered">
          <div class="v20-stat-title">${t('v20.statBlood')}${derived.gen ? `<span class="v20-gen-info" title="${t('v20.genInfoTitle', { gen: m.header.generation, blood: derived.gen.bloodMax ?? t('v20.genNoCap'), turn: derived.gen.bloodPerTurn ?? '—', dots: derived.gen.maxDots })}">ⓘ</span>` : ''}<button type="button" class="v20-mini-action" data-v20-action="spend-blood" title="${t('v20.spendBloodTip')}">−1</button></div>
          ${derived.gen && derived.gen.bloodMax == null
            ? `<input type="number" min="0" class="v20-mini-input v20-blood-counter" data-tpath="bloodPoolCount" value="${escAttr(m.bloodPoolCount)}" title="${t('v20.gen3CountTitle')}">`
            : _v20BoxesHtml('bloodPool', m.bloodPool)}
          <label class="v20-inline-field">${t('v20.lblBloodTurn')} ${derived.gen && derived.gen.bloodPerTurn != null ? _v20AutoBadge(m.bloodPerTurn, derived.gen.bloodPerTurn, 'bloodPerTurn', 'text') : ''}<input class="v20-mini-input" data-tpath="bloodPerTurn" value="${escAttr(m.bloodPerTurn)}"></label>
        </div>` : ''}
      </div>
      <div class="v20-col">
        <div class="v20-col-title">${t('v20.colHealth')}</div>
        <div class="v20-health">${healthRows}</div>
        <div class="v20-stat-block" style="margin-top:12px">
          <div class="v20-stat-title">${t('v20.statExp')}</div>
          <label class="v20-inline-field">${t('v20.expTotal')} <input class="v20-mini-input" data-tpath="experience.total" data-exp value="${escAttr(m.experience.total)}"></label>
          <label class="v20-inline-field">${t('v20.expSpent')} <input class="v20-mini-input" data-tpath="experience.spent" data-exp value="${escAttr(m.experience.spent)}"></label>
          <label class="v20-inline-field">${t('v20.expRemain')} <span class="v20-exp-remain" id="v20-exp-remain">${_num(m.experience.total, 0) - _num(m.experience.spent, 0)}</span></label>
        </div>
        ${m.experience.log?.length ? `<div class="v20-xp-log">
          <div class="v20-xp-log-title">${t('v20.xpLogTitle')}</div>
  ...
  const specRows = m.specializations.map((s, i) =>
    `<div class="v20-pair-row"><input class="v20-line-input" data-tpath="specializations.${i}.ability" value="${escAttr(s.ability)}" placeholder="${t('v20.specAbilityPh')}"><input class="v20-line-input" data-tpath="specializations.${i}.spec" value="${escAttr(s.spec)}" placeholder="${t('v20.specSpecPh')}"></div>`).join('');
  ...
  const ritRows = m.rituals.map((r, i) => {
    const rm = i >= _V20_BASELINE_LEN.rituals ? `<button type="button" class="v20-row-remove-btn" data-v20-remove="rituals" data-v20-remove-idx="${i}" title="${t('v20.removeRowTip')}">×</button>` : '';
    return `<div class="v20-pair-row">${rm}<input class="v20-line-input" data-tpath="rituals.${i}.name" value="${escAttr(r.name)}" placeholder="${t('v20.ritNamePh')}"><input class="v20-line-input v20-ritual-lvl" data-tpath="rituals.${i}.level" value="${escAttr(r.level)}" placeholder="${t('v20.ritLevelPh')}"></div>`;
  }).join('');
  const V20_DESC = [['birthDate', t('v20.descBirth')], ['apparentAge', t('v20.descAppAge')], ['deathDate', t('v20.descDeath')], ['gender', t('v20.descGender')], ['race', t('v20.descRace')], ['hair', t('v20.descHair')], ['eyes', t('v20.descEyes')], ['heightWeight', t('v20.descHeightW')], ['build', t('v20.descBuild')], ['nationality', t('v20.descNationality')]];
  ...
  const descFields = V20_DESC.map(([k, l]) => {
    ...
    return `<label class="v20-field"><span class="v20-field-lbl">${escHtml(l)}${badge}</span><input class="v20-field-input" data-tpath="description.${k}" value="${escAttr(m.description[k] || '')}" title="${isComputed ? t('v20.apparentAgeTip') : ''}"></label>`;
  }).join('');
  ...
  const V20_COMBAT_LABELS = [t('v20.combatWeapon'), t('v20.combatDiff'), t('v20.combatDamage'), t('v20.combatRange'), t('v20.combatRate'), t('v20.combatClip'), t('v20.combatSize')];
  ...
  const page2 = `
    <div class="v20-band">${t('v20.bandSpecParams')}${(isVamp || isMortal) ? t('v20.bandSepRituals') : ''}</div>
    <div class="v20-grid3">
      <div class="v20-col"><div class="v20-col-title">${t('v20.colSpec')}</div>${specRows}</div>
      <div class="v20-col"><div class="v20-col-title">${t('v20.colOther')}${_v20AddRowBtn('otherTraits')}</div>${otRows}</div>
      ${(isVamp || isMortal) ? `<div class="v20-col"><div class="v20-col-title">${t('v20.colRituals')}${_v20AddRowBtn('rituals')}</div>${ritRows}</div>` : '<div class="v20-col"></div>'}
    </div>
    <div class="v20-band">${t('v20.bandHistory')}</div>
    <div class="v20-grid3">
      <div class="v20-col">
        <div class="v20-col-title">${t('v20.colHistory')}</div>
        <textarea class="v20-textarea" data-tpath="history" rows="5" placeholder="${t('v20.historyPh')}">${escHtml(m.history)}</textarea>
        <div class="v20-col-title" style="margin-top:12px">${t('v20.colGoals')}</div>
        <textarea class="v20-textarea" data-tpath="goals" rows="3" placeholder="${t('v20.goalsPh')}">${escHtml(m.goals)}</textarea>
      </div>
      <div class="v20-col">
        <div class="v20-col-title">${t('v20.colDesc')}</div>
        <div class="v20-desc-fields">${descFields}</div>
      </div>
      <div class="v20-col">
        <div class="v20-col-title">${t('v20.colAllies')}</div>
        <textarea class="v20-textarea" data-tpath="allies" rows="4" placeholder="${t('v20.onePerLinePh')}">${escHtml(m.allies)}</textarea>
        <div class="v20-col-title" style="margin-top:12px">${t('v20.colPossessions')}</div>
        <textarea class="v20-textarea" data-tpath="possessions" rows="4" placeholder="${t('v20.onePerLinePh')}">${escHtml(m.possessions)}</textarea>
      </div>
    </div>
    <div class="v20-band">${t('v20.bandCombat')}</div>
    <div class="v20-combat">${combatHead}${combatRows}</div>`;

  panel.innerHTML = `
    <div class="cdet-sheet-toolbar">
      <button class="cdet-sheet-btn primary" id="v20-save" disabled title="${t('v20.saveTip')}">${t('v20.savedSheet')}</button>
      <button class="cdet-sheet-btn" id="v20-regen" title="${t('v20.regenTip')}">${t('v20.regenBtn')}</button>
      <button class="cdet-sheet-btn" id="v20-validate" title="${t('v20.validateTip')}">${t('v20.validateBtn')}</button>
      <button class="cdet-sheet-btn${_v20XpMode ? ' active' : ''}" id="v20-xpmode" title="${t('v20.xpTip')}">${t('v20.xpModeBtn')}${_v20XpMode ? t('v20.xpModeOn') : ''}</button>
      <button class="cdet-sheet-btn" id="v20-roll-btn" title="${t('v20.rollTip')}">${t('v20.rollTitle')}</button>
      ${(isVamp || isMortal) ? `
      <button class="cdet-sheet-btn" id="v20-foundry-export" title="${t('v20.foundryExportTip')}">${t('v20.foundryExportBtn')}</button>
      <button class="cdet-sheet-btn" id="v20-foundry-import" title="${t('v20.foundryImportTip')}">${t('v20.foundryImportBtn')}</button>` : ''}
    ...
    <div class="v20-foot">${t('v20.creationFoot')}</div>`;
  ...
  const onDot = async dot => {
    ...
    if (_v20XpMode && nv > cur) {
      ...
      if (info) {
        ...
        if (cost > avail && !await showConfirm(t('v20.xpConfirm', { name: info.label, cur, next: nv, cost, avail }), { confirmText: t('v20.xpConfirmBtn') })) return;
...
function _v20MarkDirty() {
  ...
  if (btn) { btn.disabled = false; btn.textContent = t('v20.btnSaveSheet'); }
  if (st) { st.textContent = t('v20.unsaved'); st.className = 'v20-status dirty'; }
}

async function _v20Save() {
  const btn = document.getElementById('v20-save'), st = document.getElementById('v20-status');
  btn.disabled = true; const old = btn.textContent; btn.textContent = t('v20.saving');
  ...
  try {
    ...
    if (!r.ok) throw new Error(r.error || t('v20.errGeneric'));
    _v20DirtyFlag = false;
    btn.textContent = t('v20.savedSheet'); btn.disabled = true;
    if (st) { st.textContent = t('v20.savedStatus'); st.className = 'v20-status ok'; }
  } catch (e) {
...
async function _v20ImportFoundryFile(file) {
  ...
  try {
    ...
    showToast(t('v20.foundryReadErr', { msg: e.message }), 'error');
    return;
  }
  ...
  try {
    ...
    if (!r.ok) throw new Error(r.error || t('v20.errGeneric'));
    showToast(t('v20.foundryImported'), 'success');
    ...
    if (r.cardFields && (r.cardFields.clan || r.cardFields.sect || r.cardFields.generation || r.cardFields.sire)) {
      showToast(t('v20.foundryCardFields', { clan: r.cardFields.clan || '—', sect: r.cardFields.sect || '—', gen: r.cardFields.generation || '—' }), 'info', 8000);
    }
  } catch (e) {
    showToast(t('v20.foundryImportErr', { msg: e.message }), 'error');
  }
}
...
async function _v20Regen(btn) {
  if (_v20DirtyFlag && !await showConfirm(t('v20.regenDirty'), { danger: true, confirmText: t('v20.regenBtnConfirm') })) return;
  else if (!_v20DirtyFlag && !await showConfirm(t('v20.regenClean'), { confirmText: t('v20.regenBtnConfirm') })) return;
  const old = btn.textContent; btn.disabled = true; btn.textContent = t('v20.regenLoading');
  try {
    const ok = await _generateSheet({ scope: 'character', name: _v20Ctx.name, slug: _v20Ctx.slug }, null);
    if (!ok) throw new Error(t('v20.genFailed'));
    ...
    showToast(t('v20.errPrefix', { msg: e.message }), 'error');
    btn.disabled = false; btn.textContent = old;
  }
}
...
async function _loadCharSheet(charSlug) {
  ...
  panel.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.sheetLoading')}</div>`;
  ...
  catch (e) { panel.innerHTML = `<div class="cdet-empty">${t('v20.loadError', { msg: escHtml(e.message) })}</div>`; return; }
  _v20Model = _v20ModelFrom(d);
...
function _renderLibList(state, groupIdx, filter) {
  ...
  if (!filtered.length) return `<div class="sheet-lib-empty">${t('v20.sheetNothingFound')}</div>`;
...
function _renderSheetEditor(state) {
  ...
  if (!parsed.editable.length) return `<div class="cdet-empty">${t('v20.sheetEmpty')}</div>`;
  return parsed.groups.map((g, gi) => {
    ...
    const rows = g.rows.map(r => {
      ...
      return `<div class="sheet-edit-row">
        ...
        <button type="button" class="sheet-row-remove" data-row="${idx}" title="${t('v20.removeRowTip')}">✕</button>
      </div>`;
    }).join('');
    const kind = _sheetLibraryKind(g);
    const addUi = (kind && state.library[kind]) ? `
      <button type="button" class="sheet-add-btn" data-group-idx="${gi}">${t('v20.sheetAddFromLib')}</button>
      <div class="sheet-lib-picker" data-group-idx="${gi}" hidden>
        <input type="text" class="sheet-lib-search" data-group-idx="${gi}" placeholder="${t('v20.sheetSearchPh')}">
...
function _ensureSheetOverlay() {
  ...
  ov.innerHTML = `<div class="sheet-modal">
    <div class="sheet-modal-head">
      <span class="sheet-modal-title" id="sheet-modal-title">${t('v20.sheetModalTitle')}</span>
      <div class="sheet-modal-actions" id="sheet-modal-actions"></div>
      <button class="sheet-modal-close" id="sheet-modal-close" title="${t('v20.sheetCloseTip')}">✕</button>
    </div>
    <div class="sheet-modal-body" id="sheet-modal-body"></div>
    <div class="sheet-modal-status" id="sheet-modal-status"></div>
  </div>`;
  document.body.appendChild(ov);
  bindBackdropDblClickClose(ov, _closeSheetOverlay);
...
async function openSheetOverlay(ctx, mode) {
  ...
  title.textContent = t('v20.sheetTitle', { name: ctx.name || ctx.label || t('v20.personDefault') });
  body.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('v20.loadingSoon')}</div>`;

  let d;
  try { d = await fetch(_sheetApi(ctx).get).then(r => r.json()); }
  catch (e) { body.innerHTML = `<div class="cdet-empty">${t('v20.errPrefix', { msg: escHtml(e.message) })}</div>`; return; }
  if (!d.exists || !d.content) { body.innerHTML = `<div class="cdet-empty">${t('v20.sheetNotGenerated')}</div>`; return; }
  ...
  if (mode === 'edit') {
    ...
    actions.innerHTML = `<button class="sheet-btn sheet-btn-save" id="sheet-save">${t('v20.btnSaveSheet')}</button>`;
    ...
    actions.innerHTML = `<button class="sheet-btn" id="sheet-to-edit">${t('v20.editBtn')}</button>`;
...
async function _saveSheetEdit() {
  ...
  btn.disabled = true; btn.textContent = t('v20.saving'); status.textContent = ''; status.className = 'sheet-modal-status';
  try {
    ...
    if (!d.ok) throw new Error(d.error || t('v20.errGeneric'));
    status.textContent = t('v20.savedStatus'); status.classList.add('ok');
    _sheetEditState.ctx.onSaved?.();
  } catch (e) { status.textContent = '✗ ' + e.message; status.classList.add('err'); }
  finally { btn.disabled = false; btn.textContent = t('v20.btnSaveSheet'); }
}

// Generate (or regenerate) a sheet for any context (character / module NPC).
async function _generateSheet(ctx, btn) {
  const old = btn ? btn.textContent : '';
  if (btn) { btn.disabled = true; btn.textContent = t('v20.genLoading'); }
  try {
    ...
    ctx.onSaved?.();
    return true;
  } catch (e) { showToast(t('v20.genError', { msg: e.message }), 'error'); return false; }
  finally { if (btn) { btn.disabled = false; btn.textContent = old; } }
}

async function _diaryGenerate(charSlug) {
  const period = document.getElementById('diary-period').value.trim();
  if (!period) { _diaryMsg(t('v20.diaryPeriodPrompt'), false); return; }
  const btn = document.getElementById('diary-gen'); const old = btn.textContent;
  btn.disabled = true; btn.textContent = t('v20.genLoading'); _diaryMsg('');
  try {
    ...
    _diaryMsg(t('v20.diaryGenerated', { source: r.source }));
  } catch (e) { _diaryMsg(t('v20.errPrefix', { msg: e.message }), false); }
...
async function _diarySave(charSlug) {
  ...
  if (!period) { _diaryMsg(t('v20.diaryPeriodPrompt'), false); return; }
  if (!text)   { _diaryMsg(t('v20.diaryEmptyText'), false); return; }
