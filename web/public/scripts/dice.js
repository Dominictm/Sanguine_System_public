...
if (typeof document !== 'undefined' && document.getElementById('dice-fab')) {
  ...
  function _diceFillVirtues(m) {
    ...
    const items = [
      [t('dice.virtueConscience'), has ? m.virtues.conscience : null],
      [t('dice.virtueSelfcontrol'), has ? m.virtues.selfcontrol : null],
      [t('dice.virtueCourage'), has ? m.virtues.courage : null],
      [t('dice.virtueWillpower'), has ? m.willpower.permanent : null],
    ];
    virtueSel.innerHTML = '<option value="">' + t('dice.virtue.ph') + '</option>' +
      items.map(([label, val]) => {
  ...
  function _diceFillFromModel(m) {
    ...
    if (!m || !m.attributes) {
      ...
      attrSel.innerHTML = '<option value="">' + t('dice.attr.ph') + '</option>' +
        attrLabels.map(label => `<option value="">${escHtml(label)}</option>`).join('');
      abilSel.innerHTML = '<option value="">' + t('dice.abil.ph') + '</option>' +
        abilLabels.map(label => `<option value="">${escHtml(label)}</option>`).join('') +
        '<option value="unskilled">' + t('dice.unskilledShort') + '</option>';
  ...
  async function _diceFillCharSelect() {
    ...
    charSel.innerHTML = '<option value="">' + t('dice.char.ph') + '</option>' +
  ...
  function _diceSuccessWord(n) {
    if (n % 10 === 1 && n % 100 !== 11) return t('dice.success.one');
    if (n % 10 >= 2 && n % 10 <= 4 && (n % 100 < 12 || n % 100 > 14)) return t('dice.success.few');
    return t('dice.success.many');
  }

  function _diceVerdict(r) {
    if (r.result === 'botch') return '<span class="dice-verdict botch">' + t('dice.verdict.botch') + '</span>';
    if (r.result === 'failure') return '<span class="dice-verdict fail">' + t('dice.verdict.failure') + '</span>';
    const n = r.net;
    return `<span class="dice-verdict ok">${n} ${_diceSuccessWord(n)}</span>`;
  }
  ...
  document.getElementById('dice-roll').addEventListener('click', () => {
    ...
    histEl.innerHTML = _history.map(h =>
      `<div class="dice-hist-row">${h.pool}к${h.difficulty} → ${
        h.r.result === 'botch' ? t('dice.hist.botch') : h.r.result === 'failure' ? t('dice.hist.failure') : h.r.net + ' ' + t('dice.hist.success')
      }</div>`).join('');
  });
}
