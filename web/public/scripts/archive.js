...
const THREAD_STATUS_OPTS = [
  ['active', 'threads.status.active'], ['background', 'threads.status.background'],
  ['closed', 'threads.status.closed'], ['abandoned', 'threads.status.abandoned'],
];
const THREAD_PRIO_OPTS = ['threads.prio.high', 'threads.prio.medium', 'threads.prio.low'];
...
function fileLabel(f) {
  if (f === 'archive/open_threads.md') return t('archive.file.city');
...
async function loadThreads() {
  ...
  try {
    ...
    document.getElementById('threads-sub').textContent =
      t('archive.th.sub', { active: active.length, bg: bg.length, done: done.length });
    ...
    const statusSelect = thr => `<select class="thread-status">${
      THREAD_STATUS_OPTS.map(([v, l]) => `<option value="${v}"${v === thr.status ? ' selected' : ''}>${t(l)}</option>`).join('')}</select>`;
    const prioSelect = thr => `<select class="thread-prio">${
      THREAD_PRIO_OPTS.map(p => `<option${p === thr.priority ? ' selected' : ''}>${t(p)}</option>`).join('')}</select>`;
    ...
    const section = (icon, label, list) => list.length
      ...
      : '';
    html += section('🔴', t('archive.th.secActive'), active);
    html += section('🟡', t('archive.th.secBackground'), bg);
    html += section('🟢', t('archive.th.secClosed'), done);
    el.innerHTML = html || `<div class="loading-state" style="height:120px">${t('archive.th.none')}</div>`;
...
document.getElementById('threads-list')?.addEventListener('change', async e => {
  ...
  try {
    ...
    if (r.ok) loadThreads(); else showToast(r.error || t('archive.th.updateError'), 'error');
  } catch (err) { showToast(t('archive.errorMsg', { msg: err.message }), 'error'); }
});
...
(function wireThreadForm() {
  ...
  document.getElementById('btn-save-thread').addEventListener('click', async () => {
    ...
    if (!payload.title) { err.textContent = t('archive.th.titleReq'); err.style.display = ''; return; }
    try {
      ...
      if (!r.ok) { err.textContent = r.error || t('archive.error'); err.style.display = ''; return; }
      ['th-title', 'th-desc', 'th-source'].forEach(id => document.getElementById(id).value = '');
      toggle(false);
      loadThreads();
    } catch (e) { err.textContent = t('archive.errorMsg', { msg: e.message }); err.style.display = ''; }
  });
})();
...
const SPINNER = `<div class="loading-state"><div class="spinner"></div>${t('ui.loading')}</div>`;
...
async function loadChronicle() {
  ...
  try {
    ...
    el.innerHTML = `<div class="loading-state" style="color:var(--accent3)">${t('archive.chronicleLoadErr')}</div>`;
  }
}
...
function renderChronicle() {
  ...
  if (!data.exists) {
    document.getElementById('chronicle-sub').textContent = t('archive.chronicleNotSet');
    el.innerHTML = `<div class="loading-state" style="height:140px">${t('archive.chronicleFileNotFound')}</div>`;
    return;
  }
  ...
  if (st.tab === 'world') {
    sub.textContent = t('archive.worldStateTitle');
    loadWorldStateForm();
    return;
  }
  if (st.tab === 'lore') {
    sub.textContent = t('archive.timelineTitle');
    loadTimelineForm();
    return;
  }
  ...
  const evWord = evCount % 10 === 1 && evCount % 100 !== 11 ? t('archive.ev.one')
    : (evCount % 10 >= 2 && evCount % 10 <= 4 && (evCount % 100 < 12 || evCount % 100 > 14) ? t('archive.ev.few') : t('archive.ev.many'));
...
async function _loadArchiveEditable(getUrl, putUrl, containerId, emptyMsg, opts = {}) {
  ...
  const d20Part = opts.d20
    ? `<button class="archive-d20-btn" id="${uid}-d20-btn">${t('archive.rollD20')}</button><span class="archive-d20-badge" id="${uid}-d20-badge"></span>`
    : '';

  el.innerHTML = `
    <div class="archive-toolbar">
      ${d20Part}
      <button class="archive-edit-btn" id="${uid}-edit-btn">${t('common.edit')}</button>
    </div>
...
<button class="btn-submit" id="${uid}-save-btn">${t('archive.saveBtn')}</button>
      <button class="btn-submit btn-secondary" id="${uid}-cancel-btn">${t('ui.btn.cancel')}</button>
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!r.ok) throw new Error(r.error || t('archive.error'));
      ...
      saveBtn.disabled = false; saveBtn.textContent = t('archive.saveBtn');
    }
  });
...
async function loadFactionsInfluence() {
  ...
  const rows = factions.map(f => `
    <div class="faction-inf-row" data-faction="${escAttr(f.name)}">
      ...
      <button class="chron-toggle faction-inf-del" title="${t('archive.factionDelTitle', { name: escAttr(f.name) })}" aria-label="${t('archive.factionDelAria', { name: escAttr(f.name) })}">🗑</button>
    </div>`).join('');

  panel.innerHTML = `
    <div class="faction-inf-panel">
      <div class="faction-inf-header">
        <span class="faction-inf-title">${t('archive.factionInfluenceTitle')}</span>
        <div class="faction-inf-add">
          <input type="text" id="faction-inf-add-name" placeholder="${t('archive.factionNewPh')}" class="chr-form-input">
          <button class="modp-edit-btn" id="faction-inf-add-btn">${t('relmgr.btn.add')}</button>
        </div>
      </div>
      ${rows || `<div class="cdet-empty">${t('archive.factionNoSection')}</div>`}
    </div>`;
  ...
  panel.querySelectorAll('.faction-inf-input').forEach(input => {
    input.addEventListener('change', async () => {
      ...
      } catch (e) { showToast(t('archive.factionInfluenceSaveErr', { msg: e.message }), 'error'); }
    });
  });
  ...
  panel.querySelectorAll('.faction-inf-del').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const ok = typeof showConfirm === 'function'
        ? await showConfirm(t('archive.factionDelConfirm', { name }), { confirmText: t('ui.btn.delete') })
        : window.confirm(t('archive.factionDelConfirm', { name }));
      if (!ok) return;
      try {
      ...
      } catch (e) { showToast(t('archive.factionDelErr', { msg: e.message }), 'error'); }
    });
  });
  ...
  panel.querySelector('#faction-inf-add-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (!r.ok) throw new Error(r.error || t('archive.error'));
      await loadFactionsInfluence();
    } catch (e) { showToast(t('archive.factionAddErr', { msg: e.message }), 'error'); }
  });
}
...
function _archiveRumors(viewEl, type) {
  ...
  if (!told.size) {
    showToast(t('archive.rumorNoTold'), 'warning');
    return;
  }
  ...
  if (!rumorData.length) { showToast(t('archive.rumorNoData'), 'error'); return; }
...
function _renderRumorsArchive(city, type) {
  ...
  const typeLabel  = type === 'dreaming' ? t('archive.rumorTypeDream') : t('archive.rumorTypeElysium');
  ...
  container.innerHTML = `
    <div class="rumors-arch-header" style="--arch-accent:${accentVar}">
      ${t('archive.rumorArchTitle', { type: typeLabel })}
    </div>
    ${archive.map((sess, i) => {
      const pl = sess.rumors.length === 1 ? t('archive.rumorCount.one') : sess.rumors.length < 5 ? t('archive.rumorCount.few') : t('archive.rumorCount.many');
      return `
        <div class="rumors-arch-session">
          ...
          <div class="rumors-arch-body" id="rarch-body-${i}" style="display:none">
            <table class="rumors-arch-table">
              <thead><tr><th>#</th><th>${t('archive.rumorColRumor')}</th><th>${t('archive.rumorColSource')}</th><th>✓</th></tr></thead>
              <tbody>
...
function loadRumors() {
  ...
  _loadArchiveEditable(
    ...
    '/api/rumors',
    'rumors-content',
    t('archive.rumorFileNotFound', { file: type === 'dreaming' ? 'rumors_dreaming.md' : 'rumors_elysium.md' }),
    {
      d20: true,
      rumorsType: type,
      putExtra: { type },
      onRendered: (viewEl) => {
        ...
        if (toolbar && !toolbar.querySelector('.rumors-gen-btn')) {
          ...
          genBtn.textContent = t('archive.rumorGenBtn');
          genBtn.title = t('archive.rumorGenTitle');
...
function renderWorldState(ws) {
  if (!ws || !ws.sections || !ws.sections.length)
    return `<div class="cdet-empty" style="padding:30px">${t('archive.wsEmpty')}</div>`;
  let html = '';
  if (ws.lastUpdate)
    html += `<div class="ws-updated">${t('archive.wsUpdatedLabel')} <b>${escHtml(ws.lastUpdate)}</b></div>`;
...
function renderTimeline(events) {
  if (!events.length)
    return `<div class="cdet-empty" style="padding:30px">${t('archive.noEvents')}</div>`;
  ...
  return events.map(ev => {
    ...
    const chips = (ev.participants || []).map(p => {
      ...
      if (char) {
        const icon = lineageIconHtml(char.lineage);
    ...
    const finaleBlock = modLink
      ? `<div class="chron-block-label">${t('archive.chronFinaleLabel')}</div><div class="chron-finale" data-finale-mod="${escHtml(modLink.module)}">${SPINNER}</div>`
      : '';

    const body = [
      ev.eventsText ? `<div class="chron-block-label">${t('archive.chronEventsLabel')}</div><div class="md-body chron-md">${mdToHtmlBlock(ev.eventsText)}</div>` : '',
      ev.consequences.length ? `<div class="chron-block-label">${t('archive.chronConsequencesLabel')}</div><ul class="chron-list">${ev.consequences.map(c => `<li>${mdInline(c)}</li>`).join('')}</ul>` : '',
      ev.worldChanges.length ? `<div class="chron-block-label">${t('archive.chronWorldChangesLabel')}</div><ul class="chron-list">${ev.worldChanges.map(c => `<li>${mdInline(c)}</li>`).join('')}</ul>` : '',
      finaleBlock,
    ].filter(Boolean).join('');
    ...
    return `
      <article class="chron-event" data-id="${ev.id}">
        <div class="chron-event-head">
          ...
          ${ev.parallel ? `<span class="chron-parallel" title="${escHtml(ev.parallel)}">${t('archive.evParallel')}</span>` : ''}
        </div>
        ...
        ${body ? `<button class="chron-toggle" data-id="${ev.id}" aria-expanded="false">${t('archive.details')}</button>
...
async function loadTimelineForm() {
  ...
  try {
    ...
    el.innerHTML = `<div class="loading-state" style="color:var(--accent3)">${t('archive.timelineLoadErr')}</div>`;
  }
}
  ...
  const epochsHtml = data.epochs.map(ep => `
    <section class="chron-event" data-epoch="${escHtml(ep.heading)}">
      <div class="chron-event-head">
        ...
        <button class="chron-toggle" data-epoch-del="${escHtml(ep.heading)}" title="${t('archive.delEpoch')}">🗑</button>
      </div>
      <table class="ws-table"><thead><tr><th>${t('archive.tlColYear')}</th><th>${t('archive.tlColType')}</th><th>${t('archive.tlColEvent')}</th><th>${t('archive.tlColSource')}</th><th>${t('archive.tlColLinks')}</th><th></th></tr></thead>
      ...
      <button class="chron-toggle tl-row-add" data-epoch="${escHtml(ep.heading)}">${t('archive.tlAddRow')}</button>
      <div class="tl-row-form" data-epoch-form="${escHtml(ep.heading)}" hidden></div>
    </section>`).join('');

  return `
    ${epochsHtml}
    <button class="chron-toggle" id="tl-add-epoch">${t('archive.tlAddEpoch')}</button>
    <div class="tl-epoch-form" id="tl-new-epoch-form" hidden>
      <input type="text" class="form-control" id="tl-new-epoch-heading" placeholder="${t('archive.tlEpochNamePh')}">
      <button class="chron-toggle" id="tl-new-epoch-save">${t('ui.btn.save')}</button>
    </div>
    <button class="chron-toggle" id="tl-raw-edit-toggle" style="margin-top:20px">${t('archive.tlEditWhole')}</button>
...
function _timelineRowFormHtml(epochHeading, rowIndex, row) {
  ...
  return `
    <div class="tl-form-fields">
      <input type="text" class="form-control tl-f-year" placeholder="${t('archive.tlYearPh')}" value="${escHtml(row?.year || '')}">
      <input type="text" class="form-control tl-f-type" list="tl-legend-datalist" placeholder="${t('archive.tlTypePh')}" value="${escHtml(row?.type || '')}">
      <textarea class="form-control tl-f-event" placeholder="${t('archive.tlEventPh')}">${escHtml(row?.event || '')}</textarea>
      <select class="form-control tl-f-source">
        <option value="📚" ${row?.source === '📚' ? 'selected' : ''}>${t('archive.srcCanon')}</option>
        <option value="🏙️" ${row?.source === '🏙️' ? 'selected' : ''}>${t('archive.srcProject')}</option>
        <option value="❓" ${row?.source === '❓' ? 'selected' : ''}>${t('archive.srcAmbiguous')}</option>
      </select>
      <div class="tl-f-links">
        <input type="text" class="form-control tl-link-search" placeholder="${t('archive.linkSearchPh')}">
        <div class="tl-link-suggest" hidden></div>
        <div class="tl-link-chips">${linkChips()}</div>
      </div>
      <button class="chron-toggle tl-row-save" data-epoch="${escHtml(epochHeading)}" data-row="${rowIndex ?? ''}">${t('ui.btn.save')}</button>
      <button class="chron-toggle tl-row-cancel">${t('ui.btn.cancel')}</button>
    </div>`;
}
...
async function openTimelineAddModal(prefill) {
  ...
  try {
    ...
    if (!data.epochs || !data.epochs.length) {
      body.innerHTML = `<div class="cdet-empty">${t('archive.noEpochsYet')}</div>`;
      return;
    }
    ...
    body.innerHTML = `
      <div class="tl-form-fields">
        ...
        <input type="text" class="form-control tl-f-year" placeholder="${t('archive.tlYearPh')}" value="${escHtml(year)}">
        ...
        <textarea class="form-control tl-f-event" placeholder="${t('archive.tlEventPh')}">**${escHtml(prefill.title || '')}.** </textarea>
        <select class="form-control tl-f-source">
          <option value="📚">${t('archive.srcCanon')}</option>
          <option value="🏙️" selected>${t('archive.srcProject')}</option>
          <option value="❓">${t('archive.srcAmbiguous')}</option>
        </select>
        <div class="tl-f-links">
          <input type="text" class="form-control tl-link-search" placeholder="${t('archive.linkSearchPh')}">
          <div class="tl-link-suggest" hidden></div>
    ...
    body.innerHTML = `<div class="cdet-empty">${t('archive.timelineLoadErr')}</div>`;
  }
}
...
document.getElementById('timeline-add-save').addEventListener('click', async () => {
  ...
  if (!year || !event) { errEl.textContent = t('archive.fillYearEvent'); return; }
  ...
  try {
    ...
    showToast(t('archive.rowAdded'), 'success');
    closeModal('timeline-add-modal');
  } catch (e) {
    errEl.textContent = t('archive.errorMsg', { msg: e.message });
  } finally { btn.disabled = false; }
});
document.getElementById('timeline-add-close').addEventListener('click', () => closeModal('timeline-add-modal'));
bindBackdropDblClickClose(document.getElementById('timeline-add-modal'), () => closeModal('timeline-add-modal'));
...
async function loadWorldStateForm() {
  ...
  try {
    ...
    el.innerHTML = `<div class="loading-state" style="color:var(--accent3)">${t('archive.wsLoadErr')}</div>`;
  }
}
  ...
  const sectionsHtml = (ws.sections || []).map(s => {
    ...
    <section class="ws-section" data-ws-section="${escHtml(s.heading)}">
      <div class="ws-heading">${escHtml(s.heading)}
        <button class="chron-toggle" data-ws-section-del="${escHtml(s.heading)}" title="${t('archive.wsSectionDel')}">🗑</button>
      </div>
      ...
      <tbody>${s.rows.map((row, i) => {
        ...
        <tr data-ws-section="${escHtml(s.heading)}" data-row="${i}"${full ? ' class="ws-clock-full"' : ''}>
          ...
          <td>${clock && !full ? `<button class="chron-toggle ws-clock-tick" data-ws-section="${escHtml(s.heading)}" data-row="${i}" title="${t('archive.wsClockTick')}">⏱ +1</button>` : ''}${full ? `<span class="ws-clock-hit" title="${t('archive.wsClockHit')}">🔥</span>` : ''}
      ...
      <button class="chron-toggle ws-row-add" data-ws-section="${escHtml(s.heading)}">${t('archive.wsAddRow')}</button>
      ...
      <button class="chron-toggle ws-note-edit" data-ws-section="${escHtml(s.heading)}">${t('archive.wsEditNote')}</button>
    </section>`;
  }).join('');

  return `
    <div class="ws-updated">${t('archive.wsUpdatedLabel')}
      <input type="text" class="form-control" id="ws-last-update-input" value="${escHtml(ws.lastUpdate || '')}">
      <button class="chron-toggle" id="ws-last-update-save">${t('ui.btn.save')}</button>
    </div>
    ${sectionsHtml}
    <button class="chron-toggle" id="ws-add-section">${t('archive.wsAddSection')}</button>
    <div class="ws-section-form" id="ws-new-section-form" hidden>
      <input type="text" class="form-control" id="ws-new-section-heading" placeholder="${t('archive.wsSecPh')}">
      <input type="text" class="form-control" id="ws-new-section-columns" placeholder="${t('archive.wsColsPh')}">
      <button class="chron-toggle" id="ws-new-section-save">${t('ui.btn.save')}</button>
    </div>
    <button class="chron-toggle" id="ws-raw-edit-toggle" style="margin-top:20px">${t('archive.wsEditBlock')}</button>
...
function _worldStateRowFormHtml(heading, rowIndex, columns, cells) {
  ...
  return `
    <div class="tl-form-fields">
      ${fields}
      <button class="chron-toggle ws-row-save" data-ws-section="${escHtml(heading)}" data-row="${rowIndex ?? ''}">${t('ui.btn.save')}</button>
      <button class="chron-toggle ws-row-cancel">${t('ui.btn.cancel')}</button>
    </div>`;
}
...
document.getElementById('chronicle-content').addEventListener('click', e => {
  ...
  if (e.target.id === 'tl-new-epoch-save') {
    ...
    if (!heading) { alert(t('archive.epochNameReq')); return; }
  ...
  if (epochDel) {
    if (!confirm(t('archive.confirmDelEpoch', { name: epochDel.dataset.epochDel }))) return;
  ...
  if (e.target.id === 'tl-raw-edit-toggle') {
    _loadArchiveEditable('/api/timeline', '/api/timeline', 'tl-raw-edit-container', t('archive.timelineEmpty'));
    return;
  }
  ...
  if (wsSectionDel) {
    if (!confirm(t('archive.confirmDelSection', { name: wsSectionDel.dataset.wsSectionDel }))) return;
  ...
  if (wsTick) {
    ...
    fetch(`/api/world-state/section/${encodeURIComponent(section.heading)}/row/${idx}${_cityQS()}`, {
      ...
      .catch(err => { showToast(t('archive.clockTickErr', { msg: err.message }), 'error'); wsTick.disabled = false; });
    return;
  }
  ...
  if (wsNoteEdit) {
    ...
    view.innerHTML = `<textarea class="form-control ws-f-note">${escHtml(section.note)}</textarea>
      <button class="chron-toggle ws-note-save" data-ws-section="${escHtml(wsNoteEdit.dataset.wsSection)}">${t('ui.btn.save')}</button>`;
    return;
  }
  ...
  if (e.target.id === 'ws-raw-edit-toggle') {
    _loadArchiveEditable('/api/world-state/raw', '/api/world-state/raw', 'ws-raw-edit-container', t('archive.wsBlockEmpty'));
    return;
  }
  ...
  if (tog) {
    ...
    if (body) {
      ...
      if (open) {
        body.removeAttribute('hidden'); tog.textContent = t('archive.collapse');
        tog.setAttribute('aria-expanded', 'true');
        _loadEventFinale(body);
      } else { body.setAttribute('hidden', ''); tog.textContent = t('archive.details');
    ...
    .then(d => {
      finEl.innerHTML = d.finale
        ? `<div class="md-body chron-md">${mdToHtmlPlain(d.finale)}</div>
           <button class="chron-toggle tl-finale-to-timeline" data-chr="${escHtml(d.chronicle || '')}" data-mod="${escHtml(finEl.dataset.finaleMod)}" data-title="${escHtml(d.title || '')}">${t('timeline.add.title')}</button>`
        : `<div class="cdet-empty">${t('archive.finaleNotWritten')}</div>`;
    })
    .catch(() => { finEl.innerHTML = `<div class="cdet-empty">${t('archive.finaleLoadErr')}</div>`; });
}
