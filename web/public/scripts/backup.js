  ...
  return `
    ...
    <div class="ls-section">
      ...
      <div class="btn-row">
        ...
        <button class="btn-submit btn-secondary" id="bkp-settings-restore-trigger">Загрузить настройки…</button>
      </div>
    </div>
...
function wireBackupHandlers() {
  ...
  document.getElementById('bkp-settings-restore-trigger').addEventListener('click', async () => {
    const ok = await showConfirm('Заменит текущие ключи AI и назначения генераций содержимым архива (прежняя версия сохранится на сервере — восстановить можно вручную). Продолжить?', { danger: true });
...
async function runSettingsRestore(file) {
  ...
  try {
    ...
    if (!resp.ok || !data.ok) throw new Error(data.error || data.results?.env || 'не удалось восстановить настройки');
    ...
    if (data.needsRestart && data.supervised) {
      btn.textContent = '✓ Восстановлено — сервер перезапускается…';
      ...
      showToast('Настройки восстановлены. Перезапусти сервер вручную, чтобы новые ключи применились.', 'warning');
      btn.disabled = false;
      btn.textContent = prevText;
    } else {
      showToast('Настройки восстановлены', 'success');
      btn.disabled = false;
      btn.textContent = prevText;
    }
  } catch (e) {
