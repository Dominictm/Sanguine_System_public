...
function _audioLibCardHtml(tr) {
  ...
  const loopOn   = tr.loop !== false;
  const isMusic  = tr.category === 'music';
  return `<div class="audio-card" data-audio-id="${escAttr(tr.id)}">
    <div class="audio-card-title" data-audio-title-view>${escHtml(tr.title)}</div>
    <audio data-audio-el src="${escAttr(tr.url)}" ${loopOn ? 'loop' : ''} preload="none"></audio>
    <div class="audio-card-row">
      <button type="button" class="audio-card-play-btn" data-audio-play aria-label="${t('audio.playPause')}">▶</button>
      <button type="button" class="audio-card-icon-btn" data-audio-stop aria-label="${t('audio.stopAll')}">⏹</button>
      <input type="range" class="audio-card-volume" data-audio-volume min="0" max="1" step="0.01" value="${tr.volume}">
      <div class="audio-card-actions">
        <button type="button" class="audio-card-icon-btn" data-audio-category aria-label="${t('audio.switchCategory', { cat: isMusic ? t('audio.cat.music') : t('audio.cat.effect') })}">${isMusic ? '🎵' : '🔊'}</button>
        <button type="button" class="audio-card-icon-btn${loopOn ? ' active' : ''}" data-audio-loop aria-pressed="${loopOn}" aria-label="${t('audio.loop')}">🔁</button>
        <button type="button" class="audio-card-icon-btn" data-audio-rename aria-label="${t('audio.rename')}">✎</button>
        <button type="button" class="audio-card-icon-btn" data-audio-delete aria-label="${t('ui.btn.delete')}">🗑</button>
      </div>
    </div>
  </div>`;
}
...
function _audioLibRender() {
  ...
  musicEl.innerHTML = music.length ? music.map(_audioLibCardHtml).join('') : `<div class="loading-state audio-lib-col-empty">${t('audio.emptyMusic')}</div>`;
  fxEl.innerHTML    = fx.length    ? fx.map(_audioLibCardHtml).join('')    : `<div class="loading-state audio-lib-col-empty">${t('audio.emptyEffects')}</div>`;
}

async function loadAudioLibrary() {
  const musicEl = document.getElementById('audio-lib-music-cards');
  const fxEl    = document.getElementById('audio-lib-effects-cards');
  const loading = `<div class="loading-state"><div class="spinner"></div>${t('ui.loading')}</div>`;
  if (musicEl) musicEl.innerHTML = loading;
  if (fxEl)    fxEl.innerHTML    = loading;
  try {
    _audioLibCache = await apiFetch('/api/audio');
  } catch (e) {
    showToast(t('audio.loadError') + e.message, 'error');
    _audioLibCache = [];
  }
  _audioLibRender();
  ...
  if (playBtn) {
    ...
    if (audioEl.paused) {
      ...
      try {
        ...
        showToast(t('audio.playError') + err.message, 'error');
      }
    } else {
      audioEl.pause();
      playBtn.textContent = '▶';
...
async function _audioLibToggleLoop(card, loopBtn) {
  ...
  try {
    ...
    showToast(t('audio.loopSaveError') + e.message, 'error');
  }
}
...
async function _audioLibToggleCategory(card, btn) {
  ...
  try {
    ...
    btn.setAttribute('aria-label', t('audio.switchCategory', { cat: entry.category === 'music' ? t('audio.cat.music') : t('audio.cat.effect') }));
    ...
    showToast(t('audio.switchCategoryError') + e.message, 'error');
  }
}

function _audioLibRefreshColumnEmptyStates() {
  [
    ['audio-lib-music-cards', t('audio.emptyMusic')],
    ['audio-lib-effects-cards', t('audio.emptyEffects')],
...
function _audioLibDebouncedSaveVolume(id, volume) {
  ...
  _audioLibVolumeTimers[id] = setTimeout(async () => {
    ...
    }); } catch (e) { showToast(t('audio.volumeSaveError') + e.message, 'error'); }
  }, 400);
}
...
function _audioLibStartRename(card) {
  ...
  const commit = async () => {
    ...
    try {
      ...
      showToast(t('audio.renameError') + e.message, 'error');
    }
    _audioLibRender();
  };
...
async function _audioLibDelete(card) {
  const id = card.dataset.audioId;
  const entry = _audioLibCache.find(t => t.id === id);
  if (!await showConfirm(t('audio.deleteConfirm', { name: entry?.title || '' }), { danger: true, confirmText: t('ui.btn.delete') })) return;
  try {
    await apiFetch(`/api/audio/${encodeURIComponent(id)}`, { method: 'DELETE' });
    _audioLibCache = _audioLibCache.filter(t => t.id !== id);
    _audioLibRender();
  } catch (e) {
    showToast(t('audio.deleteError') + e.message, 'error');
  }
}
...
function _audioPresetTrackPickerRowHtml(tr) {
  return `<label class="audio-preset-picker-row">
    <input type="checkbox" data-preset-pick-track value="${escAttr(tr.id)}">
    <span class="audio-preset-picker-title">${escHtml(tr.title)}</span>
    <input type="range" data-preset-pick-volume min="0" max="1" step="0.01" value="${tr.volume}" disabled>
...
async function _audioPresetPopulateLocationSelect(selectedSlug = '') {
  ...
  sel.innerHTML = `<option value="">${t('audio.presetNoLocation')}</option>` +
...
async function _audioPresetOpenModal(preset = null) {
  ...
  document.getElementById('audio-preset-modal-title').textContent = preset ? t('audio.presetEditTitle') : t('audio.presetCreateTitle');
...
bindBackdropDblClickClose(_audioPresetModal, () => closeModal('audio-preset-modal'));
...
document.getElementById('audio-preset-form')?.addEventListener('submit', async e => {
  ...
  if (!name) { errEl.textContent = t('audio.nameRequired'); errEl.style.display = ''; return; }
  if (!tracks.length) { errEl.textContent = t('audio.tracksRequired'); errEl.style.display = ''; return; }
  ...
  try {
    ...
    showToast(_audioPresetEditingId ? t('audio.presetUpdated') : t('audio.presetCreated'), 'success');
  } catch (e) {
...
function _audioPresetCardHtml(p) {
  ...
  const tracksHtml = p.tracks.map(tr => {
    const loopOn = tr.loop !== false;
    return `
    <div class="audio-preset-track-row" data-preset-track-id="${escAttr(tr.trackId)}">
      <span class="audio-preset-track-title">${escHtml(tr.title)}</span>
      <input type="range" data-preset-track-volume min="0" max="1" step="0.01" value="${tr.volume}">
      <button type="button" class="audio-preset-track-loop${loopOn ? ' active' : ''}" data-preset-track-loop aria-pressed="${loopOn}" aria-label="${t('audio.loop')}">🔁</button>
    </div>`;
  }).join('');
  return `<div class="audio-preset-card${hasBg ? ' has-bg' : ''}" data-preset-id="${escAttr(p.id)}">
    ...
    <div class="audio-preset-content">
      ...
      <div class="audio-card-row">
        <button type="button" class="audio-card-play-btn${isActive ? ' playing' : ''}" data-preset-play aria-label="${t('audio.presetPlay')}">${isActive ? '⏸' : '▶'}</button>
        <div class="audio-card-actions">
          <button type="button" class="audio-card-icon-btn" data-preset-edit aria-label="${t('audio.presetEdit')}">✎</button>
          <button type="button" class="audio-card-icon-btn" data-preset-delete aria-label="${t('audio.presetDelete')}">🗑</button>
        </div>
      </div>
    </div>
  </div>`;
...
function _audioPresetRender() {
  ...
  el.innerHTML = (_audioPresetCache && _audioPresetCache.length)
    ...
    : `<div class="loading-state">${t('audio.emptyPresets')}</div>`;
}
...
async function loadAudioPresets() {
  try {
    ...
    showToast(t('audio.presetLoadError') + e.message, 'error');
    _audioPresetCache = [];
  }
  _audioPresetRender();
}
...
async function _audioPresetPlay(presetId) {
  ...
  for (const tr of preset.tracks) {
    const card = document.querySelector(`.audio-card[data-audio-id="${CSS.escape(tr.trackId)}"]`);
    if (!card) continue; // трек удалён из библиотеки — тихо пропускаем
    const audioEl = card.querySelector('[data-audio-el]');
    audioEl.volume = tr.volume;
    audioEl.loop = tr.loop !== false;
    try {
      await audioEl.play();
      const playBtn = card.querySelector('[data-audio-play]');
      playBtn.textContent = '⏸';
      ...
      showToast(t('audio.presetPlayError', { name: tr.title }) + e.message, 'error');
    }
  }
...
async function _audioPresetDelete(presetId) {
  const preset = (_audioPresetCache || []).find(p => p.id === presetId);
  if (!await showConfirm(t('audio.presetDeleteConfirm', { name: preset?.name || '' }), { danger: true, confirmText: t('ui.btn.delete') })) return;
  try {
    ...
    showToast(t('audio.presetDeleteError') + e.message, 'error');
  }
}
...
function _audioPresetDebouncedSaveTrackVolume(presetId, trackId, volume) {
  ...
  _audioPresetVolumeTimers[key] = setTimeout(async () => {
    ...
    try {
      ...
      showToast(t('audio.presetVolumeError') + e.message, 'error');
    }
  }, 400);
}
...
bindBackdropDblClickClose(_audioUploadModal, () => closeModal('audio-upload-modal'));
...
document.getElementById('audio-upload-form')?.addEventListener('submit', async e => {
  ...
  if (!title) { errEl.textContent = t('audio.nameRequired'); errEl.style.display = ''; return; }
  if (!categoryInput) { errEl.textContent = t('audio.uploadCategoryRequired'); errEl.style.display = ''; return; }
  if (!file)  { errEl.textContent = t('audio.uploadFileRequired'); errEl.style.display = ''; return; }
  if (!ALLOWED_AUDIO_MIME.includes(file.type)) {
    errEl.textContent = t('audio.uploadBadFormat'); errEl.style.display = ''; return;
  }
  if (file.size > MAX_AUDIO_BYTES) {
    errEl.textContent = t('audio.uploadTooBig'); errEl.style.display = ''; return;
  }
