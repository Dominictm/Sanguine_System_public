...
async function loadSessionScreen() {
  ...
  chrSel.innerHTML = `<option value="">${t('session.chronicle.ph')}</option>` +
    (chrs || []).map(c => `<option value="${escHtml(c.slug)}">${escHtml((c.hidden ? '📂 ' : '') + (c.display || c.slug))}${c.status === 'closed' ? ` ${t('session.closedSuffix')}` : ''}</option>`).join('');
...
async function _sessLoadModules(chr, preselect) {
  ...
  modSel.innerHTML = `<option value="">${t('session.module.ph')}</option>`;
...
function _sessClearModule() {
  ...
  document.getElementById('sess-scenario').innerHTML = `<div class="cdet-empty">${t('session.empty')}</div>`;
...
async function _sessLoadModule(chr, mod) {
  ...
  scEl.innerHTML = `<div class="loading-state"><div class="spinner"></div>${t('ui.loading')}...</div>`;
  _sessBlocks = [];
  ...
  try {
    ...
    scEl.innerHTML = `<div class="cdet-empty">⚠ ${t('session.loadError')}</div>`;
    _sessBlocks = [];
    _sessSyncSceneNavVisibility();
    return;
  }
  ...
  _sessBlocks = raw.trim()
    ? raw.split(/\n(?=##\s+)/).map(part => {
        ...
        return m ? { heading: m[1].trim(), body: m[2] } : { heading: t('session.intro'), body: part };
...
function _sessRenderScenario() {
  ...
  if (!_sessBlocks.length) {
    scEl.innerHTML = `<div class="cdet-empty">${t('session.noScenario')}</div>`;
    return;
  }
  ...
  scEl.innerHTML = _sessBlocks.map((b, i) => `
    <section class="sess-block${i === _sessScene ? ' open' : ''}" data-scene="${i}">
      <button class="sess-block-head" data-scene-head="${i}">${escHtml(b.heading || t('session.noHeading'))}</button>
...
function _sessRenderNpcs() {
  ...
  if (!people.length) {
    el.innerHTML = `
      <div class="sess-side-title">👥 ${t('session.npcsTitle')}</div>
      <div class="cdet-empty">${t('session.noParticipants')}</div>`;
    return;
  }
  el.innerHTML = `
    <div class="sess-side-title">👥 ${t('session.npcsTitle')}</div>
...
async function _sessRenderAudio() {
  ...
  el.innerHTML = `
    <div class="sess-side-title">🎵 ${t('session.audioTitle')}</div>
    <div class="sess-preset-row">
      ...
      <button class="chron-toggle sess-preset-btn" id="sess-preset-toggle" title="${selectedId === activeId ? t('session.audioStop') : t('session.audioPlay')}">${selectedId === activeId ? '⏹' : '▶'}</button>
    </div>`;
}
...
function _sessBindSaveButton(btn, ta, getLoaded, onSave) {
  ...
  btn.addEventListener('click', async () => {
    ...
    btn.disabled = true;
    btn.textContent = `⏳ ${t('session.saving')}…`;
    try {
      ...
      btn.textContent = label;
      btn.disabled = false;
      showToast(t('session.saveError') + err.message, 'error');
    }
  });
  return syncDirty;
}
...
function _sessSceneNoteHistoryHtml(entries, currentNum) {
  ...
  return `<div class="sess-scene-note-history">${past.map(e => `
    <div class="sess-scene-note-entry">
      <div class="sess-scene-note-entry-tag">${e.session ? `${t('session.sessionNum', { n: e.session })}` : t('session.noSessionTag')}</div>
...
function _sessRenderSceneNote() {
  ...
  wrap.innerHTML = `
    ...
    <textarea class="form-control" id="sess-scene-note" rows="6" placeholder="${t('session.sceneNotePh')}" aria-labelledby="sess-scene-note-title"></textarea>
    <button class="chr-modal-btn" id="sess-scene-note-save" disabled>${t('ui.btn.save')}</button>`;
  ...
  _sessBindSaveButton(btn, ta, () => _sessSceneNoteLoaded, async text => {
    ...
    if (!r.ok) throw new Error(result.error || t('session.saveErr'));
  ...
  document.getElementById('sess-scene-note-title').textContent = t('session.sceneNoteTitle', { heading, n: currentNum });
  const curEntry = entries.find(e => e.session === currentNum);
  _sessSceneNoteLoaded = curEntry?.text || '';
  ta.value = _sessSceneNoteLoaded;
  btn.disabled = true;
  btn.textContent = t('ui.btn.save');
}
...
function _sessRenderNotes() {
  ...
  wrap.innerHTML = `
    <div class="sess-side-title">📝 ${t('session.notesTitle')}</div>
    <textarea class="form-control" id="sess-notes" rows="8" placeholder="${t('session.notesPh')}"></textarea>
    <button class="chr-modal-btn" id="sess-notes-save" disabled>${t('ui.btn.save')}</button>
    <button class="chr-modal-btn" id="sess-to-log">${t('session.toLog')}</button>`;
  ...
  wrap.querySelector('#sess-to-log').addEventListener('click', () => {
    ...
    if (!chr || !mod) {
      showToast(t('session.pickBeforeLog'), 'error');
      return;
    }
    ...
    if (otherParts.length) noteParts.push(`${t('session.miscBlock')}\n\n${otherParts.join('\n\n')}`);
...
document.getElementById('page-session').addEventListener('change', e => {
  ...
  const label = isActive ? t('session.audioStop') : t('session.audioPlay');
  btn.title = label;
});
