...
function lsSceneRow() {
  ...
  d.innerHTML = `
    <input class="form-control ls-scene-title" placeholder="${t('ls.sceneNamePh')}">
    <textarea class="form-control ls-scene-text" rows="1" placeholder="${t('ls.sceneWhatPh')}"></textarea>
    <button class="ls-del" type="button" title="${t('ls.remove')}">✕</button>`;
  return d;
}
function lsPartRow() {
  const d = document.createElement('div');
  ...
  d.innerHTML = `
    <div class="ls-row">
      <input class="form-control ls-part-name" list="ls-charnames" placeholder="${t('ls.partNamePh')}">
      <input class="form-control ls-part-role" placeholder="${t('ls.partRolePh')}">
      <label class="ls-check"><input type="checkbox" class="ls-part-diary"> ${t('ls.partDiary')}</label>
      <label class="ls-check"><input type="checkbox" class="ls-part-pc"> ${t('ls.partPc')}</label>
      <button class="ls-del" type="button" title="${t('ls.remove')}">✕</button>
    </div>
    <div class="ls-row ls-row-sub">
      <select class="form-control ls-part-lineage" title="${t('ls.lineageHint')}">
        <option value="">${t('ls.lineageExisting')}</option>
        <option value="vampires">🧛 ${t('ls.newLineage.vampires')}</option>
        <option value="fairies">🧚 ${t('ls.newLineage.fairies')}</option>
        <option value="mortals">🧑 ${t('ls.newLineage.mortals')}</option>
        <option value="werewolves">🐺 ${t('ls.newLineage.werewolves')}</option>
        <option value="mages">🔮 ${t('ls.newLineage.mages')}</option>
        <option value="hunters">🏹 ${t('ls.newLineage.hunters')}</option>
      </select>
      <input class="form-control ls-part-status" placeholder="${t('ls.partStatusPh')}">
      <input class="form-control ls-part-statusd" placeholder="${t('ls.partStatusDetailPh')}">
      <input class="form-control ls-part-comment" placeholder="${t('ls.partCommentPh')}">
    </div>`;
  return d;
}
function lsThreadRow() {
  ...
  d.innerHTML = `
    <input class="form-control ls-thread-title" placeholder="${t('ls.threadTitlePh')}">
    <input class="form-control ls-thread-desc" placeholder="${t('ls.threadDescPh')}">
    <select class="form-control ls-thread-prio"><option>${t('threads.prio.high')}</option><option>${t('threads.prio.medium')}</option><option>${t('threads.prio.low')}</option></select>
    <button class="ls-del" type="button" title="${t('ls.remove')}">✕</button>`;
  return d;
}
...
const LS_ACTION_LABEL = { create: t('ls.actCreate'), modify: t('ls.actModify') };
...
async function lsRunPreview() {
  ...
  } catch { res.innerHTML = `<div class="ls-err">⚠ ${t('ls.serverDown')}</div>`; return; }

  if (!j.ok) {
    lsInvalidate();
    res.innerHTML = `<div class="ls-err"><b>${t('ls.cannotWrite')}:</b><ul>${(j.errors || []).map(e => `<li>${escHtml(e)}</li>`).join('')}</ul></div>`;
    return;
  }
  ...
  res.innerHTML = `
    <div class="ls-preview-head">
      <span class="ls-badge">${t('ls.previewBadge')}</span>
      ${t('ls.previewModule')} <b>${escHtml(s.module || '')}</b>${s.moduleNew ? ` ${t('ls.moduleNew')}` : ''} ·
      ${t('ls.previewParts')}: ${s.participants} · ${t('ls.previewDiaries')}: ${s.diaries} · ${t('ls.previewFinale')}: ${s.finale ? t('ls.yes') : t('ls.no')}
    </div>
    ...
    ${(j.stubs || []).length ? `<div class="ls-stubs"><b>${t('ls.proseStubs')}:</b><ul>${j.stubs.map(x => `<li>${escHtml(x)}</li>`).join('')}</ul></div>` : ''}
    <div class="ls-hint-confirm">${t('ls.previewHint')}</div>`;
}
...
async function lsRunWrite() {
  ...
  wbtn.disabled = true; wbtn.textContent = `⏳ ${t('ls.writing')}...`;
  let j;
  try {
    j = await fetch('/api/log-session', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  } catch { res.innerHTML = `<div class="ls-err">⚠ ${t('ls.serverDown')}</div>`; wbtn.textContent = t('ls.writeBtn'); return; }
  wbtn.textContent = t('ls.writeBtn');

  if (!j.ok) {
    lsInvalidate();
    res.innerHTML = `<div class="ls-err"><b>${t('ls.notWritten')}:</b><ul>${(j.errors || []).map(e => `<li>${escHtml(e)}</li>`).join('')}</ul>${t('ls.retryPreview')}</div>`;
    return;
  }
  ...
  res.innerHTML = `
    <div class="ls-ok-head">✓ ${t('ls.written', { n: (j.written || []).length })}</div>
...
async function lsSetupProseZone(stubs) {
  ...
  const claudeNote = claudeAvail
    ? t('ls.proseClaudeTip')
    : `<span style="color:var(--accent2)">${t('ls.proseClaudeMissing')}</span>`;
  const apiBtnLabel = isOpenAI ? t('ls.genProseGpt') : t('ls.genProseOpenRouter');

  zone.innerHTML = `
    <div class="ls-prose-controls">
      ${useApi ? '' : `<select class="form-control ls-prose-model" id="ls-prose-model">
        <option value="">${t('ls.modelDefault')}</option>
        <option value="opus">${t('ls.modelOpus')}</option>
        <option value="sonnet">${t('ls.modelSonnet')}</option>
        <option value="haiku">${t('ls.modelHaiku')}</option>
      </select>`}
      ${useApi
        ? `<button class="btn-submit btn-genprose btn-genprose-or" id="ls-genprose" type="button">${apiBtnLabel}</button>`
        : `<button class="btn-submit btn-genprose" id="ls-genprose" type="button" ${claudeAvail ? '' : 'disabled'}>🪄 ${t('ls.genProseClaude')}</button>`
      }
    </div>
    <div class="ls-prose-note">${useApi
      ? t('ls.proseApiNote', { provider: isOpenAI ? 'GPT' : 'OpenRouter' })
      : claudeNote
    } ${t('ls.proseConfigure')}: <a class="dash-ai-link" data-nav="tools" data-tab="ai-connect">${t('ls.proseModelsLink')} ↗</a></div>
...
async function lsGenProse(stubs, source = 'claude') {
  ...
  const apiBtnLabel = isOpenAI ? t('ls.genProseGpt') : t('ls.genProseOpenRouter');
  const providerName = isOpenAI ? 'GPT' : (useApi ? 'OpenRouter' : 'Claude');
  ...
  btn.disabled = true;
  btn.textContent = `⏳ ${t('ls.proseWriting', { provider: providerName })}…`;
  out.innerHTML = `<div class="ls-note">${t('ls.proseGeneration')}</div>`;
  ...
  try {
    ...
    btn.disabled = false; btn.textContent = useApi ? apiBtnLabel : `🪄 ${t('ls.genProseClaude')}`;
    return;
  }

  if (!j.ok && !(j.written || []).length) {
    out.innerHTML = `<div class="ls-err">⚠ ${escHtml(j.error || t('ls.proseGenFailed'))}</div>`;
    btn.disabled = false; btn.textContent = useApi ? apiBtnLabel : `🪄 ${t('ls.genProseClaude')}`;
    return;
  }

  STATE.characters = []; STATE.chronicle = null; STATE.graph.inited = false;
  btn.textContent = `✓ ${t('ls.done')}`;
  out.innerHTML = `
    <div class="ls-ok-head">✓ ${t('ls.proseWritten')}${j.cost != null ? ` · $${Number(j.cost).toFixed(3)}` : ''}</div>
    <div class="ls-changes">${(j.written || []).map(s =>
      `<div class="ls-change"><span class="ls-change-act act-modify">✎ ${t('ls.actProse')}</span><span class="ls-change-rel">${escHtml(s)}</span></div>`).join('')}</div>
    ${(j.pending || []).length ? `<div class="ls-warn">⚠ ${t('ls.prosePending')}: ${j.pending.map(escHtml).join(', ')}</div>` : ''}
    <div class="ls-note">${t('ls.gitDiffNote')}</div>`;
}
