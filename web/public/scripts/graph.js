...
// Служебные лейблы (neutral/aggregate) показываются в фильтре чипов и панели —
// локализуются, в отличие от городских видов связи, которые приходят готовыми
// с /api/graph (typeLabel из документа города, язык города не меняется).
function relTypeLabel(k) {
  return REL_LABELS[k] ? t('graph.rtype.' + k) : null;
}
...
async function loadGraph() {
  ...
  // Загружаем персонажей только если ещё не загружены (для портретов узлов).
  // Если STATE.characters уже заполнен (напр. с «Персонажей») — экономим запрос.
  const [charsRes, graphRes] = await Promise.all([
    !STATE.characters.length ? fetch('/api/characters').then(r => r.json()).catch(() => null) : Promise.resolve(null),
    fetch('/api/graph').then(r => r.json()).catch(() => null),
  ]);
  if (Array.isArray(charsRes)) STATE.characters = charsRes;
  ...
  if (!data.nodes.length) {
    ...
    if (!document.getElementById('graph-empty-state')) {
      ...
      overlay.textContent = t('graph.empty');
...
function buildRelTypeFilter() {
  ...
  for (const l of links) {
    ...
    meta.set(l.type, {
      label: l.typeLabel || relTypeLabel(l.type) || l.type,
  ...
  const keys = [...present].sort((a, b) => {
    const la = meta.get(a)?.label || relTypeLabel(a) || a;
    const lb = meta.get(b)?.label || relTypeLabel(b) || b;
  ...
  document.getElementById('graph-reltype-filter').innerHTML = keys.map(k => {
    const m = meta.get(k) || { label: relTypeLabel(k) || k, color: REL_COLORS[k] || REL_COLORS.neutral };
    return `
    <label class="graph-filter-chip">
...
function showInfoPanel(d, links, nodes) {
  ...
  document.getElementById('info-content').innerHTML = `
    ...
    <div class="info-section-label">${t('graph.relsCount', { n: outLinks.length })}</div>
    ${relsHtml || `<div style="color:var(--text3);font-size:var(--fs-body);font-style:italic">${t('graph.noRels')}</div>`}
  `;
