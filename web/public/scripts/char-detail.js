...
document.getElementById('chars-grid').addEventListener('click', e => {
  ...
  if (!card) return;
  if (typeof _charExportMode !== 'undefined' && _charExportMode) { _charExportToggleCard(card.dataset.slug); return; }
...
bindBackdropDblClickClose(charDetailModal, () => closeModal('char-detail-modal'));
...
document.getElementById('char-detail-content').addEventListener('click', e => {
  ...
  // Belongings (вкладка «Имущество»): добавить / править подпись / удалить / просмотр
  ...
  const belImg = e.target.closest('.cdet-bel-tile');
  if (belImg && !e.target.closest('.cdet-bel-btn')) {
    const img = belImg.classList.contains('cdet-bel-img') ? belImg : belImg.querySelector('.cdet-bel-img');
    const slug = img?.dataset.char;
    const file = img?.dataset.file || belImg.dataset.file;
    if (slug && file) _openBelongingLightbox(slug, file);
    return;
  }
...
// Двойной клик по арту в модалке → лайтбокс по центру экрана.
// Переиспользуем глобальный #img-lightbox из locations.js.
document.getElementById('char-detail-content').addEventListener('dblclick', e => {
  const img = e.target.closest('#cdet-carousel-img');
  if (!img) return;
  const images = _carouselImages.length ? _carouselImages : [img.src];
  const idx = _carouselImages.length ? _carouselIdx : 0;
...
async function initCarousel(charSlug) {
  ...
  // Стартуем с того же изображения, что уже показано на карточке (c.imageUrl).
  // HTML-шаблон стартовал с c.imageUrl (последний slug_NN.* файл), а API /images
  // отдаёт массив по алфавиту — индекс может быть любым. Без поиска клик «назад»
  // из позиции 0 вёл на images[length-1] = другой файл.
  const imgEl = document.getElementById('cdet-carousel-img');
  const currentSrc = imgEl?.getAttribute('src') || '';
  const startIdx = images.findIndex(u => u === currentSrc);
  _carouselIdx = startIdx >= 0 ? startIdx : 0;
  // Синхронизируем src (на случай если currentSrc не совпал — фоллбэк на [0])
  if (imgEl) imgEl.src = images[_carouselIdx];
...
function renderBelongingsPanel(c) {
  ...
  const tiles = list.map((b, idx) => `
    <figure class="cdet-bel-tile" data-idx="${idx}" data-file="${escAttr(b.file)}">
      <img class="cdet-bel-img" src="${escAttr(_belThumbUrl(c, b))}" alt="${escHtml(b.caption)}" loading="lazy" decoding="async" data-char="${escHtml(c.slug)}" data-file="${escAttr(b.file)}" title="${t('cdet.belViewTitle')}">
      <figcaption class="cdet-bel-cap">${escHtml(b.caption)}</figcaption>
      ${b.description ? `<div class="cdet-bel-desc" title="${escAttr(b.description)}">${escHtml(b.description.slice(0,60))}${b.description.length>60?'…':''}</div>` : ''}
...
function _promptCaption({ title = t('cdet.belAddBtn'), initial = '' } = {}) {
  return new Promise(resolve => {
    ...
    bindBackdropDblClickClose(ov, () => cleanup(null));
    inp.focus();
    inp.select();
    validate();
  });
...
// Модалка вещи: подпись + описание (description опционально, 0..1000).
function _promptBelonging({ title = t('cdet.belAddBtn'), initialCaption = '', initialDescription = '' } = {}) {
  return new Promise(resolve => {
    const ov = document.createElement('div');
    ov.id = 'bel-caption-overlay';
    ov.setAttribute('role', 'dialog');
    ...
    ov.innerHTML = `
      <div class="confirm-box" style="max-width:520px">
        <div class="confirm-title">${escHtml(title)}</div>
        <label class="form-label" style="margin-top:8px">${t('cdet.belCaptionLabel')} *</label>
        <input class="bel-cap-inp form-control" id="_bel-cap-inp" maxlength="200" placeholder="${t('cdet.belCaptionPh')}" value="${escAttr(initialCaption)}">
        <div class="bel-cap-err" id="_bel-cap-err" style="display:none;color:var(--accent3);font-size:var(--fs-sm);margin-top:4px"></div>
        <label class="form-label" style="margin-top:10px">${t('cdet.belDescLabel')}</label>
        <textarea class="bel-desc-inp form-control" id="_bel-desc-inp" rows="3" maxlength="1000" placeholder="${t('cdet.belDescPh')}">${escHtml(initialDescription)}</textarea>
        <div class="bel-cap-hint" style="font-size:var(--fs-sm);color:var(--text3);margin-top:2px">${t('cdet.belDescHint')}</div>
        <div class="confirm-acts" style="margin-top:12px">
          <button class="chr-modal-btn cancel" id="_bel-cap-cancel" type="button">${t('cdet.cancelBtn')}</button>
          <button class="chr-modal-btn create" id="_bel-cap-ok" type="button" disabled>${t('cdet.saveBtn')}</button>
        </div>
      </div>`;
    document.body.appendChild(ov);
    const capInp = ov.querySelector('#_bel-cap-inp');
    const descInp = ov.querySelector('#_bel-desc-inp');
    const err = ov.querySelector('#_bel-cap-err');
    const ok  = ov.querySelector('#_bel-cap-ok');
    const cleanup = v => { ov.remove(); document.removeEventListener('keydown', onKey); resolve(v); };
    const validate = () => {
      const v = capInp.value.trim();
      const hasForbidden = /[[\]\r\n]/.test(v);
      const tooLong = v.length > 200;
      const descTooLong = descInp.value.trim().length > 1000;
      if (hasForbidden) { err.textContent = t('cdet.belCaptionInvalid'); err.style.display = ''; }
      else if (tooLong) { err.textContent = t('cdet.belCaptionTooLong'); err.style.display = ''; }
      else if (descTooLong) { err.textContent = t('cdet.belDescTooLong'); err.style.display = ''; }
      else err.style.display = 'none';
      ok.disabled = !v || hasForbidden || tooLong || descTooLong;
    };
    capInp.addEventListener('input', validate);
    descInp.addEventListener('input', validate);
    capInp.addEventListener('keydown', e => { if (e.key === 'Enter' && !ok.disabled) { e.preventDefault(); descInp.focus(); } });
    descInp.addEventListener('keydown', e => { if (e.key === 'Enter' && (e.ctrlKey || e.metaKey) && !ok.disabled) { e.preventDefault(); cleanup({ caption: capInp.value.trim(), description: descInp.value.trim().replace(/\s+/g,' ') }); } });
    const onKey = e => { if (e.key === 'Escape') cleanup(null); };
    document.addEventListener('keydown', onKey);
    ok.onclick = () => cleanup({ caption: capInp.value.trim(), description: descInp.value.trim().replace(/\s+/g,' ') });
    ov.querySelector('#_bel-cap-cancel').onclick = () => cleanup(null);
    bindBackdropDblClickClose(ov, () => cleanup(null));
    capInp.focus();
    capInp.select();
...
function _openBelongingLightbox(charSlug, file) {
  const c = STATE.characters.find(ch => ch.slug === charSlug);
  if (!c || !c.belongings?.length) return;
  const imgs = c.belongings.map(b => _belUrl(c, b));
  const metas = c.belongings.map(b => ({ caption: b.caption, description: b.description || '', file: b.file }));
  const idx = c.belongings.findIndex(b => b.file === file);
  openLightbox(imgs, idx >= 0 ? idx : 0, metas);
}
...
async function _triggerBelongingUpload(charSlug) {
  ...
  input.onchange = async () => {
    ...
    const file = input.files[0];
    if (!file) return;
    const prompted = await _promptBelonging({ title: t('cdet.belAddBtn'), initialCaption: '', initialDescription: '' });
    if (!prompted) return;
    const { caption, description } = prompted;
    ...
    try {
      ...
      const resp = await fetch(`/api/characters/${encodeURIComponent(charSlug)}/upload-belonging${window.location.search}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ base64, ext, caption, description })
      });
      const result = await resp.json();
      ...
      if (charInState) {
        ...
        charInState.belongings.push({ file: result.filename, caption: result.caption || caption, description: result.description || description });
      }
...
async function _editBelongingCaption(charSlug, file) {
  ...
  if (!item) return;
  const prompted = await _promptBelonging({ title: t('cdet.belCaptionEdit'), initialCaption: item.caption, initialDescription: item.description || '' });
  if (!prompted) return;
  const { caption, description } = prompted;
  if (caption === item.caption && description === (item.description || '')) return;
  try {
    ...
    item.caption = d.caption || caption;
    item.description = d.description !== undefined ? d.description : description;
    _repaintBelongingsPanel(charSlug);
    // Если лайтбокс открыт на этой вещи — обновить правую панель
    if (document.getElementById('img-lightbox')?.classList.contains('open') && _lbMeta) {
      const idx = _lbMeta.findIndex(m => m.file === file);
      if (idx !== -1) { _lbMeta[idx].caption = item.caption; _lbMeta[idx].description = item.description; _lbRender(); }
    }
