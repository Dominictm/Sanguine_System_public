...
bindBackdropDblClickClose(document.getElementById('chr-detail-modal'), () => closeModal('chr-detail-modal'));
...
bindBackdropDblClickClose(document.getElementById('mod-create-modal'), () => closeModal('mod-create-modal'));
...
bindBackdropDblClickClose(document.getElementById('mod-fill-modal'), () => {
  closeModal('mod-fill-modal');
  _fillModTarget = null;
});
...
document.getElementById('mod-fill-generate').addEventListener('click', async () => {
  ...
  try {
    ...
    const scenPref = _getPref(prefs, 'scenario', 'openrouter');
    const scenSrc = prefs.scenario ? scenPref.provider : null; // null → авто-выбор (как раньше)
    const d  = await fetch(
      ...
      { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pcs: _fillPCs, npcs: _fillNPCs, content,
          locSource: locPref.provider, locModel: locPref.model,
          preferSource: scenSrc, orModel: scenSrc ? scenPref.model : null }) }
    ).then(r => r.json());
...
bindBackdropDblClickClose(document.getElementById('mod-delete-modal'), () => {
  closeModal('mod-delete-modal');
  _modDeleteTarget = null;
});
...
bindBackdropDblClickClose(document.getElementById('chr-create-modal'), () => closeModal('chr-create-modal'));
...
bindBackdropDblClickClose(document.getElementById('chr-delete-modal'), () => {
  closeModal('chr-delete-modal');
  _chrDeleteSlug = null;
});
...
bindBackdropDblClickClose(document.getElementById('finale-preview-modal'), () => closeModal('finale-preview-modal'));
...
function _buildScenarioSkeleton(title, modSlug) {
  return [
    `# Сценарий — ${title}`,
    '',
    `> 🔗 [Модуль](${modSlug}.md) | [Хроника](../../chronicle.md) | [НПС](npc.md)`,
    '',
    '<!-- meta:npcs: -->',
    '<!-- meta:locations: -->',
    '',
...
document.getElementById('modp-panel-scenario').addEventListener('click', e => {
  ...
  if (e.target.id === 'modp-regen-scenario-btn') {
    ...
    (async () => {
      ...
      try {
        const fprefs  = JSON.parse(localStorage.getItem('ai-feature-prefs') || '{}');
        const scenPref = _getPref(fprefs, 'scenario', 'openrouter');
        const scenSrc = fprefs.scenario ? scenPref.provider : null; // null → авто-выбор (как раньше)
        const r = await fetch(
  ...
  if (regenSecBtn) {
    ...
    (async () => {
      ...
      try {
        const fprefs2  = JSON.parse(localStorage.getItem('ai-feature-prefs') || '{}');
        const scenPref2 = _getPref(fprefs2, 'scenario', 'openrouter');
        const scenSrc2  = fprefs2.scenario ? scenPref2.provider : null;
        const r = await fetch(
          ...
          { method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              pcs:  (d.pcs  || []).map(p => typeof p === 'string' ? p : p.name),
              npcs: (d.npcs || []).map(p => typeof p === 'string' ? p : p.name),
              preferSource: scenSrc2,
              orModel: scenSrc2 ? scenPref2.model : null,
            })
          }
        );
        const result = await r.json().catch(() => ({}));
...
document.getElementById('modp-gen-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (d.normalized)   lines.push(t('mod.genScenarioNormalized'));
    if (d.structureWarnings?.length) {
      lines.push(t('mod.genScenarioStructureWarn'));
      for (const w of d.structureWarnings) lines.push(`🟡 ${w}`);
    }
...
bindBackdropDblClickClose(document.getElementById('modp-close-modal'), () => closeModal('modp-close-modal'));
...
bindBackdropDblClickClose(document.getElementById('modp-delete-modal'), () => closeModal('modp-delete-modal'));
