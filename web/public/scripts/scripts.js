...
// «◂») и медиазапросом max-width:700px, где класса нет. Проверять надо оба,
// иначе на узком окне вся логика рейла молча не включится.
function _navRail() {
...
function _navGroupMarkers() {
  document.querySelectorAll('.nav-group').forEach(sec => {
    ...
    btn.setAttribute('aria-label', marked ? `${name} — здесь открытая страница` : name);
  });
}
...
(function () {
  ...
  modal.addEventListener('click', e => { if (e.target === modal) closeModal('about-modal'); });
})();
...
(function () {
  ...
  function show(item) {
    // Оба источника рейла, а не только класс: на окне < 700px подписи пунктов
...
function _renderFeatCard(feat, icon, label, desc, pref, orModels) {
  ...
  return `
    <div class="ais-feat-card" data-feat="${feat}">
      ...
      <div class="ais-feat-card-radios" role="radiogroup" aria-label="Провайдер — ${escAttr(label)}">
        ${radio('openrouter', 'OpenRouter')}
        ${radio('openai', 'GPT')}
        ${radio('claude', 'Claude')}
        ${radio('gemini', 'Gemini')}
      </div>
      <select class="ais-feat-model-select" id="feat-${feat}-model" aria-label="Модель — ${escAttr(label)}">${opts}</select>
    </div>`;
}
    ...
    <div class="ais-right">
      <div class="ais-section ais-features-section">
        ...
        <div class="ais-feat-actions">
          <button class="ais-confirm-btn" id="ais-feat-save">✓ Применить</button>
