...
//      web/tests, CLAUDE.md, DESIGN.md, web/DESIGN.md — дев-окружение, инструменты
//      разработки и внутренняя документация, не нужны Рассказчику в релизе).
//   4. Опустошает корзины Библиотеки: system/library/<раздел>/_deleted.json → [] и
//      system/library/<раздел>/_deleted/ удаляется — пользователь получает релиз без
//      чужих удалённых записей (страховка от тестового мусора, закоммиченного по ошибке).
//
// Используется автосборкой релиза (.github/workflows/release-public.yml) и локально:
...
function resetLibraryTrash() {
  const libDir = path.join(ROOT, 'system', 'library');
  if (!fs.existsSync(libDir)) return [];
  const reset = [];
  for (const entry of fs.readdirSync(libDir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    const json = path.join(libDir, entry.name, '_deleted.json');
    if (fs.existsSync(json) && fs.readFileSync(json, 'utf8').trim() !== '[]') {
      reset.push(`${entry.name}/_deleted.json`);
      if (!dryRun) fs.writeFileSync(json, '[]\n');
    }
    const bin = path.join(libDir, entry.name, '_deleted');
    if (fs.existsSync(bin)) {
      reset.push(`${entry.name}/_deleted/`);
      if (!dryRun) fs.rmSync(bin, { recursive: true, force: true });
    }
  }
  return reset;
}
...
function main() {
  ...
  const trash = resetLibraryTrash();
  console.log(`[build_release] reset library trash: ${trash.length ? trash.join(', ') : '(none)'}`);
}
