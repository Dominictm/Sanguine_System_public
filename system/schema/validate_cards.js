...
// Запуск:  node system/schema/validate_cards.js [--strict] [--root <dir>]
// До Фазы 5 новые поля (Слаг/Родной город) дают WARNING; с --strict — ERROR.
// Карточки локаций проверяются только на битые ссылки на файлы (art/, belongings/).

const fs = require('fs');
const path = require('path');

const _rootIdx = process.argv.indexOf('--root');
const ROOT = _rootIdx !== -1 ? path.resolve(process.argv[_rootIdx + 1]) : path.resolve(__dirname, '..', '..');
...
// Ссылки на файлы рядом с карточкой (секции «🖼️ Изображения», «🧳 Имущество»).
// Всегда WARNING (даже в --strict): вручную удалённая картинка не должна валить прогон.
function checkFileLinks(content, cardDir, W) {
  for (const m of content.matchAll(/\]\(<?((?:art|belongings)\/[^)>]+)>?\)/g)) {
    const rel = m[1].trim();
    let target;
    try { target = decodeURIComponent(rel); } catch { target = rel; }
    if (!exists(path.join(cardDir, target))) W(`битая ссылка на файл: ${rel}`);
  }
}
...
// Карточки локаций: cities/<city>/locations/<район>/<локация>/<локация>.md — только ссылки на файлы.
let locCount = 0;
for (const city of listDirs(path.join(ROOT, 'cities'))) {
  const ld = path.join(ROOT, 'cities', city, 'locations');
  for (const district of listDirs(ld)) {
    if (district.startsWith('_')) continue; // _deleted и прочие служебные
    for (const loc of listDirs(path.join(ld, district))) {
      const dir = path.join(ld, district, loc);
      const main = path.join(dir, loc + '.md');
      if (!exists(main)) continue;
      locCount++;
      const rel = path.relative(ROOT, main).replace(/\\/g, '/');
      checkFileLinks(fs.readFileSync(main, 'utf8'), dir, m => warns.push(`${rel}: ${m}`));
    }
  }
}
...
console.log(`\nКарточек: ${cardCount} · локаций: ${locCount} · ошибок: ${errors.length} · предупреждений: ${warns.length}${STRICT ? ' · режим: strict' : ''}`);
