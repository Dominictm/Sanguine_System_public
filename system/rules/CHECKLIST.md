...
- [ ] Добавить обратные ссылки в карточки сира / союзников (поле «Отношения», формат — [card_schema.md#Отношения](../schema/card_schema.md)). При создании через веб — чекбокс «Взаимно» и авто-пары из колонки «Пара» словаря города (`cities/<город>/rules/relation_types.md`: Сир↔Чайлд, Домитор↔Гуль, Брат/Сестра — по полю «Пол» цели) делают это автоматически; вручную нужно только при прямой правке Markdown
...
| [UI-тесты](../../web/tests/run-ui.bat) | Прогон UI-тестов разделов и модулей (см. протокол веб-интерфейса в `CLAUDE.md`) |
